...
Аппаратные замечания после Natro 2.8.4/2.8.5 и соответствующие изменения кандидата 2.8.6 актуализированы ниже; это не означает аппаратную приёмку до отдельного измерения на KX11.
...
## Дополнение 11.09.2026: кандидат 2.8.6 после фото KX11
...
строки на данной KX11. В кандидате 2.8.6 они удалены из runtime-пути и диагностики;
...
и загрузка package icon/uninstallability из `AppsAdapter.getView`. Это состояние исходников
до CI/подписи; аппаратные результаты проверяются `GATE-076…079`.
...
У unit enum `0=km/h`, `1=mph`, неизвестное значение не публикуется. Кандидат 2.8.6
...
В кандидате 2.8.6 resolver дополнительно сверяет список активных сессий каждые 2 минуты,
