...
KnobReceiver принимает SHOW и PREV/NEXT_1/2/3: цифра — число шагов по активному
списку, итоговый режим задаётся один раз. ISSHOWING — обратное уведомление о видимости.
На Android 14+ whitelist внешнего приложения не содержит ru.natro.statuswidget;
готовые действия Natro поэтому направляются в собственный контроллер, не подменяют отправителя.
Контроллер использует существующий CarIntegration vehicle.drive_mode SET и runtime-каталог,
без новых CAN/PA значений. SHOW не пишет в автомобиль, неизвестное состояние не заменяется
предположением. Физическая приёмка смены режимов и оформления GATE-104/105 открыта.

Сервисный режим DezzK/stealth 47ca7f6cc02f3edac5a3f4c220852b390f156b82 относится
к PackageManager Android: disable-user/enable, не сервисному диагностическому режиму ECU.
Перенос в Natro не доказывает доставку NEW_OUTGOING_CALL штатной звонилкой KX11.
OEM force-stop остаётся риском возврата по PIN. См. docs/SERVICE_MODE_PORT_RU.md,
GATE-106 открыт; на автомобиле команды при разработке не выполнялись.

## Вечер 14.09.2026 — отрицательная проверка хвостиков и музыки 2.9.4

В status-widget-debug(3).txt подтверждены два разных наблюдения: main Natro в записи
DiagnosticJournal (18:00:00.996; восстановление через 4698 мс) и шесть системных KeyEvent NEXT
с возрастающими eventTime ages вплоть до 88129 мс. Последние пришли уже после восстановления;
их нельзя автоматически приписать этому stall или объявить доказанным overflow MConfig.
Плеер меняет state/metadata вскоре после системного DOWN, но звукового ACK нет.
Все обычные записи DiagnosticJournal вынесены на bounded writer, отделённый от producer lock;
новый journal разделяет исходные pid/tid, известные стадии Android 9 и возраст KeyEvent.
deviceId=-1 является допустимым виртуальным устройством и отличается от отсутствующего поля.
Схема MConfig/Android и системные разрешения не менялись.

Для штатной traffic-light капсулы установлен вырожденный боковой leg: радиус равен половине
высоты, поэтому после ограничения базовых точек остаётся 0–1 пиксель. Новый выбор исключает
такие стороны, не перерисовывая знак. Обновление shadow bounds в 2.9.4 этого не исправляло.
Полная аппаратная приёмка, включая другие исчезновения хвостика, остаётся открытой.
Источник, хэши и разграничение доказательств: [вечерний разбор](docs/FIELD_AUDIT_2026_09_14_EVENING_RU.md).

## Результат после пары 2.8.9 — карты не восстановлены

13.09.2026 пользователь отклонил 2.8.9: на HUD и приборке карта не работает;
раньше она отображалась с белым засветом. Исправление запрещённого readback
внутри TextureView.draw не оказалось достаточным. Точный последний работающий
номер пользователь в этом сообщении не назвал; до 2.8.8 есть сообщения о
видимой карте с засветом, начиная с 2.8.8 — о полном отсутствии карт.
MapLoadedListener означает однократную полную загрузку всех видимых тайлов,
а не обязательное событие первого доступного кадра; обязательное ожидание
этого события способно оставлять готовые частичные карты скрытыми бессрочно.
Источник: https://yandex.com/maps-api/docs/mapkit/com/yandex/mapkit/map/MapLoadedListener.html.
Отсутствие callback в данном новом запуске не доказано журналом; технический
дефект зависимости и отрицательная приёмка пользователя фиксируются отдельно.
GATE-082 провален для 2.8.9. Восстановление 2.9.0 (NAV-019/GATE-087) снимает
скрытие по update текущей surface без ACK/readback; resize сохраняет кадр,
MapLoadedListener — необязательная диагностика. Возможен прежний белый кадр,
отсутствие свечения не объявляется исправленным. Пара 2.9.0 собрана из 15dd691 и проверена по APK/DEX; release CI
34756328599 успешен (1881+114 тестов). Физический результат новой версии пока не получен. Все последующие старые выводы о first-frame gate — история.
...
Версия 1.8 • дополнение 11 сентября 2026 года • автомобиль и прошивка из переданных пользователем материалов. Исходные разделы и публичный ZIP 1.0 сохраняют результаты 4 сентября, редакция 1.2 — расширенный разбор 5 сентября.
Аппаратные замечания после Natro 2.8.4/2.8.5 и соответствующие изменения подписанных 2.8.6/2.8.7 актуализированы ниже; это не означает аппаратную приёмку до отдельного измерения на KX11.
...
## Дополнение 13.09.2026: полный map-журнал и чтение TextureView

Map-архив 131720 подтвердил точные хеши выданных APK и успешный attach обеих
карт после mapfix. Все 19 диагностических чтений успешны; транспорт сборщика
1.0.0 проверен на KX11. Прежняя ошибка listener встречается 60 раз до установки,
после установки в журнале её нет. Окна и GlLayer видимы по Android, но наличие
карты этим не доказано. ACK и результаты пикселей старая версия не записывала.

В фактическом Natro DEX оба onSurfaceTextureUpdated вызывают getBitmap; в Android 9
callback исполняется из TextureView.draw, а readback внутри рисования запрещён API.
Проверка вынесена из рисования, старые задачи отменяются при смене Surface;
ACK текущей сессии/generation, три квалифицированных кадра и пиксельные пороги
сохранены. Добавлены ограниченные состояния producer/ACK/readback в журнал.
17 локальных проверок прошли; устранение всей аппаратной причины ещё не доказано.
GATE-082 открыт, нового APK нет. [Источники, проверка и границы](docs/map-startup-2026-09-13/MAP_CHECK_131720_RU.md).

## Дополнение 13.09.2026: второй захват поездок и сохраняющийся отказ карт

Архив 121730 от сборщика 1.0.1 физически подтвердил раздельный транспорт: все
40 PCAP-сообщений целые, IPv4/UDP checksums и FILES.json совпали, ремонта нет.
Фото IMG_8379/8380: Trip 2 — 4,9 км / 00:39 / 19,9 / 8; Trip 1 — 7439,7 км /
19:01 / 10,6 / 24. PA33749=104684…104731 секунд, PA33751=3278504. Прямое
соответствие строкам отсутствует; между снимками Trip 1 рост PA на 364 с близок
росту штатного времени на 6 мин. Общий счётчик с базой сброса остаётся допустимой
гипотезой; потребитель, единицы и база не установлены. Фото не являются основанием
для калибровки смещения. GATE-086 открыт. [Проверка и границы](docs/geely-kx11/research-2026-09-13-trip2/SCREEN_RESULTS_121730_RU.md).

Пользователь также сообщил, что после установки Navigator mapfix обе карты
по-прежнему не запускаются. Статически исправленная сигнатура listener не закрыла
аппаратный GATE-082. В trip-архиве нет Android-диагностики; подготовлен отдельный
Mac-сборщик журналов и состояния приложений. Следующий runtime-этап отказа пока
не установлен. [Продолжение](docs/map-startup-2026-09-13/POST_MAPFIX_FAILURE_RU.md).

## Дополнение 13.09.2026: получены оба штатных экрана и 40 текущих сообщений

[Новый архив и IMG_8377/8378](docs/geely-kx11/research-2026-09-13-trip2/SCREEN_RESULTS_RU.md)
дают независимое отрицательное сопоставление PA33749/33751 со штатной строкой.
Все 40 пакетов восстановлены: collector 1.0.0 через exec-out вставил текст stderr
в PCAP; полезные packet bytes не повреждены и сохранены буквально. Подтверждены
размеры, маршрут, IPv4/UDP checksums и совпадение записанного SHA native VHAL.
У «Поездки 2» на фото 4,9 км / 00:32 / 19,9 л/100 км / 9 км/ч; у «Поездки 1» —
7439,7 км / 18:55 / 10,6 л/100 км / 24 км/ч. PA-время равно 104311…104367 секунд
(28:58:31…28:59:27), PA-пробег постоянен, raw=3278504. Исправление секундной единицы
не даёт штатную поездку, а /10 и /1000 для расстояния не совпадают с эталоном.
Фотографии без EXIF не дают точного времени отдельного пакета; reset не наблюдался.

Источник штатных четырёх полей не восстановлен. Этот фильтр захватил только
006e/00c8 и не позволяет исследовать другие сервисы по новому архиву. Следующий
пробел — фактический consumer/producer DIM в уже переданных образах, а не повтор
той же PA-записи. Непроверенная PA-пара не подставляется в HUD/DIM. Исправленный
транспорт collector 1.0.1 и строгий recovery прошли 19 офлайн-тестов; запуск новой
версии на KX11 не заявляется. NATRO-027/GATE-086 остаются открытыми.

Отдельно по команде «Собирай» выпущен [Navigator 2.8.8-mapfix](docs/releases/2.8.8-mapfix/README_RU.md)
с исправлением регистрации MapLoadedListener и прежним сертификатом. Он совместим
с установленным Natro 2.8.8; физическая проверка обеих карт ещё требуется.

## Дополнение 13.09.2026: аппаратный отказ карт 2.8.8

Пользователь сообщил полное отсутствие обеих карт после обновления до 2.8.8.
NAV-018 отмечен ошибкой, GATE-082 не пройден. В текущем коде отсутствие capability
`CAP_MAP_CONTENT_READY` блокирует передачу обеих Surface; после передачи нужны ACK
актуальной session/generation и три прошедших pixel-проверку кадра. Любой из этих
этапов может оставить карту скрытой. Позднее пользователь подтвердил установку обоих APK из пары 2.8.8.
Проверка фактически выданного Navigator установила несовпадение сигнатуры:
Map и native MapBinding принимают `setMapLoadedListener(WeakReference)`, а наш
startRenderer искал `setMapLoadedListener(MapLoadedListener)`. Такой метод отсутствует,
и NoSuchMethodException обрывает запуск до `addSurface` на обоих экранах.
Регистрация и снятие исправлены в исходниках; сильное владение listener, ACK и
проверка кадров сохранены. [Хеши, фактический API и воспроизведение](docs/map-startup-2026-09-13/README_RU.md)
отделяют установленный дефект кода от ещё не выполненной повторной проверки KX11.
Это аппаратный отрицательный результат, а не основание объявить новую формулу
готовности рабочей без проверки. Исследование источника поездки остаётся отдельным.

## Дополнение 13.09.2026: «Поездка 2» сбрасывается автоматически

Пользователь уточнил, что ручного сброса «Поездки 2» нет: она сбрасывается
автоматически. Это учтено в NATRO-027/GATE-086. В исходном XCSettings2
`ComputerClearFragment` разделяет группу `mLlTrip1` с ручными сбросами пробега,
средней скорости, времени и расхода и группу `mRlTrip2` с вариантами условия
автоматического сброса. Русский заголовок второй группы — «Пробег за текущую
поездку». Статические маршруты и SHA сохранены в
[stock-reset-route.json](docs/geely-kx11/research-2026-09-13-trip2/stock-reset-route.json).
Они не устанавливают активное условие или физический момент автоматического сброса.

Суффикс `2` в PA-getter пока нельзя отождествлять с названием строки приборки:
сопоставление PA с ручной либо автоматической поездкой остаётся гипотезой.
Следующий конкретный шаг — два коротких пассивных снимка вместе с фото обеих
штатных строк, без изменения счётчиков. Подготовлен
[сборщик для Mac](tools/geely-trip2/SCREEN_CHECK_RU.md); неизвестный SHA native
не разрешает автоматическую интерпретацию. Аппаратное совпадение, единицы пробега
и совместное поведение при штатном автоматическом сбросе ещё не подтверждены.

## Дополнение 13.09.2026: исходный PA-счётчик найден в IPCP, прежние единицы времени ошибочны

[Повторный разбор прошивки и всех 25 PCAP с trip-сообщениями](docs/geely-kx11/research-2026-09-13-trip2/README_RU.md)
проследил PA33749/33751 до native `0x217083d5/0x217083d7`, IPCP006e/00c8,
712 байт, смещения 0x30/0x50. 1928 сообщений подтверждают секундный масштаб
`PA_TS_EDT_time2`; Natro 2.8.5 ошибочно считал raw минутами. Старые формулировки
ниже о полной непригодности PA — исторический вывод, уточнённый этим разбором:
фото отвергло реализацию 2.8.5, а не все возможные правильные декодеры PA.
Во всех 25 захватах пробег постоянен; `/1000` и соответствие штатному Trip 2/reset
ещё не доказаны. Ненулевой и нулевой пробег в разных захватах не размечают reset.

Добавлено только явное фоновое диагностическое чтение PA с raw/quality/format и
секундным пересчётом, отдельно от HUD/DIM. Старое минутное толкование и деление
PA-пробега на 10 не возвращаются. NATRO-027 уточняет запрет старой PA-диагностики;
NATRO-026/GATE-086 открыты. Полной работающей поездки и нового подписанного APK
этот исходниковый результат не означает.

## Дополнение 11.09.2026: белый bootstrap-буфер карты и стек reader 2.8.5

Видео пользователя `IMG_8363.mov`, SHA-256
`ac43f94a9bc1511076e4c2589a68ff9980e1e01fd49a294c512ad3effbbb2dd4`, длительностью
15,733 с и частотой 30 fps, аппаратно подтверждает белое свечение внутри прямоугольника карты
приборки перед первым тёмным кадром MapKit. Остальная приборная композиция остаётся чёрной,
поэтому это не общая экспозиция камеры и не включение всего display 2. Старый first-frame gate
снимал alpha по первому `onSurfaceTextureUpdated`; Android 9 KX11 вызывает его и для
однородного непрозрачного белого bootstrap-буфера. Подписанная Natro 2.8.7 принимает
первый кадр HUD и приборки только после малой pixel-выборки, отвергающей такой буфер. Resize
существующей surface по-прежнему сохраняет последний готовый кадр. Это изменение исходников;
аппаратное подтверждение обеих поверхностей остаётся за `GATE-080`.

Фото пользователя `IMG_8359.jpeg`, SHA-256
`64ebfb8746e98ebf8f3836a2354a680ac70e0c7433fcc093c21e4286eaf57967`, показывает crash
Natro 2.8.5 в потоке `hud-navigation-reader`: `StackOverflowError`, после которого повторяется
`Collections$UnmodifiableCollection.isEmpty`. В renderer-модели неизменившиеся parsed-списки
переиспользовались, но каждый новый 10-Гц `HudNavigationState` снова оборачивал предыдущий
`unmodifiableList`; делегирование `isEmpty()` росло на один уровень на snapshot. Исправление
использует одну устойчивую immutable-копию и возвращает её по identity при неизменном JSON.
Автотест 20 000 snapshot прошёл в CI; длительная проверка процесса на KX11 закреплена в
`GATE-081`.

Обычные CI `34625307587/34625307479` и защищённый CI `34626014981` / run 272
завершены успешно. Исполняемый commit — `a5cbf6c2d6491cd6158d7be4a95cb555a481815d`,
дерево — `7388f4f8676386b909bbc9b455622940e9db977a`; оба APK подписаны прежним
сертификатом обновления. Это закрывает сборочные, статические и криптографические проверки,
но не аппаратные `GATE-080/081`.

## Дополнение 11.09.2026: подписанная пара 2.8.6 после фото KX11

Фото пользователя сопоставило штатную строку «Поездка 2» — `17,4 км`, `00:43`,
`11,5`, `24 км/ч` — с неверными lifetime-like значениями Natro 2.8.5. Тем самым
`PA_TS_OdometerTripMeter2` и `PA_TS_EDT_time2` отвергнуты как рабочий источник этой
строки на данной KX11. В подписанной 2.8.6 они удалены из runtime-пути и диагностики;
новый путь описан ниже в разделе второй коррекции источника.

По тем же пользовательским материалам исправлены ещё три расхождения: авторазмер синей
карточки теперь инвалидируется по значению размерообразующего содержимого и может снова
расти; линия альтернативы публикуется до и независимо от построения её выноски; свежая
медиакнопка не ждёт Binder проигрывателя, который уже висит дольше 750 мс. Дополнительно
убраны два ранее доказанных main-thread hot path: синхронный preference fsync в boot receiver
и загрузка package icon/uninstallability из `AppsAdapter.getView`. Обычные CI
`34612692293`/`34612692371` и защищённый CI `34613313248` прошли; пара подписана
прежним сертификатом. Аппаратные результаты проверяются `GATE-076…079`.

## Дополнение 11.09.2026: подписанная пара 2.8.5

Исходные изменения после аппаратной проверки 2.8.4 включены в подписанные Natro
**2.8.5 / 208021318** и Navigator **30.3.0 / 739564630**. Исполняемый исходник —
`96155d45e44a8eaa4cdd54924f07f2d4c34b1ed6`, дерево —
`f15006bb1fb5ed64d2aca3c043e428acdc3f4d3a`; полный состав и выпускные доказательства
зафиксированы в [протоколе 2.8.5](docs/releases/2.8.5/README_RU.md).

CI прошёл 1 875 тестов приложения и 56 проверок инструментов. Защищённый запуск подписи
сверил точный исходный Navigator и общий сертификат, v2/v3, 16-КиБ zipalign, CRC и границу
17 222 защищённых записей. Эти результаты доказывают состав артефактов, но не закрывают
`GATE-069…075`: первый кадр карты, визуальные labels/альтернативы/карточка, единицы Trip 2
и числовой CPU-эффект остаются проверкой на физическом KX11.

## Дополнение 10.09.2026: статический аудит CPU перед 2.8.5

Анализ выполнен именно по CPU hot paths Natro и добавленного Navigator DEX, а не по общему
размеру приложения или памяти. В повторяющемся пути кадров обнаружены измерение авторазмера
синей карточки через новые `StaticLayout`/списки, повторная подготовка текста и восьми footprints
выносок альтернатив, reflected fingerprint и defensive copy неизменного retained-списка
альтернатив, копирование неизменного списка дорожных событий, создание массивов в каждом
Liang–Barsky clipping и перерисовка HUD на одинаковом telemetry callback. PA Trip 2 также мог
держать callback при отсутствии видимого потребителя.

В подписанном выпуске 2.8.5 эти операции привязаны к изменению содержимого: авторазмер и footprints
кэшируются с инвалидированием по конфигурации/navigation/palette; однострочный текст рисуется
прямым Canvas-путём с тем же многоточием; retained alternatives и события получают ранний
выход; коэффициенты clipping считаются локальными примитивами; одинаковое telemetry-значение
только продлевает свежесть; PA callback регистрируется по реальному demand и снимается при его
исчезновении. Частоты MapKit/Guidance, предел FPS, разрешение Surface, сглаживание и состав
слоёв не снижались.

Это статически обоснованное устранение лишней работы, но не числовой результат автомобиля.
Процент снижения CPU, allocation/GC и frame pacing сравниваются с 2.8.4 только одинаковыми
прогонами Perfetto/simpleperf на KX11 по `GATE-075`; до этого нельзя обещать конкретный процент
или считать отсутствие визуальной регрессии подтверждённым.
...
### Историческая коррекция источника после проверки 2.8.4 (отвергнута 2.8.6)

Пользователь проверил подписанную 2.8.4 на KX11: время поездки осталось нулевым, а пробег
совпал с общим одометром автомобиля; фактический общий пробег — 37 278 км. Это аппаратное
отрицательное доказательство для выбранного в NATRO-021 пути `ITripData.IDrivingInfo`: даже
корректная callback/freshness-обвязка не превращает его значения в штатную «Поездку 2».
Прежний раздел выше сохраняется как история исследования, но больше не является источником
этих двух элементов Natro.

В том же комплектном ECARX JAR статически и компиляцией typed-кода подтверждены отдельные
PA-поля `PA_TS_OdometerTripMeter2` и `PA_TS_EDT_time2`, их getters и callbacks, manager ID
33751 (`ManagerId_patsodometertripmeter2`) и 33749 (`ManagerId_patsedttime2`). Новая
read-only обвязка получает оба значения через один `ECarXCarPhevManager`, принимает пару
только при `AvailabilitySts.Active`, сохраняет raw/format для диагностики и обновляет одну
monotonic timestamp. Активный callback любого из двух полей перечитывает всю пару, поэтому
пробег, время и производная скорость принадлежат одному наблюдению.

Связь названий полей со штатной «Поездкой 2» существенно сильнее прежнего общего
`CurTripDis/CurTripTime`, но не устанавливает единицы. Деление distance на 10, трактовка time
как минут, частота обновления, reset и ignition semantics остаются гипотезами до синхронного
снимка raw/format + штатного экрана + Natro по `GATE-073`. Средний расход не переводится на
эти PA-поля и продолжает поступать из отдельного ignition-сигнала.

После фото 2.8.5 этот вывод также опровергнут аппаратно: названия PA-полей оказались
недостаточным основанием, а значения Natro не совпали со штатной строкой. Раздел сохранён как
история и отрицательное доказательство; PA-пара запрещена требованиями `NATRO-024`.

### Вторая коррекция источника для 2.8.6: приборные CarSignal

**Уточнение 12.09.2026 — источник не подтверждён.** Последующее фото
`607F8570-98B8-4ED8-B817-BA58D88262B3.jpeg` показывает прочерки всех трёх полей.
В `research-2026-09-05/interfaces/entries/interfaces-0004.json` запись
`car_signals:483` / `DstTrvld2` содержит 10 экспортов, 0 изменений и
`raw_minus_one_only=true`. Это историческое наблюдение, не свежий замер установленной 2.8.7,
но оно опровергает уверенность, что наличие SDK getter доказывает рабочий источник.
Продолжение аудита 12.09 подтверждает то же для обоих остальных кандидатов:
`interfaces-0014.json`, `car_signals:1763` / unit 30956 и `car_signals:1764` /
average 30957 — каждый имеет 10 экспортов, 10 событий, 0 изменений,
`raw_minus_one_only=true`. Таким образом, старый корпус не содержит работающей
тройки, которую можно просто вернуть вместо дефектного чтения. Это не доказывает
невозможность иного штатного пути на данной прошивке.
Масштаб `/10` также не подтверждён синхронной raw-записью со штатным экраном.
Нельзя объявлять расчёт `distance / rounded indicated average` точным штатным временем:
информация, утраченная при округлении средней скорости, не восстанавливается.

В подписанной 2.8.8 убран отказ от всей тройки при недоступности одного поля, raw=-1
остаётся в диагностике, каждое поле проверяется отдельно, а Binder death инвалидирует
старое значение. Точная штатная поездка остаётся открытой: `NATRO-026/GATE-086` и
`CURRENT_TRIP_DATA`. PA/ITripData/общий одометр не возвращены как замена. Описание ниже
сохраняет историю реализации 2.8.6, а не новое подтверждение её работы на KX11.

**Проверка ID 13.09.2026:** все десять копий каждой из трёх текущих констант
CarSignalManager в опубликованном каталоге 621 DEX содержат соответственно
30878, 30957 и 30956. Конфликт числовых ID этой тройки не найден. Это не определяет
загруженный runtime-класс и не доказывает поддержку сигналов, их масштаб или время.
[Отчёт с SHA-256 входов и DEX-источниками](docs/releases/2.8.8/trip-source-id-audit.json).

В комплектном `ecarx-adaptapi.jar` и публичном каталоге прошивки подтверждены typed read-only
сигналы одного `CarSignalManager`:

| Значение | SDK | Signal ID | Native VHAL | Тип / режим |
|---|---|---:|---:|---|
| Пробег Trip 2 | `getDstTrvld2()` | 30878 | `0x2140789E` | `INT32`, global, on-change |
| Единица средней скорости | `getVehSpdAvgIndcdVeSpdIndcdUnit()` | 30956 | `0x214078EC` | `INT32`, global, on-change |
| Штатная средняя скорость | `getVehSpdAvgIndcdVehSpdIndcd()` | 30957 | `0x214078ED` | `INT32`, global, on-change |

У unit enum `0=km/h`, `1=mph`, неизвестное значение не публикуется. Подписанная 2.8.6
подписывается на все три ID только при видимом потребителе, на callback перечитывает всю
тройку из того же manager generation, хранит одну monotonic timestamp и атомарно скрывает
зависимые значения после 10 минут без наблюдения. Пробег интерпретируется как десятые километра;
средняя скорость берётся напрямую, а не вычисляется из ошибочного PA-времени.

Отдельного typed elapsed-time сигнала Trip 2 в этом SDK не найдено. Поэтому время Natro
восстанавливается из той же пары `distance / stock average speed` и отображает завершённые
минуты: `17,4 / 24 × 60 = 43,5`, на экране — `00:43`. Это совпадает с переданным фото, но
округление штатной средней скорости может давать пограничную разницу в минуту; raw-значения,
reset/reconnect/ignition и обе единицы всё ещё проверяются на KX11 по `GATE-076`. Старый общий
одометр, PA-пара и pre-callback `ITripData` zero object в новый путь не входят. Средний расход
остаётся отдельным ignition-сигналом.
...
После 2.7.8 пользователь сообщил, что курсор независимых карт застывает на полилинии,
хотя основной Navigator продолжает движение. В нашем publisher найдена безусловная
подмена Guidance location сохранённой RoutePosition; существование маршрута само по себе
не доказывает, что автомобиль всё ещё движется по нему. В оригинальном Navigator 30.3.0
SHA-256 `663018fb66074e001eed7caba8e33bee1bcf78f6798bc84949d253dcb348f27f`
проверены классы Guidance/RouteStatus/RoutePosition (`classes12.dex`), включая отдельные
NOT_ON_ROUTE и ROUTE_LOST. Выбор позиции в моде 2.7.9 учитывает эти состояния и очищает
старый кэш при уходе; привязка к маршруту сохраняется только при положительном статусе.
Это контракт конкретного предоставленного Navigator, не новый CAN/OEM-сигнал KX11.

Фотографии IMG_8278/8279 также подтверждают отсутствие настоящей строки «Еринское ш.»
в нашей карточке. Проверено штатное получение nextRoadName в ContextManeuverView
(`classes8.dex`); ошибочное подавление строки было в direct-renderer Natro.
Подробности и границы проверки: [разбор 2.7.9](docs/NATRO_2_7_9_NAVIGATION_POSITION_RU.md).
Автомобильные GATE-041/050 открыты; тесты политики не заменяют поездку и сравнение экранов.
Последующее MAP-051 уточняет только обычные route traffic light pins: их native sublayer
ставится сразу над маршрутом/подписями, отдельно от живых светофоров с сигналом/секундами.
Проверены разные feature types, позднее создание и сохранение ручных настроек; GATE-019 открыт.

## Дополнение 05.09.2026: HUD, топливо и пауза медиа

Переданный mHUD 6.1, SHA-256
`8ba8da9aea2b6072cd5ec3cb42fc145a82547dc48f5e7b1d9e705cfee9a25d2e`, проверен по реальным
пунктам меню, MediaInfoReceiver и пути HUD Presentation. Медиа скрывается по is_playing=false;
отдельное распознавание телефонного вызова в этом пути не найдено. Нельзя выдавать паузу
воспроизведения за доказательство полного состояния HFP. [Таблица и границы](docs/MHUD_6_1_WIDGET_AUDIT_2026_09_05_RU.md).
Новый Natro использует существующий источник MediaSession/media broadcast; Bluetooth/ANCS
по-прежнему принадлежит PhoneConnectorController, новых команд или ECU-функций нет.

Топливо для остатка/долива продолжает читать ISensor.fuel_level в мл с переводом в литры,
как существующая VehicleDerivedMetrics/InstrumentValuePolicy. Пользовательский режим бака
по умолчанию — 64 л; ручной объём не подменяется ICarInfo. Это настройка расчёта, не измеренная
ёмкость конкретной машины. P использует прежний подтверждённый код ISensor.gear=2097712;
неизвестная передача не считается P. GATE-051/054 требуют проверки на автомобиле.
...
## Повторный запуск после обновления Natro 2.7.7 — 05.09.2026

Новые наблюдения относятся к пользовательскому KX11 и не меняют таблицу display/zone/ID.
В `status-widget-debug(20260905-113110).txt` (SHA-256
`f1dbe32dd42f62ac3e70211da75453c7bc0865ef6ea525b342a944571fd0c6c0`)
первый HUD attach 2.7.7 происходит в 14:28:00.149. Cluster lease 1920×720 появляется
только в 14:29:22.748 после SettingsHubActivity, attach — 14:29:23.017.
Это подтверждает раздельную готовность поверхностей: работа HUD не доказывает запуск DIM.
В host-пути WidgetService отсутствовал `InstrumentDisplayLauncher.reconcileAutomatic`;
его добавление использует существующий прямой launcher и рабочий поток DIM, без новых OEM
значений или остановки Natro. Приёмка на автомобиле остаётся открытой (CLUSTER-022/GATE-043).

Тот же журнал сообщает Navigator hello `capabilities=3fe7`, без бита `0x4000`, необходимого
для штатного изображения манёвра. Фото 14:33 показывает исходную синюю карточку «налево 100 м»
в головном Navigator и её отсутствие на HUD. Это несовместимость установленной пары, а не
доказательство потери HUD Surface. Нужен мод Navigator с artwork IPC; повторять handoff
или переключать DIM для исправления этой несовместимости не следует (CARD-013/REL-017).

Подробности ANCS, возврата HOME, исходников и проверки APK:
[разбор 05.09.2026](docs/STARTUP_RECOVERY_2026_09_05.md). Сырые пользовательские журналы,
координаты, идентификаторы телефона и приватные ключи в репозитории не публикуются.
...
В подписанной 2.8.6 resolver дополнительно сверяет список активных сессий каждые 2 минуты,
чтобы восстановиться при потерянном vendor callback. Пока command looper находится внутри
Binder выбранного проигрывателя, фиксируются sequence и время начала. Если через 750 мс приходит
новая физическая команда, она не добавляется позади зависшей: route сбрасывается/обновляется,
а `dispatch` возвращает `false`, позволяя Android выполнить свежее событие штатным media path.
Уже зависший Binder-вызов Java прервать безопасно нельзя; ограничение предотвращает рост очереди
и позднее воспроизведение новой пачки, но не доказывает исправность внешнего проигрывателя.
...
| Текущая поездка | ITripData 2.8.4 и PA-пара 2.8.5 отвергнуты аппаратно. Для 2.8.6 статически подтверждены `DstTrvld2` 30878 и stock average/unit 30957/30956; фото 17,4 км / 00:43 / 24 км/ч согласуется с `/10` и завершёнными минутами производного времени | Синхронные raw/штатный экран/Natro, reset, reconnect, ignition и km/h/mph; пограничное округление времени, GATE-076 |
| CPU Natro/HUD/DIM | Статически удалены повторные layout/list/reflection/allocation hot paths и callbacks без demand; 2.8.6 дополнительно убирает snapshot identity churn, boot fsync и package-icon work из `getView`; разрешение/FPS/AA/частоты не снижались | Три парных 10-минутных прогона 2.8.4/2.8.5/2.8.6 с Perfetto/simpleperf, allocation/GC и frame pacing; процент снижения не объявлять до GATE-075 |
...
| MEDIA_LATENCY | Полная задержка кнопок руля | Нажатие → callback → отдельная command lane → player → audio на общей временной шкале; отдельно сценарий stale route и Binder дольше 750 мс после 10/20 минут | На KX11 снять 30-минутный журнал с eventTime/downTime/callback/queue/dispatch/fallback/callback метками и синхронным звуком; отделить MConfig/Android до callback от Binder плеера, GATE-079 |
...
| CURRENT_TRIP_DATA | Единицы и reset текущей поездки | Raw `DstTrvld2` 30878, `VehSpdAvgIndcdVehSpdIndcd` 30957, unit 30956 и штатная строка 0,1 км / hh:mm / km/h на одной временной шкале; PA/ITripData отвергнуты | `GATE-076`: записать raw/экран/Natro при 0, движении, сбросе, reconnect и ignition; проверить `/10`, unit и минутное округление производного времени |
...
## Дополнение 07.09.2026: восстановление всей Android-страницы приборки

Пользователь уточнил, что при загрузке не появилась вся приборка. В журнале
`status-widget-debug(20260907-152526).txt`, SHA-256
`dc0319e68768bf47638c18b2610499098cab7ca0ff55a8c73b04a149e4c7510b`, есть ранний
cluster lease, но он не доказывает видимость страницы на физической приборке.
В исходнике 2.8.1 проверка isActive использует лишь ссылку на незавершённую Activity;
эта ветвь пропускает повторную DIM-последовательность и вывод скрытого окна.
Идентификаторы и протокол из таблицы выше сохраняются. Новый код отдельно проверяет
readback getNaviMode, выбранный display, STARTED/attached/visible и первый draw,
ограничивает повторный запуск и сбрасывает только задачу приборки при потере дисплея
или QuickBoot. RESUMED/focus не являются обязательным признаком видимости на Android 9.
Ручной внешний self-force-stop удалён. Это завершённые изменения исходника, а не
подтверждение физической причины конкретного эпизода или приёмка на KX11.
Подробности: [запуск приборки](docs/INSTRUMENT_PANEL_STARTUP_FIX_2026_09_07_RU.md).


## Дополнение 13.09.2026 после 2.9.1: порядок подключения карты

Пользователь подтвердил, что карта в 2.9.0 заработала, но белый старт сохранился.
Это уточняет прежнее «результат 2.9.0 отсутствует»: восстановление показа принято,
устранение засвета не принято. В исходнике 2.9.1 `HudMapRenderer.startRenderer`
сначала подключал Surface, затем вызывал `applyProfile`, который ещё и применял
ночной/прозрачный фон после необязательных FPS/слоёв внутри общей try-ветки.
Кандидат применяет фон независимо и полностью настраивает карту до `addSurface`.
Первый update текущей TextureView по NAV-019 остаётся единственным условием показа;
ACK, ожидания тайлов и bitmap readback не возвращаются. Это статически проверяемый
порядок вызовов, не доказательство цвета первого native/GPU-буфера на KX11.

Продолжение поиска «Поездки 2»: проверены вложенные DIM-журналы старого архива
135239 (260 элементов / 82 уникальных SHA) и все 143 обычных файла LV initramfs.
Совпадений по выбранным именованным Trip-полям нет; числовые кадры и неизвестные
алиасы этим не исключены. `DIM_Slog` — название выгрузки, не доказательство наличия
в ней процесса, который рисует четыре штатных значения. Android init.visteon-dim.sh
содержит UART-команды и загрузку dim_dtc; это не подтверждённый getter поездки.
Эти команды не выполнялись и не добавлены в Natro. Граница поиска и SHA:
[остаточный источник Trip2](docs/geely-kx11/research-2026-09-13-trip2/DIM_SOURCE_GAP_RU.md).

Перед выдачей 2.9.2 обнаружено, что `GeelyCarIntegration` всё ещё публиковал
`distance*60/roundedAverage` в штатный ID времени при валидной CarSignal-паре.
NATRO-026 называл это оценкой, но поле карточки не отличало её от штатного таймера.
В итоговом кандидате оба emit-пути инвалидируют время; оценка остаётся только
в явной диагностике. Нового источника таймера этим не получено. Это уточняет
прежнее слишком широкое утверждение об отсутствии расчётной подстановки.

Итоговая пара 2.9.2 собрана из `950bd32f157eb3a631dad9acf3db3cbe5c653770`,
CI `34767329139` успешен. Проверены подпись и фактические DEX обоих изменений;
[протокол](docs/releases/2.9.2/README_RU.md). Это программная проверка,
не подтверждение устранения засвета или восстановления четырёх штатных полей.

### 13.09.2026 — KX11: новые свидетельства 2.9.2, без изменения vehicle-интерфейсов

Поступили status-widget-debug(1).txt (SHA-256 `50ee300fb72a9bf4737b30dda675ff97fe7e202c479f0b1a87090289e413e6ce`) и Natro-ANCS-2026-09-13_19-54-31.log (SHA-256 `73000579863e813d5db8fc40c16975b0d44033aa0baf9c9066873523ec36738f`), фото IMG_8391/8392/8393(1). Версия Natro 2.9.2 / 208021325 подтверждена session-маркером, установленный Navigator не хеширован. Подробная хронология и требования — в новом разделе PROJECT_REQUIREMENTS_RU.md «повторный отказ на KX11 после 2.9.2».

Белый старт пользователем повторно отклонён: локальный update приборки записан до bind/отправки surface Навигатору, поэтому TextureView update нельзя считать доказательством нового кадра текущего MapWindow. Это не измерение пикселей и не основание возвращать обязательную полную загрузку тайлов. Watchdog main фиксирует QueuedWork-ожидание 5367 мс и чтение полного ActionRecorder-файла в DiagnosticsActivity.onResume 3342 мс; записей физических медиакнопок нет, связь с задержкой конкретного нажатия не доказана.

ANCS после package replacement действительно стартует и выдерживает 8-секундную паузу. Сохранённая exact LE identity находится, HFP подключён, но GATT выдаёт 133 и не достигает READY до перезагрузки. Такие ошибки присутствуют и до обновления. После BOOT: READY 19:46:42.507, временный link status=22, самостоятельное READY 19:47:01.176, затем реальная доставка уведомлений. Нет доказательств потери сопряжения или необходимости его пересоздания; внутреннюю причину Bluetooth-стека app-level журнал не раскрывает. Отдельно найден путь STOPPING с уже пустым owner, который без немедленной maybeCompleteTeardown может ждать ложный 4-секундный timeout; устранение этого пути не гарантирует устранения 133.

Двойной камерный слой подтверждён route inventory POLICE=51 и реплеем фактического RouteRoadEventMapLayer на 80 смешанных записях: удаление control-тегов оставляет POLICE-пины; после исчезновения списка камер впереди возвращаются обычные control-пины полного маршрута. Синий знак выбирается нашим detailDrawableName по любому LANE_CONTROL/ROAD_MARKING_CONTROL, без сверки выбранного штатного варианта. Исправления камеры/старта/ANCS/медиа и новые APK в этом диагностическом аудите не выполнялись. Новых vehicle ID, зон, управляющих команд или источника Trip2 нет.

### 13.09.2026 — сопоставление iPhone и KX11 при отказе ANCS

Журнал iPhone Helper 70.0/build 70 (iOS 26.6.1): Natro-ANCS-2026-09-13T17-17-21Z.log, SHA-256 `23e46ff210b5557d7cae84e6269a75c483d1f2b133ec5c7598b13af7d4e085b2`, 262 967 байт. Суффикс Z относится к моменту экспорта; строки записаны локальным DateFormatter. Соответствующие Android callback-строки примерно на 2,6 с позже iPhone, без общего синхронного clock source.

После CONTROL loss в 19:03:34 Helper завершил старого владельца и подтвердил публикацию/advertising примерно за 1,24 с. Во всём журнале 30 drain fired, максимальная lateness 109 мс; старый многоминутный провал самого drain здесь не подтверждается. Затем почти 43 минуты нет записей до нового session ID/запуска Helper в 19:46:35.859. Успешная аутентификация/ANCS наступает после BOOT Android и нового запуска Helper, поэтому успех нельзя причинно приписывать только перезагрузке KX11. Причина новой сессии Helper неизвестна.

В Helper выявлена конкретная ошибка смысловой готовности: UI связывает C5 transportReady с локальным phase=active, хотя peerReady ещё false и реальный sendCarRemoteFrame блокирует передачу. Строки C5 «маршрут аутентифицирован» и «HELLO отправлен» до encrypted CONTROL не являются доказательствами ни обмена, ни готовности ANCS. Отдельно различать local published, exact peer authenticated и Android ANCS subscriptions ready; сопряжение/криптографические ограничения сохранять. Последний iPhone CONTROL loss 19:54:40 произошёл за пределами переданного Android-журнала, причину этого эпизода не утверждать.

Подробности добавлены в PROJECT_REQUIREMENTS_RU.md. Это новый результат диагностики, без изменений приложения, новых vehicle-команд/ID, сборки или установки.

Уточнение со слов пользователя: около 19:46 Helper на iPhone вручную не открывался и не перезапускался. Новая сессия Helper подтверждена журналом, но не была отдельным ручным действием: она могла возникнуть автоматически вслед за перезагрузкой KX11 и восстановлением BLE. Это уточняет прежнюю причинную оговорку: наличие новых сессий на обеих сторонах само по себе не означает два независимых вмешательства. Точный механизм запуска iOS не установлен; считать ручной запуск Helper необходимым условием восстановления нельзя. Выводы о ложной готовности C5 до аутентифицированного CONTROL и о неустановленной причине GATT 133 сохраняются.

### 13.09.2026 — уточнение BASE-MEDIAKEY и автозапуска cluster-map

Источник тот же status-widget-debug(1).txt, SHA256 50ee300fb72a9bf4737b30dda675ff97fe7e202c479f0b1a87090289e413e6ce; сопоставлены исходники Natro 2.9.2 без их изменения. Пользователь сообщает: после обновления обоих приложений достаточно открыть настройки Natro, чтобы карта приборки появилась.

1. В 2.9.2 окно display=2/task=15756 готово уже 19:44:50.738, Navigator/HUD подключены 19:44:59, а первая передача cluster generation=2 — только 19:45:36.330 после SettingsHubActivity/ui-bootstrap/reload. Аналогичный порядок в 2.9.1: SettingsHub 19:04:08.978 → cluster lease 19:04:09.482. Подтверждено разделение готовности окна и карты; отсутствие запуска всего Natro не объясняет эпизод.
2. SettingsHub вызывает повторное updateConfig живой панели, включая геометрию/requestLayout/invalidate/publishLeaseIfReady. Таймер холодной surface уже существует, но лишь проверяет готовность; он не повторяет этот layout-путь и не объясняет в журнале, чего ждёт. Нельзя выбирать причиной alpha=0, размеры 1×1, отсутствие callback или задержку GPU без соответствующего наблюдения. Дополнительно источник допускает устаревший leasePublished после замены mapSurface в onSurfaceTextureAvailable; это отдельный статический риск, не установленная последовательность в данном логе.
3. Приёмка окна должна оставаться независимой от готовности карты, но восстановление самой карты требует отдельного контроля действующей Surface/поколения и переподключения производителя. Полные тайлы, старый ACK/pixel gate и ожидание открытия Settings не являются допустимой зависимостью. Исправная карта должна сохраняться; выключенная пользователем панель не должна оживать от запоздалого callback.
4. По музыке подтверждены main stalls 5367 мс (QueuedWork; поток-исполнитель отсутствует в ограниченном дампе) и 3342 мс (полное чтение ActionRecorder под LOCK при открытии DiagnosticsActivity). Во время первого снимка потоки steering-media-route/command свободны в MessageQueue. В файле нет фактических steering-input/dispatch, поэтому источник задержки конкретного нажатия и обход Accessibility штатным hardkey-путём остаются открытыми. Существующий каталог BASE-MEDIAKEY и разбор ECarXCarHardKeyService/XSFInputService подтверждают необходимость различать штатный путь и Natro, но не доказывают маршрут каждого нажатия пользователя.
5. Повторные 11 fixture-replay проходят, но не моделируют полный холодный Android/GPU lifecycle. Следующие согласуемые изменения: фоновое ограниченное чтение хвоста журнала, информативный дамп блокировки и безопасная диагностика мультимедийного входа, самостоятельное восстановление карты с проверкой живой Surface и геометрии. Сборка, установка, новые автомобильные команды/ID, изменение разрешений и удалённая публикация не выполнялись.

Подробный план и временные отметки сохранены в PROJECT_REQUIREMENTS_RU.md; программная и аппаратная готовность по этим замечаниям не объявляются.

### 13.09.2026 — BASE-MEDIAKEY: фактический вход MConfig v45 и граница Accessibility

Повторно прочитаны оригинальный MConfig_0.103_src.zip и DEX двух идентичных пользовательских копий MConfig+ v45. SHA256 архива 7ca75727f64d3a1bdfb28f33965a98285a0f85fc7c0c04fb027703d018da9d8a; SHA256 APK f217534230cada7def4c522c5ca4add694eb9c6936a069ad22a1a61d68baf5b6. Подробные локаторы методов и матрица ветвей сохранены в PROJECT_REQUIREMENTS_RU.md. APK/системные команды не выполнялись.

1. У v45 plus.monjaro.KeysConfigReceiver — экспортированный BroadcastReceiver для IEDIA_BUTTON и MEDIA_BUTTON. При mediaKeysCfg=true и src_mode_default=0 он передаёт исходный KeyEvent в AudioManager.dispatchMediaKeyEvent. При режиме 1 входящий IEDIA пересылается обычным MEDIA_BUTTON; полученный обычный MEDIA при ненулевом режиме игнорируется, что исключает собственную повторную пересылку этой ветвью. При выключенном mediaKeysCfg только IEDIA преобразуется в обычный MEDIA. В этой функции нет ограничения конкретным плеером и задержек распознавания SRC/VA.
2. unbindArrows — отдельный от mediaKeysCfg переключатель. Его v45-реализация меняет MEDIA_BUTTON ↔ IEDIA_BUTTON в кэше XSFInputService и затем перезапускает системный inputservice. Это проверенная возможность кода, не установленное состояние пользовательской магнитолы. Не считать любое включение медиакнопок автоматическим применением этого патча; не применять системную модификацию по аналогии без разрешения.
3. AudioManager направляет ручную кнопку текущему системному медиаполучателю. Поддержка разных плееров означает стандартную обработку Android media keys, не одновременное управление всеми и не гарантию для несовместимого приложения. MConfig не выбирает Яндекс Музыку в проверенной ветви.
4. Natro SteeringMediaKeyRouter универсален по MediaSession, но начинается с Accessibility.onKeyEvent, а не с этих broadcast. Строка MEDIA_BUTTON в манифесте Natro относится к queries; receiver IEDIA/MEDIA для руля в текущем коде отсутствует. Уточнение прежних записей про изоляцию MConfig: обязательный переход MConfig → Accessibility → Natro не доказан и из кода не следует. Потоки steering-media-* не ускоряют автоматически внешнюю пересылку, которая вообще не попала в их вход. Отсутствие steering-input в журнале не локализует задержку физической кнопки.
5. Следующий этап после согласования — наблюдение фактического входа и выбор одного владельца передачи. Не добавлять конкурирующую пересылку поверх MConfig: одна кнопка должна вызывать ровно одну команду. Сохранить явно фиксированный плеер и раздельность ручных кнопок/адресного автозапуска MUSIC-009; исключить неожиданный запуск музыки на iPhone. Аппаратная проверка необходима с несколькими плеерами, их переключением, паузой и продолжительной работой.

Это уточнение источников и требований, не реализация устранения задержки; функциональные исходники и APK не менялись.

### 13.09.2026 — граница ответственности Natro в штатной схеме кнопок

Пользователь исключил подмену существующего маршрута MConfig/Android новым обработчиком Natro: задача — не мешать штатному управлению разными плеерами и локализовать задержку. Ранее предложенный выбор нового владельца обработки не является согласованным планом.

- Текущий Natro уже вмешивается для событий, которые приходят в его Accessibility: запрошена фильтрация клавиш, поддерживаемый DOWN при успешной постановке в очередь SteeringMediaKeyRouter поглощается, а команда самостоятельно направляется выбранной MediaSession; связанные UP/repeat тоже поглощаются. Это не только наблюдение. В пользовательском журнале нет доказательства доставки спорных нажатий в этот callback, поэтому причинная связь с зависанием пока не установлена.
- Следующее согласуемое изменение должно исключить именно этот дополнительный перехват/пересылку физических медиакнопок, не отключая всю Accessibility-службу, экранные кнопки или адресный автозапуск. Не добавлять receiver-посредник, не выбирать штатный плеер за MConfig/Android и не менять XSFInputService.
- Нужна диагностика существующего пути и вклада нагрузки Natro без дублирования команд и нового синхронного препятствия. Наблюдение main stall Natro не равно доказанному заполнению очереди MConfig. Метки недоступного этапа нельзя выдумывать или заменять временем обработки нашей отдельной очереди.

Ограничение отражено в PROJECT_REQUIREMENTS_RU.md; изменения только документационные, работа со сборками и кодом приложения не выполнялась.

### 13.09.2026 — согласованные локальные исправления; границы результата

После явного разрешения пользователя исходники изменены. Более ранние формулировки «код не менялся» относятся к диагностическому этапу, не к этому пакету. Полный состав и аппаратные критерии — в конце PROJECT_REQUIREMENTS_RU.md.

1. **Кнопки и музыка:** устранён собственный SteeringMediaKeyRouter и Accessibility key-filter. Добавлено только пассивное наблюдение динамических IEDIA/MEDIA broadcast и read-only обратной связи до 16 MediaSession. Низкоприоритетный receiver на отдельном looper не пересылает и не поглощает события; Binder-запросы вынесены в независимый session worker. Временная связь с ближайшей кнопкой — не доказательство исполнения команды; адресная/прерванная рассылка и внутренний AudioManager dispatch могут остаться невидимыми. При выключенной записи/debug подписки снимаются. MConfig/системный inputservice не изменялись. Вывод «канал MConfig забит» по-прежнему не доказан.
2. **Диагностика задержки:** ограниченный tail ActionRecorder читается с конца вне LOCK; экран и экспорт не читают файлы на main. Асинхронная запись имеет bounded queue и identity-fence сессии. В дампе отдельно резервируются queued-work и доступные SharedPreferences/диагностические писатели. Это исключает конкретный найденный полный readTail на main и улучшает следующее наблюдение, но не устанавливает источник всех задержек в движении.
3. **Приборка:** cold retry восстанавливает недостающую геометрию без Settings; late available callback не освобождает опубликованную Surface той же TextureView; stale callbacks отсекаются. Локальный TextureView update до отправки текущей lease в Navigator не раскрывает карту. Журнал различает local_surface_published / IPC sent / awaiting_dispatch / surface_updated. Успешный IPC не является ACK рисования и не подтверждает отсутствие белого native буфера после отправки; старый tile/pixel gate не возвращён.
4. **Дубли камер:** исходная control-запись может нести POLICE-алиас. При её замене единым знаком в обычных route/native on-route слоях исключаются control + этот POLICE, до вызова stock-отрисовки/создания placemark. Самостоятельные POLICE/ACCIDENT и genuine другие теги сохраняются. Свежий пустой ahead-инвентарь сохраняет ownership до истечения свежести/конца маршрута, а не восстанавливает пройденные pins. На входе в ownership обычные pins снимаются раньше единых; на выходе единые убираются раньше fallback. IMG_8390 уточняет приёмку: никакого маленького значка под основным уже при приближении, не только после проезда. Размер/z-index не служат исправлением.
5. **Подтверждённый stock-выбор типа камеры:** read-only исследован пользовательский Navigator-30.3.0-Natro-2.9.2-signed.apk, SHA256 3a59e53618bc86865983ed7dfac45552f0dab4c10f50c2353fc6979ac2189d71; classes7.dex SHA256 d4f0a5f5552bc63662e5167b594cee787eb8f9274e57e0b5d29ec4bf215d285a. `r74.c.provideStyle` выбирает тег с наибольшим приоритетом: SPEED_CONTROL=140 выше LANE_CONTROL=130 и ROAD_MARKING_CONTROL=110; TRAFFIC_CONTROL=141 выбирает camera_pdd, а не cross_road. Это объясняет риск прежнего «contains lane => синяя табличка». Новый код не копирует приватную таблицу: спрашивает установленный stock-провайдер через реальный RoadEventStylingProperties/RoadEventStyle и публичный ImageProvider.getId (`r74.a`: resource:<id>). Для poi_alerts_speed_control_24 добавочная табличка отсутствует; lane_control_26 / cross_road_control_26 / no_stopping_control_24 разрешают только соответствующий проверенный stock-вектор. Отсутствие ресурса не превращается в придуманный тип. Один раз на комбинацию тегов журналируется camera-stock-choice. В classes12.dex проверен интерфейс RoadEventStylingProperties: есть isOnRoute/getTags, но нет event ID/координат; глобально скрывать соседние ALWAYS-камеры вместо отсутствующего per-ID доказательства нельзя.
6. **ANCS:** Android пустой owner при STOPPING проходит существующий exact-owner/process-drain terminal путь сразу; live scan/GATT и чужая process reservation всё ещё блокируют терминальное доказательство. На iPhone local phase active отделён от peerReady в Snapshot/UI/C5; формулировка HELLO означает запрос, не подтверждённую доставку. Это конкретные локальные дефекты, не доказанное исправление GATT 133 или всех случаев post-update reconnect.

Проверки: 147 Python/Java fixture-тестов, 146 passed, один skipped из-за отсутствующего org.json JAR; Helper v70 personal contract script PASS. Продукционные методы выполнялись на инертных коллабораторах, а не реальном Android/GPU/CoreBluetooth; полной компиляции приложений нет. Новые APK/IPA не создавались, версии/подписи и удалённый Git не менялись. Подтверждение на KX11, длительная проверка физического управления музыкой и штатные данные «Поездки 2» остаются открытыми.


### 13.09.2026 — согласованный пакет вошёл в проверенную пару 2.9.3

Предыдущая запись об отсутствии полного Android build/APK относится к локальному этапу и дополнена фактическим выпуском. Source bc9a069460493711d4c51b7f58a752972007ff34, tree 3b12cdf7a66658c8ea566f74940d50afca024208; release CI 34777315541 успешен. 1883 Android unit-теста и 152 tool/production-policy проверки прошли без пропусков, Helper v70 contract PASS. Оба скачанных APK имеют прежний сертификат и проверенные v2/v3; CI zipalign/CRC/KX11 static gates и локальная сверка identity, хешей и DEX успешны. В Navigator относительно 2.9.2 изменён только classes19.dex; 17030 остальных payload entries совпадают побайтно, подписи META-INF исключены. Хеши пары, выбранных методов, исходного дерева и CI-артефактов сохранены в docs/releases/2.9.3.

Это подтверждает включение ранее описанных source-изменений в APK. Новых физических ECARX-измерений, снимка реального звука/канала MConfig или доказанного источника «Поездки 2» нет. Post-update запуск карты и ANCS, белый native/GPU кадр, камеры и ДТП на маршруте остаются аппаратными gates. iOS изменения Helper опубликованы без новой IPA; установки на KX11 не выполнялись.

### 14.09.2026 — отрицательная приёмка 2.9.3 и подтверждённые локальные дефекты

Подробный источник, SHA-256 трёх новых журналов и временная шкала: docs/FIELD_AUDIT_2026_09_14_RU.md. Процесс 2.9.3 отделён от старой истории 2.9.2; в 07:17:14 main стоит в QueuedWork, queued-work-looper в SharedPreferencesImpl/FileDescriptor.sync; восстановление через 8682 мс. Это блокировка Natro, не доказанный overflow MConfig или измеренная задержка физической кнопки. Broadcast observer получил 0 событий при трёх MediaSession. В кандидате 2.9.4 частые телеметрические кэши переведены на последний RAM-снимок и фоновый commit раз в 5 с; управляющие и постоянные данные не переводятся. Добавлен read-only logcat key observer только при имеющемся READ_LOGS; смены input-маршрута, системных настроек и root-команд нет.

После обновления Android получил полный GATT inventory, проверил Helper, подписал CONTROL/телеметрию. Два transient NS CCCD failure в 07:06:36–37 перевели его в WAIT_ANCS без таймера. Это source-дефект recovery: present-service отказ не обязан вызывать Service Changed. В кандидате отдельный same-owner retry с интервалами до 60 с и raw CCCD status; просроченный raw ATT callback не может быть назначен новой операции. GATT 133/причина самих CCCD-отказов не объявлены раскрытыми; bond/радио не сбрасывались.

Коррекция анализа камер 13.09: max-priority выбор обычного pin не описывает полный большой знак. Штатный PlatformIconsProviderImpl/RoadEventPlatformImage компонует независимые controls. Libmaps-mobile.so SHA-256 93e2334589328cbe9d1082925caeb9c2631cde43983aaab112c197fdf4885069 проверена статически, без запуска закрытого APK. Кандидат требует SPEED_CONTROL для круга скорости, сохраняет разметку в combined и не сливает соседние event IDs. Фотографии main/cluster задают три обязательных аппаратных случая; полная parity ещё не доказана.

IMG_8401 добавляет исчезающий хвостик светофора. Обнаружено несовпадение размеров stock getSize и полной bitmap/anchor рамки с тенью; исправлен shadow union и добавлены метки геометрии. Связь именно с пропажей хвоста на фото не доказана. Белая вспышка после правильной отправки lease тоже остаётся открытой; tile/pixel/ACK gate не возвращается. Достоверного нового источника «Поездки 2» нет, отвергнутые PA-значения не используются как её замена.

Выпуск 2.9.4: source 32ad1445df4e9fcd476d52a8a3fee22fe9bd9234, tree 7bdf381c13d75942ddd06e46e170ba648a63cb36. Source CI 34810494239 и release CI 34810494266 успешны; 1893 unit-теста/372 suite, 154 tool tests, актуальный Helper v70 contract прошли. Скачанная пара Natro 2.9.4 / 208021327 и Navigator 30.3.0 / 739564630 проверена по CI hashes/CRC/identity и прежнему одному сертификату; apksigner v2/v3, zipalign и KX11 static gates пройдены. Реальный Navigator отличается от 2.9.3 только classes19.dex, 17030 других payload entries совпадают без META-INF. Новые методы обнаружены в DEX обоих APK. Протокол docs/releases/2.9.4 отделяет эти проверки от ещё открытой физической приёмки; новых записей автомобиля и установок на KX11 не выполнялось.


## MConfig+ v46.1: MEDIA, TSR и белая полоса — 15.09.2026

Точный исходный APK v46.1 (SHA256 a1f2f1d486422afd65a47ba06201ed0a87a8dd4f7b9052db860765a5e3fb35e0) проверен по DEX/manifest/resources. Исторические выводы v45 сохраняются с указанием версии. Выключение mediaKeysCfg не отключает экспортированный IEDIA/MEDIA receiver: остаётся IEDIA → MEDIA bridge. Для запрошенного переноса MEDIA Natro использует отдельный NEDIA адрес той же длины; общий IEDIA адрес дал бы риск двойной команды. Исходный KeyEvent передаётся AudioManager при mode=0; при mode!=0 используется штатный broadcast, системный выбор плеера сохранён. Разрешение пользователя на новую опцию заменяет прежнее пассивное ограничение только при её явном включении.

Скрытие знака в MConfig — CB_ASY_TSR(0), восстановление (1), проверка getPA_Asy_TSR через 300 мс, повтор при ignition_state=2097415 (sensor2097408). Каталог подтверждает CB32770/0x8002, VHAL0x21408002. PA33289 системного SDK и PA33282 из DIM не взаимозаменяемы: новый доступ строго через именованный runtime getter. Compile SDK SHA25633d0c62851e834dbf443f969627973dbc6e00eec3dd1e8fb6fc9dfe5af78e886; AvailabilitySts Active1/NotActive2, NotAvailable3/Error4/invalid0. Успех команды и чтение не заменяют проверку экрана.

Белая полоса: при NaviMode=3 MConfig после1500мс запрещает SYSTEM_ALERT_WINDOW только com.ecarx.dimmenu, при другом режиме возвращает allow; до3 проходов с интервалом100мс. Natro применяет именованный AppOp, проверяет readback и явно восстанавливает allow при OFF; влияние на другие наложения этого пакета нужно проверить на KX11. Никаких QNX-записей/остановки DIM/SystemUI нет.

Фото альтернатив подтверждают отсутствие таблички при видимой линии на приборке. JVM replay воспроизвёл отказ collision по всему bitmap вокруг общей развилки; исправление проверяет тело таблички, сохраняет viewport и нативный MINOR. Успешная передача MapKit не доказывает видимость пикселей. Журнал(4) не доказывает конфликт MConfig и не исключает нагрузку Navigator; белый старт и минутные задержки остаются открытыми.

[Полные источники, матрица и проверки](docs/MCONFIG_MEDIA_OEM_ALTERNATIVES_2026_09_15_RU.md). GATE-090…092 открыты.

Проверенная пара 2.9.6 собрана из source `9e24b4fa5ae02275604c0fe65243b0b615dcf853`, tree `a719b92509b13f0ef432804615b89507a5a05a5c`. 1895 unit-тестов и166 проверок инструментов PASS; фактические DEX подтверждают наличие новых путей MEDIA/TSR/AppOps и collision тела таблички. Это статическая/программная проверка; режимы, задержки и видимость на реальном KX11 не приняты. [Протокол выпуска](docs/releases/2.9.6/README_RU.md).

### 15.09.2026: полный перенос кнопок, уточнение по DEX v46.1

Частный источник MConfig+ v46.1 SHA-256 a1f2f1d486422afd65a47ba06201ed0a87a8dd4f7b9052db860765a5e3fb35e0.
InputImpl даёт onKeyPressed/onKeyReleased с keyCode 210004/200400 (SRC/WeChat),
200231 (VA),119 (★),300001/300002 (DM L/R),26 (POWER). Это не подтверждает, что
неопознанная кнопка левого подрулевого переключателя из записи имеет какой-либо из этих кодов.
Таймер серии600мс существует только при ненулевом назначении следующего количества кликов;
третье отпускание немедленное. Длинное1000мс только SRC/VA/★, подавляет короткое даже при
пустом длинном назначении. ★ имеет дополнительную ветку10000мс → CB_Power_Softkey(3).

DEX slh.y 0x019a…0x01e8 опровергает инвертированные условия JADX: в режиме SRC multimedia
на штатный RSRC отправляется **одиночное** SRC с непустым назначением, отличным от20;
двойное/тройное и длинное исполняют своё назначение. В UI действие20 исключено из SRC1X.
DM при видимом dezz.monjaro.drive_modes отдаёт PREV_1/NEXT_1; иначе регулировка громкости
приоритетна при включённой громкости и (выключенных custom или isMusicActive=true).

Штатное ★: hfp message21 → xgo.cz → setFunctionValue(554762496, Integer.MIN_VALUE,0/1),
0 unbind /1 bind. Темы:538247424, та же зона;538247425DAY →538247426NIGHT →538247427AUTO.
Пассажирский экран:539495936, та же зона,0/1; это другой уровень API, чем raw PSD1OFF/2ON.
Камера360 при глобальной настройке360=true (значение по умолчанию) использует
**двухаргументный** setFunctionValue(587399424,value); альтернативный путь —
com.ecarx.action.avm.openclose. Нельзя переносить default zone на двухаргументную камеру.
Маршруты и значения подтверждены APK; физическая приёмка новых путей GATE-095 открыта.

### 15.09.2026 — уточнение режимов движения и локальный кандидат общего медиа

Фактический switch-payload `xke.f` различает `restoreDriveMode` (boolean, false,
`xgo.kc`) и `restoreDriveModeCfg` (int, −1, `xgo.gi`). Это разрешение сохранения/возврата
режима и необязательный режим после запуска соответственно. Команды кнопок используют
570491136 с DEFAULT_ZONE; значения 570491137/138/139/145/149/155/158. Их допуск зависит
от ignition_state=2097415 и готовности текущего режима. Повтор выбранного возвращает
предыдущий; собственный ответ/внешнее изменение различаются интервалом 2,5 с. Тема из
неизвестного значения переходит в AUTO (538247427), а не сразу в DAY.

В локальном кандидате все отображения медиа получают общий Snapshot. Сверка Android-
уведомлений идёт вне MAIN, результаты запросов/обложек отсеиваются по владельцу и порядку
событий. Громкость HOME/шторки/общего медиа читается и меняется отдельной ограниченной
очередью. Удалены независимые фильтры одинаковой обложки у соседних треков. Контроллер
MEDIA не загружает SharedPreferences на MAIN. Эти изменения адресуют конкретные пути
задержек в журнале (5); конфликт MConfig или исчезновение белого старта ими не доказаны.

Для альтернативной таблички добавлено пять расстояний на каждой из восьми сторон
развилки при точном кончике и запрете перекрытия обязательных объектов. `submitted`
теперь сопровождается стороной/вариантом, но по-прежнему не является ACK видимых пикселей.

Полный каталог 0…34 и дополнительный общий каталог меню Natro 100, параметры, источники
и границы проверок: [аудит кандидата](docs/BUTTONS_MEDIA_MAP_2026_09_15_RU.md).
Программные replay/source gates пройдены; полного Android build/APK на этом этапе нет.
Автоматическая проверка дважды отклонила git push, не признав разрешение на эту внешнюю
публикацию. Удалённые записи другим инструментом не выполнялись. Следующий этап —
разрешённая отправка конкретного кандидата и полный CI, затем подписанная пара и KX11.

### 15.09.2026 — завершённая сборка 2.9.7
...
Предыдущая блокировка публикации снята прямым разрешением пользователя. [Итоговый протокол](docs/releases/2.9.7/README_RU.md).
