...
Версия 1.6 • дополнение 11 сентября 2026 года • автомобиль и прошивка из переданных пользователем материалов. Исходные разделы и публичный ZIP 1.0 сохраняют результаты 4 сентября, редакция 1.2 — расширенный разбор 5 сентября.
Аппаратные замечания после Natro 2.8.4, соответствующие изменения и подписанная пара 2.8.5 актуализированы ниже; это не означает аппаратную приёмку до отдельного измерения на KX11.
...
### Коррекция источника поездки после проверки 2.8.4
...
| Текущая поездка | Проверка 2.8.4 отвергла ITripData для штатной «Поездки 2»: он дал общий одометр 37 278 км и время 0. Typed PA getters/callbacks `OdometerTripMeter2` + `EDT_time2` подтверждены статически и компиляцией | Парный raw/format/штатный экран/Natro для единиц, reset, reconnect и ignition; до этого не объявлять `/10` и минуты доказанными, GATE-073 |
| CPU Natro/HUD/DIM | Статически удалены повторные layout/list/reflection/allocation hot paths, одинаковые telemetry redraw и PA callback без demand; разрешение/FPS/AA/частоты не снижались | Три парных 10-минутных прогона 2.8.4/2.8.5 с Perfetto/simpleperf, allocation/GC и frame pacing; процент снижения не объявлять до GATE-075 |
