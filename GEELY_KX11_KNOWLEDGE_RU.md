...
[14 открытых вопросов](docs/geely-kx11/open_questions.json).
...
Версия 1.0 • повторный разбор 4 сентября 2026 года • автомобиль и прошивка из переданных пользователем материалов.
...
1. Начать с этого документа, затем открыть карточку нужной подсистемы и её каталог. Для новой команды проверить файл `firmware/conflicts.json` и происхождение используемого класса.
...
| Parking distance control status | CarSignal `28995` / `PrkgDstCtrlSts` | Natro active при `2/3`, closed при `1` по записанным данным |
...
Текущая политика Natro для уведомлений: если 28995 = 2/3, overlay активен независимо от 29021/29043; иначе, если известен 29021, использовать именно его `==3`; только до первого 29021 использовать 29043=1/2. Это отражает факт, что парктроник остаётся видимым, когда 360-сигналы уже вернулись к `8/0`. Источник N1 `EcarxExternalOverlayPolicy.java:9–52`, и фактические переходы в общем LOG-каталоге. Старая схема «любое overlay окно = пауза» здесь не нужна.

Что ещё не доказано: однозначная длительность каждого переходного состояния, поведение при потере callback/Binder, старые кешированные значения после смены зажигания, различие отдельных camera/PAS поверхностей в каждой комплектации. Для следующего исправления нужен совместный журнал `28995+29021+29043` с timestamp и окном отображения; одно значение `PAC=1` недостаточно.
...
| 1 | Устойчивая pause/resume при PAS | три статуса, приоритет 28995, записи переходов | lifecycle reset при Binder/ignition; сопоставление с UI для длительных/пропущенных callback |
...
| PAS_PAUSE | Пауза и возобновление уведомлений | Сброс устаревших значений при Binder/зажигании и полное восстановление | Проверить жизненный цикл политики по timestamp; подтвердить начало и конец физического отображения окна |
