...
- синхронизация камеры с независимыми zoomDelta/tilt/focus и прямой рендер активного
  `DrivingRoute.getGeometry()` со своими цветом/обводкой/толщиной.

Ещё не реализованы в этом каталоге: цветовые сегменты пробок, полный custom cursor и применение
всех параметров `mainMap` к основной карте. Старое покадровое копирование не используется.
