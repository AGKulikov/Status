...
- отдельный слой светофоров с координатой, сигналом, стрелкой и цифровым отсчётом для HUD и
  приборной карты; у каждой карты свой переключатель, основной `MapWindow` не меняется;
...
- один владелец камеры каждого внешнего `MapWindow`: примитивные Guidance-кадры применяют точные
  zoomDelta/наклон/focus без конкурирующего `GuidanceCamera`; нестабильный automotive
  `NavigationLayer` без камеры не создаётся, маршрут и курсор остаются независимыми;
...
- штатные значки и направление камер через Yandex road-events style provider; для всех 22
  `EventTag` можно выбрать «скрыть», «всегда» или «только с маршрутом»; последний безопасно
  включает standalone-слой лишь при активном маршруте;
