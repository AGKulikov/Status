...
- один владелец камеры каждого внешнего `MapWindow`: примитивные Guidance-кадры применяют точные
  zoomDelta/наклон/focus без конкурирующего `GuidanceCamera`; второй `NavigationLayer` оставлен
  только для route-only дорожных событий, маршрут и курсор остаются независимыми;
