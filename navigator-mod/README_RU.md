...
- штатный `GuidanceCamera` во втором `NavigationLayer`, созданном с отключёнными маршрутами и
  курсором: плавная интерполяция MapKit без конкурирующих `Map.move`, независимые
  zoomDelta/2D-3D/focus и прямой рендер активного `DrivingRoute.getGeometry()` со своими
  цветом/обводкой/толщиной маршрута;
- штатные значки и направление камер через Yandex road-events style provider; для всех 22
  `EventTag` можно выбрать «скрыть», «всегда» или «только с маршрутом»;
