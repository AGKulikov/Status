...
Проверяемая карта владельцев и динамических UI-контейнеров baseline 30.3.0 находится в
[`docs/NAVIGATOR_30_3_UI_CONTAINER_AUDIT_RU.md`](../docs/NAVIGATOR_30_3_UI_CONTAINER_AUDIT_RU.md).
...
- кнопка «окно / полный экран» третьим элементом настоящего левого блока Навигатора, со
  штатной прямоугольной подложкой; её фактические visibility/alpha/scale/translation перед
  каждым draw зеркалируются с точной пары road-event/voice без собственного таймера;
- перемещение, resize, независимые locks, сохранение геометрии, aspect lock;
- скругление, фон, opacity, border, shadow и видимость кнопки режима; её размер, подложку,
  pressed-state задаёт штатная оболочка, геометрию — штатный vessel, а цикл появления — точная
  пара соседних контролов Навигатора;
...
- отдельный слой светофоров с координатой, сигналом, стрелкой и цифровым отсчётом для HUD и
  приборной карты; штатная карточка получает хвост к точной координате и выбирает свободную
  сторону маршрута, региональный fallback также содержит хвост; статичный сигнал без таймера в
  этот слой не попадает, у каждой карты свой переключатель, основной `MapWindow` не меняется;
- отдельная географически привязанная подсказка по полосам для HUD и приборной карты: позиция
  берётся из `UpcomingLaneSign.getPosition()`, полная штатная `BalloonTexture` включает хвост к
  этой точке, а слой исчезает при неактивном маршруте или устаревшем образце;
...
- штатные значки дорожных событий через стабильный Yandex `RoadEventsLayer` и отдельное
  ...
  automotive-слой. Внешняя камера HUD Speed и запись Яндекса объединяются без потери типов и двух
  направлений. Обычная камера скорости — один компактный круг без мини-камеры; составная камера
  дополняет его точной штатной пластиной контроля полос/перекрёстка/остановки. У треугольника
  направления раздельно настраиваются длина, ширина только дальнего основания, цвет и прозрачность;
...
- штатные стрелки предстоящих поворотов на маршрутной полилинии сохраняют позиции и геометрию
  ...
  цвет и толщину обводки, длину тела и независимый размер треугольного наконечника. Ширина тела
  остаётся связанной с толщиной самой маршрутной линии и не получает отдельного масштаба;
  диапазоны длины и наконечника начинаются с 10%;
- табличка полос, светофоры и составные знаки камер используют общий screen-space расчёт восьми
  сторон вокруг координаты с проверкой границ viewport и взаимных пересечений;
