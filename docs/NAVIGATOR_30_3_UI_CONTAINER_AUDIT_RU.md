...
обычная кнопка Natro не реализует этот интерфейс и не меняет решение строки. Однако это не даёт
ей видимость точной пары: строка может оставаться `VISIBLE` из-за ruler/music/другого штатного
потомка, когда road-event и voice уже получили индивидуальный `GONE`. Поэтому принадлежность
внутреннему `LinearLayout` наследует геометрию vessel, но недостаточна для автоскрытия третьей
кнопки; её состояние требуется синхронизировать с фактическими road-event/voice views.
...
5. перед каждым draw читает фактическую `visibility` обоих exact-pair controls и зеркалирует
   объединённое состояние; alpha/scale/translation копирует с видимого штатного слота, пока
   кнопка не нажата, без собственного fade или таймера;
6. pre-draw callback не ищет контейнер и не меняет иерархию: rebind-poller только обнаруживает
   заменённый controller/rail и больше не принуждает кнопку к `VISIBLE`;
7. при замене controller отсоединяет старый экземпляр и подключает его к новому валидному rail;
8. если rail временно отсутствует, держит единственный экземпляр `GONE` в `controlLayer`.
...
wrapper, третий индекс кнопки, отсутствие собственного fade/координаты, покадровое чтение
exact-pair без изменения иерархии, границы `FluidContainer` и отсутствие прямой зависимости
добавленного DEX от класса Яндекса.
...
## Дополнение после аппаратного отказа 2.7.2

Снимок `IMG_8236.jpeg` опроверг достаточность предыдущего padding-only исправления: даже при
нулевом padding известных `PaddingtonView` весь верхний Guidance-блок оставался ниже реального
верхнего края карты. Поэтому исполняемый контракт теперь проверяет экранную геометрию, а не имя
предполагаемого inset-контейнера.

Видео `IMG_8239.MOV` и `IMG_8241.MOV` также опровергли предположение, что третья кнопка
автоматически наследует скрытие от общего rail: штатная пара исчезала, а строка и кнопка Natro
оставались. Для кандидата 2.7.3 сохранён тот же точный третий слот, но добавлен отдельный
pre-draw observer фактических road-event/voice views. Пятисекундный poller теперь отвечает только
за rebind после замены Conductor-дерева и не имеет безусловного `setVisibility(VISIBLE)`.
Аппаратное подтверждение этого изменения остаётся за `GATE-024/GATE-025`.

В оконном режиме контроллер находит безымянный `MATCH_PARENT`-корень
`car_guidance_controller` по двум прямым детям — `contextmaneuverview` и `speed_group` — и
сопоставляет его `getLocationInWindow()` с фактическим `activity_search_map_view`. Излишек общей
вертикальной координаты компенсируется только `topMargin` этого общего корня. Поскольку высота
корня остаётся `MATCH_PARENT`, верхние дети поднимаются вместе, а физическая нижняя граница и
привязанная к ней ETA не перемещаются. Штатные 12 dp обоих прямых верхних детей принудительно
сохраняются.

Коррекция не применяется во время transient/alpha-анимации Conductor, не использует
`translationY` и имеет защиту от повторного накопления, если vendor-parent не учитывает
`MarginLayoutParams`. Исходные margin сохраняются по identity дерева и восстанавливаются при
замене Guidance, переходе в полный экран и уничтожении Activity. Диагностика фиксирует верх
viewport/root, margin, padding, translation и координаты двух виджетов; фактический результат
по-прежнему закрывается только `GATE-027` на KX11.
