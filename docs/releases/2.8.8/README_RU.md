...
**Отрицательный аппаратный результат 13.09.2026:** пользователь сообщил, что после
2.8.8 карта не отображается ни на HUD, ни на приборке. NAV-018 имеет статус ошибки,
GATE-082 не пройден. Пользователь подтвердил установку обоих APK этого комплекта.
В выданном Navigator найден несовместимый reflection-вызов setMapLoadedListener:
код искал MapLoadedListener, фактический API принимает WeakReference. Запуск
обрывается до addSurface. [Исправление исходников и доказательства](../../map-startup-2026-09-13/README_RU.md)
не меняют байты этого уже выданного выпуска.
Приведённые ниже успешные CI/подписи относятся к артефактам и не отменяют этот отказ.
