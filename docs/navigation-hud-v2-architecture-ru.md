...
- В этот второй `MapWindow` подключается штатный `NavigationLayer` той же automotive
  `Navigation`, но в нём включены только `GuidanceCamera` и дорожные события. Маршрут и курсор
  остаются независимыми слоями HUD со своими цветами. Поэтому камера интерполируется внутри
  MapKit, а последовательные ручные `Map.move` больше не прерывают друг друга при поворотах.
...
резервная начальная позиция. После подключения automotive `Navigation` независимую HUD-камеру
ведёт нативный `GuidanceCamera`. Быстрый snapshot идёт отдельно от геометрии:
...
- три точечных lifecycle/intent-вызова одного entry point в `MapActivity` (`classes4.dex`),
...
5. Отдельный `OffscreenMapWindow`, настоящий Surface-элемент HUD и независимые
   ...
   HUD-карт и прозрачный HUD-режим «только дороги» выполнены; отдельное управление глобальным traffic-layer основной карты отложено до
