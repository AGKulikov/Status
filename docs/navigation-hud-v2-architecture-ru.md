...
2. Разрешено изменить `classes4.dex`, добавить `classes19.dex` и заменить только бинарный
   `AndroidManifest.xml`. SHA-256 manifest/classes4 закреплены в verifier; manifest меняет только
...
3. Нативный MapKit, загрузчик офлайн-карт, Passport-контур и существующие добавки 30.3.0 не
   патчатся повторно.
4. Полная пересборка APK запрещена: apktool собирает только проверенный `classes4.dex`, бинарный
...
2. Минимальный patch pipeline: `classes4.dex`, новый `classes19.dex` и точечная правка бинарного
