...
добавляем изолированный DEX и только точечную прозрачную тему `MapActivity`.
...
1. `classes14.dex`, `classes18.dex`, все `assets/**` и `lib/**` остаются побайтно равны baseline.
2. Разрешено изменить `classes4.dex`, добавить `classes19.dex` и заменить только два
   воспроизводимых entry темы: `AndroidManifest.xml` и `resources.arsc`. Их SHA-256 закреплены в
   verifier; manifest меняет только theme `MapActivity`, а `aapt badging` остаётся идентичным.
3. Нативный MapKit, загрузчик офлайн-карт, Passport-контур и существующие добавки 30.3.0 не
   патчатся повторно.
4. Полная пересборка APK запрещена: apktool в отдельном no-source проекте строит только проверенные
   manifest/resources, после чего repacker точечно заменяет четыре entry, выполняет zipalign и
   подпись.
...
- четыре точечных lifecycle/intent-вызова одного entry point в `MapActivity` (`classes4.dex`),
  включая безопасное переключение окна в конце `onResumeFragments` после появления
  оконного токена Activity и MapKit;
...
- обязательная кнопка «окно / полный экран» внутри Навигатора, с настройкой позиции, размера и
  прозрачности.
- `NatroTransparentAppTheme`, повторяющая проверенные window-атрибуты 29.4.2: прозрачный фон,
  `windowIsTranslucent=true`, dim выключен.
...
2. Минимальный patch pipeline: `classes4.dex`, новый `classes19.dex` и отдельная воспроизводимая
   сборка только прозрачной theme пары manifest/resources — выполнено.
