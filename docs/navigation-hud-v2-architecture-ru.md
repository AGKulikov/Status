...
- Камера, маршрут и курсор второго `MapWindow` принадлежат одному независимому рендереру.
  ...
  владельца. Обязательный для MapKit automotive `GuidanceCamera` существует только внутри
  минимального слоя событий: автоматическое переключение, zoom и rotation выключены, режим
  сразу установлен в `FREE`, поэтому фактических движений карты и секундных скачков нет.
- Дорожные события разделены по семантике. Отдельный штатный `RoadEventsLayer` показывает
  `ALWAYS`. Минимальный automotive `NavigationLayer`, связанный с той же настоящей
  `Navigation`, показывает только `ROUTE_ONLY` через `setRoadEventVisibleOnRoute`; его собственные
  маршрут, курсор, request points и баллоны отключены. Слой создаётся только на время активного
  пользовательского маршрута и удаляется сразу после его завершения: свободное вождение не держит
  лишнюю нативную камеру и обработку событий. Это исправляет ошибку 2.5.4, где
