...
- Подсказки по полосам имеют отдельный `showLaneGuidance`. Полная штатная `BalloonTexture` вместе
  ...
  HUD-виджет, и слой удаляется через 1,5 секунды без свежего маршрутного образца. Табличка полос,
  светофоры и камеры разделяют один координатор занятых областей и могут выбрать восемь сторон.
