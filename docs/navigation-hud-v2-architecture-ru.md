...
- В этот второй `MapWindow` подключается штатный `NavigationLayer` той же automotive
  `Navigation`, но только для дорожных событий. Камера, маршрут и курсор принадлежат одному
  независимому рендереру. Положение берётся из Guidance с частотой до 30 Гц, одинаковые
...
- Отдельный `RoadEventsLayer` показывает выбранные события на всей карте, а
  `NavigationLayer.setRoadEventVisibleOnRoute` — только вдоль активного маршрута. Для каждого из
  22 `EventTag` доступны `HIDDEN`, `ALWAYS` и `ROUTE_ONLY`; штатный style provider 30.3.0
  сохраняет родные значки, масштаб и направление камер.
