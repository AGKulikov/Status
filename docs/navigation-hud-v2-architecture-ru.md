...
Яндекса. Мы не воспроизводим и не «улучшаем» этот механизм: сохраняем его защищённые байты,
добавляем изолированный DEX и точечно меняем bootstrap theme `MapActivity` в бинарном manifest.
...
1. `resources.arsc`, все `res/**`, `classes14.dex`, `classes18.dex`, `assets/**` и `lib/**`
   остаются побайтно равны baseline.
2. Разрешено изменить `classes4.dex` и `classes12.dex`, добавить `classes19.dex` и заменить только
   бинарный `AndroidManifest.xml`. SHA-256 manifest/classes4/classes12 закреплены в verifier;
   manifest меняет только
   theme `MapActivity`, а `aapt badging` остаётся идентичным.
3. Нативный MapKit, загрузчик офлайн-карт, Passport-контур и существующие добавки 30.3.0 не
   патчатся повторно.
4. Полная пересборка APK запрещена: apktool собирает только проверенные `classes4.dex` и
   `classes12.dex`, бинарный
   manifest патчится по точному SHA/offset, затем repacker заменяет два entry, добавляет
   `classes19.dex`, выполняет zipalign и подпись.
...
- Основная карта сохраняет штатные камеру, маршрут и курсор Навигатора. Её безопасный профиль
  меняет focus point, scale, FPS, day/night, POI, модели и видимость substrate-объектов.
...
- Камера, маршрут и курсор второго `MapWindow` принадлежат одному независимому рендереру.
  Положение берётся из Guidance с частотой до 30 Гц, одинаковые
  `CameraPosition` отбрасываются; поэтому точные zoom/наклон/focus применяются без конкурирующего
  владельца. Automotive `NavigationLayer` и его `GuidanceCamera` не создаются: журнал KX11 от
  01.09.2026 повторил завершение процесса через 9–13 секунд и с включённой камерой слоя.
- Дорожные события обслуживает только штатный стабильный `RoadEventsLayer`. `ALWAYS` видим
  постоянно, а `ROUTE_ONLY` включается при свежем активном пользовательском маршруте. Это
  безопасный fallback MapKit 30.3.0: он сохраняет родные значки и масштаб, но может включить
  ближайшее событие с параллельной дороги. Точное route matching нельзя получать ценой падения
  всего Навигатора. Направление активных камер берётся отдельно
...
- Светофоры Windshield с валидным сигналом и координатой помещаются в собственную
  ...
  `MapWindow` этот слой не получает. Маркер содержит три сигнала, стрелку, цифровой отсчёт и
  штатный хвост, который связывает перенесённую в свободную сторону карточку с координатой.
  Образец без `secondsLeft` остаётся доступен отдельному HUD-модулю, но не создаёт статичный знак
  на карте.
- Подсказки по полосам имеют отдельный `showLaneGuidance`. Полная штатная `BalloonTexture` вместе
  с хвостиком привязывается к географической точке ближайшего `UpcomingLaneSign`; это не
  HUD-виджет, и слой удаляется через 1,5 секунды без свежего маршрутного образца. Табличка полос и
  светофоры могут выбрать восемь сторон. Камера первой резервирует в том же координаторе свою
  центрированную область, но её знак и вершина сектора всегда остаются на точной координате;
  подвижные карточки обходят камеру, а не сдвигают её с маршрута.
- Конечная точка активного маршрута имеет отдельный `showDestination` и рисуется на HUD и
  приборной карте из последней уже прочитанной точки `DrivingRoute`. Дополнительного опроса
  Guidance нет; маркер удаляется тем же атомарным обновлением при завершении или отмене маршрута.
...
Камера основной карты передаётся не чаще 10 раз/с с coalescing и используется только как
резервная начальная позиция. После подключения automotive `Navigation` независимые камеры
получают примитивную позицию Guidance до 30 раз/с и имеют ровно одного владельца на MapWindow.
Быстрый snapshot идёт отдельно от геометрии:
...
Буфер создаётся в фактическом размере элемента HUD (не в разрешении основного дисплея), что
снижает fill-rate без потери резкости на проекторе.
...
используются только для холодного старта внешних поверхностей, поэтому ручное движение основной
карты не меняет независимые `zoomDelta`.

## Фоновое владение MapKit

Штатный `GuidanceService` 30.3.0 остаётся location foreground service в основном процессе и
сохраняет активную сессию при открытии другого приложения. Отдельно от него lifecycle интерфейса
вызывает `MapKit.onStop()`, поэтому патч содержит `BackgroundMapLease` и два дополнительных hook:
начало `MapActivity.onStart()` и конец `MapActivity.onStop()`.

Аренда вызывает `MapKit.onStart()` только когда Activity в фоне и хотя бы один живой
HUD/cluster `OffscreenMapWindow` подключён к Surface. После отложенного process-lifecycle stop
выполняется одно подтверждение, а не периодический watchdog. Снятие обеих Surface в фоне вызывает
`MapKit.onStop()`; при возврате в Activity владение передаётся штатному lifecycle.
...
- пять точечных lifecycle/intent-вызовов одного entry point в `MapActivity` (`classes4.dex`),
  включая безопасное переключение окна в конце `onResumeFragments` после появления оконного
  токена Activity и MapKit; в начале `onCreate` отдельная инструкция возвращает родную тему;
- обновления пробок меняют палитру существующей полилинии HUD на месте: коллекция
  маршрута не очищается и между кадрами не появляется пустой слой;
- одна фиксация окна, которая блокирует перемещение и размер и скрывает обе ручки;
...
- кнопка «окно / полный экран» в обоих режимах вставляется третьей в штатную левую временную
  колонку после кнопок дорожного события и голосового помощника. Поэтому она появляется после
  касания карты и автоматически скрывается тем же механизмом; отдельно настраиваются размер,
  прозрачность и видимость, а положение намеренно фиксировано;
- в оконном режиме верхний системный inset принудительно равен нулю только внутри viewport
  Навигатора, поэтому его элементы не резервируют место под внешнюю строку состояния. При
  возврате в полноэкранный режим немедленно восстанавливаются реальные системные insets.
- уже существующая в baseline прозрачная non-floating тема используется только при системном
  запуске (`windowIsTranslucent=true`, dim выключен), после чего до логики Навигатора
  восстанавливается его `SplashAppTheme`.
- точечный hook конструктора `MapView` в `classes12.dex` выбирает встроенный атрибут MapKit
  `MOVABLE` только для запуска с `ddnavwin`/оконным action. В этом режиме MapKit создаёт
  `PlatformGLTextureView`, после commit окна decor безопасно клипуется по круглому outline.
  Полноэкранная карта продолжает использовать исходный Vulkan/SurfaceView путь.
...
Manifest меняет только theme экспортируемой 30.3.0 `MapActivity`; component, intent filters и
permissions остаются прежними. Natro запускает её напрямую, чтобы splash не потерял extras;
`TransparentSplashActivity` остаётся fallback для старых сборок. Общий контроллер применяет
оконный контракт в конце `onResumeFragments` и в `onNewIntent`; смена режима выполняется через
пересоздание Activity. После type 2038/2003 и translucent format он фиксирует bounded frame и
удерживает прозрачными Window/decor/content/map roots. Поэтому системные панели остаются видимыми,
а полноэкранная чёрная поверхность не создаётся.
...
2. Минимальный patch pipeline: `classes4.dex`, `classes12.dex`, новый `classes19.dex` и точечная правка бинарного
   manifest при полностью неизменных `resources.arsc`/`res/**` — выполнено.
...
5. Отдельный `OffscreenMapWindow`, настоящий Surface-элемент HUD и независимые
   ...
   сегменты пробок маршрута, custom cursor, независимые camera/focus/scale/style HUD и приборной
   карт, безопасное оформление основной карты, фоновая аренда MapKit и прозрачный HUD-режим
   «только дороги» выполнены; отдельное управление глобальным traffic-layer основной карты
   отложено до стабильной публичной точки входа.
