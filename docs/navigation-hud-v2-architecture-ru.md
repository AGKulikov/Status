...
- Камера, маршрут и курсор второго `MapWindow` принадлежат одному независимому рендереру.
  ...
  `GuidanceCamera` и секундных скачков.
- Дорожные события рисует только отдельный штатный `RoadEventsLayer`. Нативный automotive
  `NavigationLayer` не создаётся: в MapKit 30.3.0 его конфигурация «road events без собственной
  камеры» асинхронно завершает процесс Навигатора на KX11. Для каждого из 22 `EventTag` доступны
  `HIDDEN`, `ALWAYS` и `ROUTE_ONLY`; последний включает слой только при активном маршруте, но
  стабильный API может показать и ближайшие к маршруту события. Штатный style provider сохраняет
  родные значки, масштаб и направление камер.
