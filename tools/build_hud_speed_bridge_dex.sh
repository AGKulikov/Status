...
SOURCE_DIR="$PROJECT_DIR/hud-speed-bridge/src/main/java"
OUTPUT_DIR="${1:-$PROJECT_DIR/build/hud-speed-bridge}"
...
BUILD_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/natro-hud-speed-dex.XXXXXX")"
...
if [ "${#SOURCES[@]}" -eq 0 ]; then
  printf 'No HUD Speed bridge Java sources found\n' >&2
  exit 1
fi
...
install -m 0644 "$BUILD_ROOT/dex/classes.dex" "$OUTPUT_DIR/classes3.dex"
sha256sum "$OUTPUT_DIR/classes3.dex"
