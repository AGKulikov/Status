...
"""
    ...
    def test_hud_renderer_forbids_native_navigation_layer_on_external_map(self):
        ...
        camera_policy = (TOOLS.parent / "navigator-mod/src/main/java/ru/natro/navigation/RouteCameraPolicy.java").read_text()
        self.assertIn("new_pin_alerts_lanecamera_40", camera_policy)
        self.assertIn("new_pin_alerts_crossroad_camera_40", camera_policy)
        self.assertIn("RouteRoadEventMapLayer.selectedCameraImageId", camera)
        self.assertNotIn('tags.contains("LANE_CONTROL")', camera)
        self.assertIn("RouteCameraPolicy.detailDrawables(camera.controlTags)", camera)
        self.assertIn("RouteCameraPolicy.showSpeed(camera.controlTags, camera.speedLimit)", camera)
        ...
        self.assertIn("float safeScale = Math.max(.01f, textureScale)",
                      traffic_lights)
        self.assertIn("BalloonTextureBounds.pixels", traffic_lights)
        self.assertIn("shadowRadius, shadowX, safeScale", traffic_lights)
        self.assertIn("shadowRadius, shadowY, safeScale", traffic_lights)
        ...
        self.assertNotIn("useCompositeIcon", traffic_lights)
        self.assertNotIn('"traffic-light-connector"', traffic_lights)
        self.assertIn("applyBalloonIcon(marker, provider, style)", traffic_lights)
        self.assertNotIn("ConnectorTexture.OVERSAMPLE", traffic_lights)
        self.assertIn("BalloonPath.create(body, bodyRadius", traffic_lights)
        ...
        self.assertNotIn("Float.valueOf(zIndex - .01f)", traffic_lights)
        self.assertEqual(traffic_lights.count("MapObjectLayerFactory.create(map"), 1)
        self.assertNotIn('"traffic_light_leg_size"', traffic_lights)
        self.assertNotIn('invoke(composite, "removeAll"', traffic_lights)
        self.assertIn("anchor.x * width, anchor.y * height", traffic_lights)
        self.assertIn("MapOverlayPlacementCoordinator.placementLegNames()", traffic_lights)
        ...
        self.assertNotIn("setRoadEventsOnRoute", event_sync)
        self.assertIn("layer.render(Collections.unmodifiableList(converted))", event_sync)
        self.assertIn('invoke(source, "getEventId"', event_sync)
        self.assertIn('invoke(source, "getLocation"', event_sync)
        self.assertIn('invoke(source, "getTags"', event_sync)
        ...
        self.assertNotIn("if (next == null) next = placement.reserve", alternative)
        self.assertIn("hide(marker);", alternative)
        self.assertIn("BalloonPath.create(body, radius", alternative)
        ...
        self.assertIn("stockContent.read(route, forkOnAlternative", alternative)
        self.assertNotIn("formatTimeDelta", alternative)
        ...
        self.assertLess(alternative.index('invoke(line, "setVisible",'
                        ...
                        alternative.index("CalloutModel model = readCallout"))
        self.assertNotIn("fallbackRoutePoint", alternative)
        ...
        self.assertNotIn('name = "Вариант " + (index + 1)', alternative)
        self.assertIn("if (!hideSharedPrefix(line, route, forkOnAlternative)) continue", alternative)
        ...
        self.assertIn("model.content.color", alternative)
        self.assertIn("model.content.icon", alternative)
