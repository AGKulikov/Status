...
"""
    ...
    def test_pair_signer_requires_one_stable_certificate_and_exact_baseline(self):
        ...
        self.assertIn('EXPECTED_NATRO_VERSION_NAME="${EXPECTED_NATRO_VERSION_NAME:-2.7.6}"', pair)
        self.assertIn('EXPECTED_NATRO_VERSION_CODE="${EXPECTED_NATRO_VERSION_CODE:-208021309}"', pair)
        self.assertIn('test "$VERSION_NAME" = "$EXPECTED_NATRO_VERSION_NAME"', pair)
        verifier = (TOOLS / "verify_kx11_navigation_pair.py").read_text()
        self.assertIn('os.environ.get("EXPECTED_NATRO_VERSION_NAME", "2.7.6")', verifier)
        self.assertIn('os.environ.get("EXPECTED_NATRO_VERSION_CODE", "208021309")', verifier)
        ...
        workflow = (TOOLS.parent / ".github" / "workflows"
                    / "verify-navigation-hud-v2.yml").read_text()
        self.assertIn("if (version == '2.7.6')", build)
        self.assertIn("return 208021309", build)
        self.assertIn("VERSION_NAME: '2.7.6'", workflow)
        self.assertIn("VERSION_CODE: '208021309'", workflow)
