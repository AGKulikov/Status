...
"""
    ...
    def test_pair_signer_requires_one_stable_certificate_and_exact_baseline(self):
        ...
        self.assertIn('EXPECTED_NATRO_VERSION_NAME="${EXPECTED_NATRO_VERSION_NAME:-2.6.8}"', pair)
        self.assertIn('EXPECTED_NATRO_VERSION_CODE="${EXPECTED_NATRO_VERSION_CODE:-208021301}"', pair)
        self.assertIn('test "$VERSION_NAME" = "$EXPECTED_NATRO_VERSION_NAME"', pair)
        verifier = (TOOLS / "verify_kx11_navigation_pair.py").read_text()
        self.assertIn('os.environ.get("EXPECTED_NATRO_VERSION_NAME", "2.6.8")', verifier)
        self.assertIn('os.environ.get("EXPECTED_NATRO_VERSION_CODE", "208021301")', verifier)
        ...
        workflow = (TOOLS.parent / ".github" / "workflows"
                    / "verify-navigation-hud-v2.yml").read_text()
        self.assertIn("if (version == '2.6.8')", build)
        self.assertIn("VERSION_NAME: '2.6.8'", workflow)
    ...
    def test_hud_renderer_forbids_native_navigation_layer_on_external_map(self):
        ...
        self.assertIn("attachModeButtonToStockLeftRail", controller)
        self.assertIn('viewByName("guidance_open_voice_search")', controller)
        self.assertIn("rail.addView(button", controller)
