...
# The isolated project has no manifest/resources and contains only classes4. Apktool therefore
# cannot rebuild any protected APK entry even accidentally.
XDG_DATA_HOME="$BUILD_ROOT/apktool-data" \
  java -jar "$APKTOOL_JAR" b --no-apk "$DEX_PROJECT"
CLASSES4_DEX="$DEX_PROJECT/build/apk/classes4.dex"
if [ ! -f "$CLASSES4_DEX" ]; then
  printf 'apktool did not produce the reviewed classes4.dex output\n' >&2
  exit 1
fi
