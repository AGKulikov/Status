...
public final class AncsProtocol {
    ...
    /**
     * ANCS {@code EventFlagPreExisting}. iOS sets this bit while replaying notifications that
     * were already present when Notification Source was subscribed; those are not real-time
     * arrivals for the current connection session.
     */
    public static final int EVENT_FLAG_PRE_EXISTING = 0x04;
    ...
    /** Returns whether an event belongs to iOS's initial Notification Center replay. */
    public static boolean isPreExisting(Event event) {
        return event != null && (event.flags & EVENT_FLAG_PRE_EXISTING) != 0;
    }
