...
public class TelnetTransport implements ShellTransport {
    ...
    private String readResponseUntilMarker() throws Exception {
        ...
        byte[] buf = new byte[4096];
        // The echoed command contains the token too; only a separate completion line counts.
        byte[] markerBytes = ("\n" + END_MARKER + "\n").getBytes(StandardCharsets.US_ASCII);
        boolean markerSeen = false;
        long deadline = System.nanoTime() + java.util.concurrent.TimeUnit.SECONDS.toNanos(15);

        while (true) {
            if (System.nanoTime() > deadline || collected.size() > 128 * 1024)
                throw new java.io.IOException("Telnet response limit exceeded");
            int len;
            try {
                len = in.read(buf);
            } catch (java.net.SocketTimeoutException e) {
        ...
        if (!markerSeen) throw new java.io.IOException("Telnet disconnected without completion marker");
