...
final class PinDialogs {
    ...
    void showSetOrChange() {
        ...
        @StringRes int title = changingPin ? R.string.service_mode_change_pin_title : R.string.service_mode_set_pin_title;
        @StringRes int message = (changingPin && appsToHideStorage.hasHiddenApps())
                ? R.string.service_mode_pin_dialog_message_warn_hidden
                : R.string.service_mode_pin_dialog_message;
     ...
     */
    void showRequired() {
        showInputDialog(R.string.service_mode_set_pin_title, R.string.service_mode_pin_required_dialog_message);
    }

    private void showInputDialog(@StringRes int title, @StringRes int message) {
        final EditText input = new EditText(context);
        input.setInputType(InputType.TYPE_CLASS_NUMBER);
        input.setHint(R.string.service_mode_pin_hint);
        ...
        new AlertDialog.Builder(context)
                ...
                .setPositiveButton(android.R.string.ok, (dialog, which) -> {
                    ...
                    if (validation == PinStorage.OK && pinStorage.save(pin)) {
                        Toast.makeText(context, R.string.service_mode_pin_set_successfully, Toast.LENGTH_SHORT).show();
                        onPinChanged.run();
                    } else if (validation == PinStorage.STARTS_WITH_ZERO) {
                        Toast.makeText(context, R.string.service_mode_pin_no_leading_zero, Toast.LENGTH_LONG).show();
                    } else {
                        Toast.makeText(context, R.string.service_mode_pin_too_short, Toast.LENGTH_LONG).show();
                    }
                })
