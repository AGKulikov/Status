...
final class PermissionGate {
     ...
     */
    void requestMissing() {
        activity.getPreferences(android.content.Context.MODE_PRIVATE).edit().putBoolean("asked_phone", true).apply();
        ...
        for (int result : grantResults) {
            if (result != PackageManager.PERMISSION_GRANTED) {
                new AlertDialog.Builder(activity)
                        .setTitle(R.string.service_mode_permissions_required_title)
                        .setMessage(R.string.service_mode_permissions_required_message)
    ...
    void showRequiredToHide() {
        if (canAskAgain()) {
            new AlertDialog.Builder(activity)
                    .setTitle(R.string.service_mode_permissions_required_title)
                    .setMessage(R.string.service_mode_permissions_required_to_hide_message)
                    .setPositiveButton(R.string.service_mode_grant_permissions, (dialog, which) -> requestMissing())
                    .setNegativeButton(android.R.string.cancel, null)
                    .show();
        } else {
            new AlertDialog.Builder(activity)
                    .setTitle(R.string.service_mode_permissions_required_title)
                    .setMessage(R.string.service_mode_permissions_open_settings_message)
                    .setPositiveButton(R.string.service_mode_open_settings, (dialog, which) -> openAppSettings())
                    .setNegativeButton(android.R.string.cancel, null)
                    .show();
        }
    }
    ...
    private boolean canAskAgain() {
        if (!activity.getPreferences(android.content.Context.MODE_PRIVATE).getBoolean("asked_phone", false)) return true;
        for (String perm : REQUIRED) {
            if (ContextCompat.checkSelfPermission(activity, perm) != PackageManager.PERMISSION_GRANTED
