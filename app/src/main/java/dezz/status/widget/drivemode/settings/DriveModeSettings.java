...
public final class DriveModeSettings {
    ...
    public boolean isEnabled() { return prefs.getBoolean("natro_observe_enabled", false); }
    public void setEnabled(boolean enabled) { prefs.edit().putBoolean("natro_observe_enabled", enabled).apply(); }
    ...
    public int getAutoHidePreviewMs() {
        return clampAutoHide(prefs.getInt(PreferenceKeys.KEY_AUTO_HIDE_PREVIEW_MS,
    ...
    public int getAutoHideSwitchMs() {
        return clampAutoHide(prefs.getInt(PreferenceKeys.KEY_AUTO_HIDE_SWITCH_MS,
    ...
    private List<ModeOrderEntry> defaultOrder() {
        ...
        for (int code : DriveModeCatalog.defaultOrder()) defaults.add(code);
        for (DriveModeDescriptor d : DriveModeCatalog.all()) {
            list.add(new ModeOrderEntry(d.code, defaults.contains(d.code)));
        }
        return list;
    }
}
