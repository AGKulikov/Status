...
public final class InformationPanelSettingsActivity extends AppCompatActivity {
    ...
    private void editItem(@NonNull InformationPanelConfig.Item item) {
        ...
        String[] valueColor = {item.valueColor};
        String[] labelColor = {item.labelColor};
        String[] iconColor = {item.iconColor};
        addColorField(form, "Цвет значения", valueColor);
        addColorField(form, "Цвет подписи", labelColor);
        addColorField(form, "Цвет иконки", iconColor);
        ...
        AppleColorPickerDialog.show(this, "Цвет фона панели", original,
                ...
                new AppleColorPickerDialog.Listener() {
                    private void apply(@Nullable String selected) {
                        config.backgroundColor = selected == null ? original : selected;
                        changed(false);
                        AppleColorPickerDialog.decorateButton(button, "Цвет фона",
                                config.backgroundColor);
                    }

                    @Override
                    public void onPreview(@Nullable String selected) {
                        apply(selected);
                    }

                    @Override
                    public void onSelected(@Nullable String selected) {
                        apply(selected);
                    }
                });
    }
    ...
    private void addColorField(@NonNull LinearLayout parent, @NonNull String title,
            ...
            AppleColorPickerDialog.show(this, title, original,
                    ...
                    new AppleColorPickerDialog.Listener() {
                        private void apply(@Nullable String selected) {
                            ...
                            current[0] = "transparent".equalsIgnoreCase(value)
                                    ? "#00000000" : value;
                            AppleColorPickerDialog.decorateButton(button, title, current[0]);
                        }
        ...
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(match(), dp(62));
