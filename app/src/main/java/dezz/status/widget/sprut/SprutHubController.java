...
public final class SprutHubController {
    ...
    public CompletableFuture<JSONObject> execute(@NonNull ActionBinding binding) {
        ...
        final Object value;
        try {
            value = actionValue(binding, characteristic);
        } catch (JSONException | IllegalArgumentException e) {
            CompletableFuture<JSONObject> failed = new CompletableFuture<>();
            failed.completeExceptionally(e);
            return failed;
        }
        boolean modern = hubRevision > 12_500;
        JSONObject params = modern
                ? SprutProtocolAdapter.buildCharacteristicUpdateParams(characteristic, value)
                : buildLegacyUpdate(characteristic, value);
        CompletableFuture<JSONObject> request = current.call(params, 5_000L);
    ...
    private static Object actionValue(ActionBinding binding,
        ...
        if (ActionBinding.OPERATION_TOGGLE.equals(binding.operation)) {
            ...
            if (current instanceof Number) {
                Number minimum = characteristic.minValue();
                Number maximum = characteristic.maxValue();
                if (minimum != null && maximum != null) {
                    double value = ((Number) current).doubleValue();
                    return Double.compare(value, minimum.doubleValue()) == 0
                            ? maximum : minimum;
                }
                return ((Number) current).doubleValue() == 0d ? 1 : 0;
            }
            throw new IllegalArgumentException("Cannot toggle a non-boolean value");
        }
        String raw = binding.payload.trim();
        ...
        if (decoded instanceof JSONObject) {
            ...
            if (object.has("value")) decoded = object.get("value");
        }
        if (decoded == JSONObject.NULL) throw new IllegalArgumentException("Action value is null");
        return decoded;
    }

    private static JSONObject buildLegacyUpdate(SprutCatalog.Characteristic characteristic,
                                                Object value) {
        JSONObject modern = SprutProtocolAdapter.buildCharacteristicUpdateParams(characteristic,
                value);
        JSONObject characteristicNode = modern.optJSONObject("characteristic");
        ...
        if (update != null) {
            ...
            if (control != null) {
                try {
                    update.put("value", control.opt("value"));
