...
final class EcarxSignalFallback {
    ...
    /** ECARX global property area reported by the KX11 vendor service. */
    private static final int ECARX_GLOBAL_AREA = 1;
    ...
    /** Confirmed byte-array property IDs read through getBytesProperty(id, area). */
    private final Set<Integer> binaryRecorderPropertyIds = new LinkedHashSet<>();
    ...
    private void scanPropertyIds(@Nullable Object manager) {
        ...
        binaryRecorderPropertyIds.clear();
        for (int propertyId : EcarxAdasSignalCatalog.discoveryFallbackPropertyIds()) {
            recorderDiscoveryIds.add(propertyId);
        }
        for (int propertyId : EcarxAdasSignalCatalog.binaryDiscoveryPropertyIds()) {
            recorderDiscoveryIds.add(propertyId);
            binaryRecorderPropertyIds.add(propertyId);
        }
        propertyNames.clear();
        ...
        Log.d(TAG, "Discovered manualMode=" + manualModeIds + ", highBeam=" + highBeamIds
                ...
                + ", typedBytesPropertyIds=" + binaryRecorderPropertyIds);
    }
    ...
    private boolean registerCallback(Object manager) throws Exception {
        ...
        if (adasRecorderDemand) {
            ...
            ids.addAll(binaryRecorderPropertyIds);
            recorderIds.addAll(binaryRecorderPropertyIds);
            ...
            for (Integer propertyId : recorderDiscoveryIds) {
                if (requiredIds.contains(propertyId)
                        ...
                        || binaryRecorderPropertyIds.contains(propertyId)
    ...
    /**
     * Reads confirmed aggregates through the vendor byte accessor, then any remaining generated
     * byte-array getters. Calling the generic integer accessor for these IDs produced the repeated
     * Integer-vs-byte[] exceptions seen in the KX11 trace.
     */
    @NonNull
    private ReadResult readBinaryCurrentValues(@NonNull Object manager) {
        boolean readerAvailable = false;
        boolean succeeded = false;
        Method bytesReader = findTwoIntMethod(manager.getClass(), "getBytesProperty");
        if (bytesReader != null) {
            readerAvailable = true;
            for (Integer propertyId : binaryRecorderPropertyIds) {
                try {
                    Object value = bytesReader.invoke(manager, propertyId, ECARX_GLOBAL_AREA);
                    succeeded = true;
                    byte[] binary = EcarxSignalDecoder.coerceByteArray(value);
                    if (binary != null) handleEvent(binary, propertyId);
                } catch (ReflectiveOperationException | RuntimeException ignored) {
                    // One unavailable aggregate must not tear down the working callback.
                }
            }
        }
        for (Map.Entry<Integer, String> entry : binaryRecorderGetterNames.entrySet()) {
            if (binaryRecorderPropertyIds.contains(entry.getKey())) continue;
            ...
            try {
                ...
                if (binary != null) {
                    succeeded = true;
                    handleEvent(binary, entry.getKey());
                }
            } catch (ReflectiveOperationException | RuntimeException ignored) {
    ...
    @Nullable
    private static Method findTwoIntMethod(Class<?> type, String name) {
        for (Method method : type.getMethods()) {
            Class<?>[] parameters = method.getParameterTypes();
            if (!method.getName().equals(name) || parameters.length != 2) continue;
            boolean firstInt = parameters[0] == Integer.TYPE || parameters[0] == Integer.class;
            boolean secondInt = parameters[1] == Integer.TYPE || parameters[1] == Integer.class;
            if (firstInt && secondInt) return method;
        }
        return null;
    }
