...
/// Helper v13 exposes two mutually exclusive BLE routes. Peripheral is the v11 production route
...
final class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        append("v13: выбран iPhone \(role.title); Peripheral сохраняет маршрут v11")
    ...
    private func buildInterface() {
        ...
        titleLabel.text = "KX11 ANCS HELPER v13"
    ...
    private func scheduleCentralReconnect(reason: String) {
        ...
        append("Central reconnect через 1 с · \(reason)")
...
extension ViewController: CBPeripheralDelegate {
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        guard runRequested, role == .central,
              peripheral.identifier == geelyPeripheral?.identifier else { return }
        if let error = error {
            append("Discover services error: \(error.localizedDescription)")
            resetCentralLink(reason: "service discovery failed · \(error.localizedDescription)")
            return
        }
        ...
        peripheral.discoverCharacteristics(
            [controlUUID, secureUUID, telemetryUUID], for: service)
    }
    ...
    func peripheral(_ peripheral: CBPeripheral,
        ...
        guard runRequested, role == .central,
              peripheral.identifier == geelyPeripheral?.identifier,
              service.uuid == serviceUUID else { return }
        if let error = error {
            append("Discover characteristics error: \(error.localizedDescription)")
    ...
    func peripheral(_ peripheral: CBPeripheral, didWriteValueFor characteristic: CBCharacteristic,
        ...
        if characteristic.uuid == controlUUID {
            if let error = error {
                append("WRITE PAIR error: \(error.localizedDescription)")
                resetCentralLink(reason: "PAIR write failed · \(error.localizedDescription)")
            } else {
        ...
        if characteristic.uuid == telemetryUUID {
            centralTelemetryWriteInFlight = false
            if let error = error {
                append("B4 telemetry WRITE error: \(error.localizedDescription)")
