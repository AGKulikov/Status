# KX11 ANCS Helper v18 for iPhone

Helper v18 accompanies Status Widget HA1175 and keeps the explicit iPhone BLE-role selector. The
...
- **Central** is opt-in: KX11 advertises `Geely_ANCS`; Helper scans for its private D2D9 service and
  connects with `CBConnectPeripheralOptionRequiresANCS`. KX11 then attaches its ANCS GATT client
  to the same incoming BLE peer.
...
The private Helper telemetry service is generation 2 in both roles. This is a UUID migration, not
a role or pairing change: install Helper v18 and HA1175 together. It prevents either Core Bluetooth
side from attaching the old generation-1 B4 handles to the updated characteristic schema.

## Central-mode current-link handshake

1. Helper scans only for GATT-generation-2 service
   `D2D9E4B0-47F1-4E44-A8BB-A932FD5A2F02`. The new UUID prevents iOS from reusing the stale
   generation-1 handles left by earlier B4 schema changes.
...
4. The first B3 read on a new physical link receives one ATT authentication challenge from HA1175;
...
sequence and CRC-8/ATM). Peripheral mode still serves it through READ/NOTIFY. In Central mode KX11
subscribes Helper to B4 after current-link confirmation and sends a five-second notification while
the link is active. This Core Bluetooth event wakes a suspended Helper; Helper then re-reads UIKit
and a fresh CoreTelephony snapshot and writes the current B4 frame with response. The ordinary
one-second timer remains only a foreground safety sampler and is no longer the background clock.

Helper v18 first accepts a complete characteristic list already attached to the current
`CBService`, then discovers only missing B2/B3/B4 entries. A bounded 15-second watchdog abandons a
saved/restored peripheral that remains stuck in `connecting` and waits for a fresh filtered
advertisement.

Helper v18 also treats a removed D2D9 service or invalid ATT handle as a broken BLE session. It
disconnects, drops all cached services/characteristics, and waits for a fresh advertisement.
Unlike v17 it never asks Core Bluetooth for system auto-reconnect: every failure, normal
disconnect and restored `isReconnecting` callback is converted into one serialized, filtered
fresh-advertisement recovery. This removes the race that reused invalid B2/B3/B4 handles while
HA1175 was handing the encrypted peer from Android's GATT-server role to its ANCS client. Repeated
failures remain bounded by the 1/2/5/10/20/30-second backoff.
