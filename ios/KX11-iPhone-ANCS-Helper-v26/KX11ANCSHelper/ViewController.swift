...
/// Helper v26 uses two explicit Central links. A plain BLE bootstrap discovers F04 and proves the
...
/// post-handoff telemetry path. Classic Bluetooth is never modified here.
final class ViewController: UIViewController {
    ...
    private let restoreIdentifier =
        "ru.natro.kx11ancshelper.peripheral.v26.single-link-g5"
    private let centralRestoreIdentifier =
        "ru.natro.kx11ancshelper.central.v26.geely-ancs-g4"
    ...
    override func viewDidLoad() {
        ...
        append("v26: выбран iPhone \(role.title); Peripheral сохраняет маршрут v11")
    ...
    private func buildInterface() {
        ...
        titleLabel.text = "KX11 ANCS HELPER v26"
    ...
    private func continueCentralConnected(_ peripheral: CBPeripheral) {
        ...
        if centralLinkPhase == .requiresAncs {
            ...
            setStatus("REQUIRES_ANCS LINK · ЖДУ B4", color: .systemOrange)
            waitForCentralTelemetryRelay("RequiresANCS relay timeout")
    ...
    private func connectCentral(_ peripheral: CBPeripheral, reason: String) {
        ...
        // deterministic: Helper v26 always resolves the next generation from a fresh advert.
    ...
    private func waitForCentralTelemetryRelay(_ reason: String,
        ...
        let item = DispatchWorkItem { [weak self] in
            ...
            self.append("B4 relay не подтвердил рабочий ANCS link; возвращаю Central bootstrap")
            self.resetCentralLink(reason: "fresh bootstrap after \(reason)")
        }
    ...
    private func updateConnectionStatus() {
        ...
        if role == .central {
            if centralHandoffActive && centralRelayProved && telemetrySubscribers.isEmpty {
                setStatus("ANCS OWNER АКТИВЕН · ЖДУ B4", color: .systemOrange)
            } else if centralHandoffActive && centralRelayProved {
                setStatus("ANCS + ТЕЛЕМЕТРИЯ АКТИВНЫ", color: .systemGreen)
            } else if centralHandoffActive {
                setStatus("ANCS HANDOFF · ЖДУ B4 RELAY", color: .systemOrange)
    ...
    private func handleCentralDisconnect(_ peripheral: CBPeripheral, isReconnecting: Bool,
        ...
        if completedSecureHandoff && hardReset == nil {
            ...
            if centralRelayProved || !telemetrySubscribers.isEmpty {
                ...
                setStatus("ANCS HANDOFF · ЖДУ B4 RELAY", color: .systemOrange)
                waitForCentralTelemetryRelay("handoff relay timeout")
            }
            return
        }
        if isReconnecting {
            // Handles a restored connection originally created by an older Helper. New v26
