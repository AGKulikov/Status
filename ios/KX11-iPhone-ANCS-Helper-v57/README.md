# KX11 ANCS Helper v57 — Bluetooth vehicle control

Minimum supported iOS: 14.0. Helper v57 uses the system CryptoKit HKDF implementation for the
enrollment protocol; devices left on iOS 13.x cannot install this build.

Helper v57 pairs with the Natro C4-enrollment runtime and keeps Route A as the production
topology:
...
v57 does not weaken identity to a BLE name, MAC/address equality, nearest RSSI or manufacturer
...
Without that explicit developer flag, Route B selection is rejected. v57 never runs both roles
...
An in-place v51 → v57 update keeps bundle ID `ru.natro.kx11ancshelper`, both stable Core Bluetooth
...
v57 removes the one-second telemetry timer. Battery, power, path and lock updates are event-driven.
...
Helper 57 embeds `NatroLiveActivityExtension` (minimum iOS 16.2) while the main Bluetooth app keeps
...
Helper 57 retains C5, `D2D9E4C5-47F1-4E44-A8BB-A932FD5AF200`, without changing the occupied C2 role
...
- iOS controls background advertising contents and scheduling. v57 does not depend on a name.
