# KX11 ANCS Helper v62 — Bluetooth vehicle control

Minimum supported iOS: 14.0. Helper v62 uses the system CryptoKit HKDF implementation for the
enrollment protocol; devices left on iOS 13.x cannot install this build.

Helper v62 pairs with the Natro C4-enrollment runtime and keeps Route A as the production
topology:
...
v62 does not weaken identity to a BLE name, MAC/address equality, nearest RSSI or manufacturer
...
Without that explicit developer flag, Route B selection is rejected. v62 never runs both roles
...
An in-place v51 → v62 update keeps bundle ID `ru.natro.kx11ancshelper`, both stable Core Bluetooth
...
v62 removes the one-second telemetry timer. Battery, power, path and lock updates are event-driven.
...
Helper 62 embeds `NatroLiveActivityExtension` (minimum iOS 16.2) while the main Bluetooth app keeps
...
Local `Activity.request` is permitted by iOS only while Natro is foreground-active. Natro starts
both cards automatically when C5 becomes ready while the app is active, or on the next foreground
activation if the magnetic head unit connected while the phone was locked. An existing card can
receive Core Bluetooth updates and can be ended from the BLE background execution window. After
force-quit, expiry, or a background relaunch with no existing activity, a fully background-first
start requires an APNs push-to-start provider and cannot be implemented by a local Bluetooth
callback alone. Settings state this boundary instead of claiming an impossible local start.
...
Helper 62 retains C5, `D2D9E4C5-47F1-4E44-A8BB-A932FD5AF200`, without changing the occupied C2 role
...
- iOS controls background advertising contents and scheduling. v62 does not depend on a name.
