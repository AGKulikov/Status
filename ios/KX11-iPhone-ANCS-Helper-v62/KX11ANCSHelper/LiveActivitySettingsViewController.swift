...
final class LiveActivitySettingsViewController: UITableViewController {
    ...
    override func tableView(
        ...
        case .functionControls: return 4
    ...
    override func tableView(
        ...
        switch Section(rawValue: section)! {
        case .behavior:
            return "В обычном режиме карточки запускаются только после полной C5-синхронизации с магнитолой и сразу завершаются при потере Bluetooth-маршрута. Демо остаётся явным исключением для работы без автомобиля и не подменяет реальный статус ANCS. Первый локальный запуск iOS разрешает только в foreground; затем карточки обновляются от BLE в фоне."
        case .climateControls:
            return "Четыре быстрые функции под изображением автомобиля и температурой."
        case .functionControls:
            return "Четыре избранные функции крупными кнопками. Полный пульт находится на вкладке «Автомобиль». Уровни подогрева и вентиляции показаны тремя сегментами; Auto отмечается буквой A."
        case .shortcuts:
