...
/// Helper v35 keeps one Central owner, recycles a stale pending connection, and gates
...
final class ViewController: UIViewController {
    ...
    // These fixed F04 UUIDs remain for the legacy iPhone-peripheral route. In Central mode v35
    ...
    /// exact v35 owner. A system-wide connection discovered through another app is never adopted
    ...
    override func viewDidLoad() {
        ...
        append("v35: выбран iPhone \(role.title); Peripheral сохраняет маршрут v11")
        ...
        append("Отвергнутый namespace не принимается повторно; жду новую Android generation")
    ...
    private func buildInterface() {
        ...
        titleLabel.text = "KX11 ANCS HELPER v35"
    ...
    private func connectCentral(_ peripheral: CBPeripheral, reason: String) {
        ...
        if peripheral.state == .connected {
            guard centralOwnerConfiguredForAncs else {
                ...
                cancelCentralConnectionSafely(peripheral, manager: centralManager,
                                                reason: "already-connected foreign owner")
                return
            }
            continueCentralConnected(peripheral)
            return
        ...
        if peripheral.state == .connecting {
            guard centralOwnerConfiguredForAncs else {
                ...
                append("Не принимаю неизвестный .connecting owner без v35 restoration; "
                    + "следующий connect сразу получит RequiresANCS")
                centralSystemAutoReconnectActive = false
                cancelCentralConnectionSafely(peripheral, manager: centralManager,
                                                reason: "unknown pending owner")
                return
            }
            armCentralConnectTimeout(peripheral)
...
extension ViewController: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral,
        ...
        if centralRequireFreshAdvertisement,
           generation == centralRejectedNamespaceGeneration {
            if Date().timeIntervalSince(centralRejectedNamespaceLogAt) >= 10 {
                centralRejectedNamespaceLogAt = Date()
                append("Игнорирую отвергнутый namespace "
                    + String(format: "%04X", Int(generation))
                    + "; жду новую Android generation")
            }
            setStatus("CENTRAL · ЖДУ НОВЫЙ NAMESPACE", color: .systemOrange)
            return
        }
        centralRejectedNamespaceGeneration = nil
        applyCentralNamespace(generation)
