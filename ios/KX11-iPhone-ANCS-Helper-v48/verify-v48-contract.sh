...
[ "$(grep -Fc 'CURRENT_PROJECT_VERSION = 48;' "$PROJECT")" -eq 2 ]
[ "$(grep -Fc 'MARKETING_VERSION = 48.0;' "$PROJECT")" -eq 2 ]
...
grep -Fq 'Status: source/replay accepted; hosted Xcode build and physical-device matrix remain release gates' \
    "$ROOT/RELEASE.txt"
grep -Fq 'ios/KX11-iPhone-ANCS-Helper-v48/**' \
    "$REPO_ROOT/.github/workflows/verify-helper-v48.yml"

echo 'PASS: Helper v48 clean-room source, shared vectors, restoration drains, and app contract'
