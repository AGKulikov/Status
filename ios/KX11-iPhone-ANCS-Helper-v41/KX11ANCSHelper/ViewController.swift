...
/// Helper v41 keeps one long-lived RequiresANCS Central owner and performs one minimal security
...
final class ViewController: UIViewController {
    ...
    /// True after this exact ordinary lineage consumed its data intent and actually submitted
    /// `connect`. Observation must survive that consumption because the missing callback happens
    /// after Core Bluetooth owns the request.
    ...
    private var centralDeferredReclaimToken: UInt64 = 0
    ...
    override func viewDidLoad() {
        ...
        append("v41: hot-update .connecting recovery по exact F04 proof; one claim")
    ...
    private func buildInterface() {
        ...
        titleLabel.text = "KX11 ANCS HELPER v41"
    ...
    private func beginCentralDeferredReclaimLineage(_ peripheral: CBPeripheral,
                                                     reason: String) {
        if centralDeferredReclaimActive,
           centralDeferredReclaimOwnerID == peripheral.identifier {
            return
        }
        centralDeferredReclaimToken &+= 1
        ...
        append("Ordinary exact-owner reclaim lineage armed · owner="
            + peripheral.identifier.uuidString + " · budget=1 · \(reason)")
    }
    ...
    /// The beacon observer and the system-table observer run in parallel. Beacon presence is an
    /// exact-identifier liveness hint and accelerates the table probe; only the exact F04 entry in
    /// `retrieveConnectedPeripherals` is physical-link proof that may arm the bounded cancel.
    private func armCentralDeferredReclaimObservation(_ peripheral: CBPeripheral) {
        ...
        if !centralDeferredReclaimWaitLogged {
            ...
            append("Deferred .connecting owner: parallel exact F04 beacon + system-table proof "
                + "armed; elapsed time cannot reclaim")
        }
        if !centralManager.isScanning {
            centralManager.scanForPeripherals(
                withServices: [managedIncomingBeaconUUID],
        ...
        let token = centralDeferredReclaimToken
        let item = DispatchWorkItem { [weak self, weak peripheral] in
            guard let self = self, let peripheral = peripheral else { return }
            self.centralDeferredReclaimProbeWorkItem = nil
            guard token == self.centralDeferredReclaimToken,
                  self.runRequested, self.role == .central,
    ...
    /// Returns true when an exact ordinary-reclaim beacon was consumed as read-only evidence.
    private func observeCentralDeferredReclaimBeacon(_ peripheral: CBPeripheral,
                                                      rssi: NSNumber) -> Bool {
        guard centralDeferredReclaimActive,
              !centralDeferredReclaimConsumed,
              !centralDeferredReclaimPendingTerminal,
              ...
              centralDeferredReclaimOwnsPendingRequest(owner) else { return false }
        if !centralDeferredReclaimBeaconObserved {
            centralDeferredReclaimBeaconObserved = true
            append("Exact saved-owner F04 beacon observed during deferred .connecting · RSSI="
                + "\(rssi); system-table proof remains required")
        }
        centralDeferredReclaimProbeWorkItem?.cancel()
        centralDeferredReclaimProbeWorkItem = nil
        centralDeferredReclaimProbeAttempt = 0
    ...
    private func observeCentralDeferredReclaimSystemProof(_ peripheral: CBPeripheral,
        ...
        let item = DispatchWorkItem { [weak self, weak peripheral] in
            ...
            guard token == self.centralDeferredReclaimToken,
                  ...
                  !self.centralDeferredReclaimConsumed,
                  !self.centralDeferredReclaimPendingTerminal,
                  self.centralDeferredReclaimSystemProofObserved,
                  self.centralDeferredReclaimOwnerID == peripheral.identifier,
                  self.centralDeferredReclaimOwnsPendingRequest(peripheral) else { return }
            if peripheral.state == .disconnected {
                self.startCentralRouteIfPossible()
                return
            }
            ...
            self.append("Exact F04 proof grace expired without didConnect; one bounded "
                + "ordinary reclaim cancel 1/1")
    ...
    private func reopenCentralDeferredReclaimAfterTerminal(_ peripheral: CBPeripheral,
        ...
        clearCentralRuntime(keepPeripheral: true)
        append("Ordinary reclaim terminal observed · \(callback); reopen same owner with "
            + "RequiresANCS, claim remains spent")
        guard runRequested, role == .central else { return }
        _ = queueCentralConnectIntent(
            peripheral, reason: "hot-update exact-owner reclaim 1/1", delay: 0.25)
    }
    ...
    private func consumeCentralDeferredConnectIfPossible() -> Bool {
        ...
        if centralDeferredReclaimActive,
           centralDeferredReclaimOwnerID == intent.peripheral.identifier {
            // CONTRACT_V41_CONSUMED_INTENT_CONNECT_STAYS_OBSERVED: the data intent is gone before
            // `connectCentral`, but this exact issued-request bit keeps evidence recovery live if
            // Core Bluetooth changes to `.connecting` and omits didConnect.
    ...
    private func queueTerminalCentralReconnect(_ peripheral: CBPeripheral, reason: String) {
        ...
        if !centralDeferredReclaimActive {
            beginCentralDeferredReclaimLineage(peripheral, reason: reason)
        }
        centralDeferredReclaimIssuedConnectPending = false
        let delayIndex = min(centralReconnectFailureCount,
                             centralReconnectDelays.count - 1)
...
extension ViewController: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral,
        ...
        if observeCentralDeferredReclaimBeacon(peripheral, rssi: RSSI) {
            return
        }
    ...
    func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        ...
        if centralDeferredReclaimActive,
           centralDeferredReclaimOwnerID == peripheral.identifier {
            ...
            // cancels any proof grace before accepting the exact current owner.
            append("Matching late exact didConnect cancelled ordinary F04 reclaim grace")
            clearCentralDeferredReclaimLineage()
        }
        if let intent = centralDeferredConnectIntent,
