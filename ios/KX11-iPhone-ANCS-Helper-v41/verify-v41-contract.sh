...
require_source() {
    grep -Fq "$1" "$SOURCE" || {
        echo "missing v41 contract marker: $1" >&2
        exit 1
    }
}
...
[ "$(grep -Fc 'CURRENT_PROJECT_VERSION = 41;' "$PROJECT")" -eq 2 ]
[ "$(grep -Fc 'MARKETING_VERSION = 41.0;' "$PROJECT")" -eq 2 ]
...
# v41 extends the safe restoration proof model to an ordinary exact-owner reconnect after a hot
# Android package update. The `.connecting` branch may only arm read-only observers; the exact
# system-table proof function owns one delayed cancel and no direct reopen.
...
require_source "CONTRACT_V41_CONSUMED_INTENT_CONNECT_STAYS_OBSERVED"
...
ISSUED_PENDING_LINE=$(printf '%s\n' "$CONSUME_BLOCK" \
    | grep -n "centralDeferredReclaimIssuedConnectPending = true" | head -n1 | cut -d: -f1)
CONSUMED_CONNECT_LINE=$(printf '%s\n' "$CONSUME_BLOCK" \
    | grep -n "connectCentral(intent.peripheral," | head -n1 | cut -d: -f1)
[ "$ISSUED_PENDING_LINE" -lt "$CONSUMED_CONNECT_LINE" ]
printf '%s\n' "$CONSUME_BLOCK" \
    | grep -Fq "armCentralDeferredReclaimObservation(intent.peripheral)"
...
printf '%s\n' "$RECLAIM_PROOF" \
    | grep -Fq "centralDeferredReclaimConsumed = true"
printf '%s\n' "$RECLAIM_PROOF" \
    | grep -Fq "centralDeferredReclaimProofGrace"
if printf '%s\n' "$RECLAIM_PROOF" | grep -Eq "queueCentralConnectIntent|issueCentralConnect"; then
    echo "ordinary reclaim proof bypasses terminal ownership" >&2
    exit 1
fi
...
printf '%s\n' "$POWER_DISCONNECTED_RECLAIM" \
    | grep -Fq "centralDeferredReclaimIssuedConnectPending = false"
printf '%s\n' "$POWER_DISCONNECTED_RECLAIM" \
    | grep -Fq "queueCentralConnectIntent("
printf '%s\n' "$POWER_DISCONNECTED_RECLAIM" \
    | grep -Fq "startCentralRouteIfPossible()"
POWER_CLEAR_ISSUED_LINE=$(printf '%s\n' "$POWER_DISCONNECTED_RECLAIM" \
    | grep -n "centralDeferredReclaimIssuedConnectPending = false" | cut -d: -f1)
...
# Executable field sequence: GREEN -> exact invalidation -> hot Android disconnect -> retained
# intent becomes `.connecting` -> exact F04 physical proof -> one cancel -> one terminal reopen ->
# matching didConnect. No elapsed-only tick or later terminal may produce a second claim.
green=1
f04_exact_object=current
...
did_connect=0
[ "$green" -eq 1 ]
f04_exact_object=invalidated       # exact Service Changed
green=0
deferred_intent=1                  # terminal Android package-update disconnect
owner_state=disconnected
deferred_intent=0                  # intent is consumed before connectCentral
issued_connect_pending=1           # exact lineage survives data-intent consumption
owner_state=connecting             # connect submitted, app-local didConnect omitted
elapsed_ticks=300
: "$elapsed_ticks"                # elapsed time is deliberately inert
[ "$ordinary_cancels" -eq 0 ]
beacon_exact_id=1
retrieve_exact_f04=1
physical_proof=$((beacon_exact_id && retrieve_exact_f04))
[ "$physical_proof" -eq 1 ]
if [ "$ordinary_claim_budget" -eq 1 ]; then
    ordinary_claim_budget=0
    ordinary_cancels=$((ordinary_cancels + 1))
    pending_terminal=1
fi
[ "$ordinary_cancels" -eq 1 ]
pending_terminal=0                 # didDisconnect/didFail/.disconnected observer
ordinary_reopens=$((ordinary_reopens + 1))
[ "$ordinary_reopens" -eq 1 ]
owner_state=connecting
did_connect=1                      # late exact callback atomically consumes intent/reclaim
deferred_intent=0
issued_connect_pending=0
[ "$did_connect" -eq 1 ]
[ "$deferred_intent" -eq 0 ]
[ "$issued_connect_pending" -eq 0 ]
[ "$ordinary_claim_budget" -eq 0 ]
# Another exact proof/terminal in the same lineage cannot cancel or reopen again.
if [ "$ordinary_claim_budget" -eq 1 ]; then
    ordinary_cancels=$((ordinary_cancels + 1))
fi
[ "$ordinary_cancels" -eq 1 ]
...
# If Bluetooth goes off after the exact request was issued, old proof is discarded but the exact
# lineage and its unchanged one-shot budget remain owned. A disconnected owner on poweredOn
# becomes one data intent, which the normal route consumes without a cancel or a new claim.
radio_claim_budget=1
radio_issued_pending=1
radio_deferred_intent=0
radio_connect_submissions=1       # request issued before poweredOff
radio_cancels=0
radio_old_physical_proof=1
radio_powered_on=0
radio_old_physical_proof=0        # poweredOff invalidates proof/grace
[ "$radio_issued_pending" -eq 1 ] # owner/lineage preserved
...
if [ "$radio_deferred_intent" -eq 1 ]; then
    radio_deferred_intent=0
    radio_issued_pending=1
...
echo "Helper v41 hot-update/restoration/ANCS recovery contract passed"
