# KX11 ANCS Helper v41 for iPhone

Helper v41 is the matched companion for Status Widget `v2.8.2-ha1206`. For normal daily use,
...
- A terminal callback with `isReconnecting == false` immediately stores one exact-owner deferred
  ...
  consumes that intent, and is accepted. If a hot Status Widget update leaves the owner in
  `.connecting` without `didConnect`, Helper observes the exact saved-identifier F04 beacon and
  `retrieveConnectedPeripherals(withServices: [F04])` in parallel. Only an exact system-table
  physical-link proof arms a short callback grace and at most one
  `cancel -> terminal/.disconnected -> same-owner RequiresANCS connect` claim. Elapsed time cannot
  claim ownership, and a failed replacement cannot obtain a second claim from another terminal
  callback. Probe and wait logs are emitted only on state transitions. When
  `isReconnecting == true`, iOS 17 AutoReconnect remains the sole owner and Helper issues no
  competing command.
...
Run `sh ./verify-v41-contract.sh` before packaging.
