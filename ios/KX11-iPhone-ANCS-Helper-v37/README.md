# KX11 ANCS Helper v37 for iPhone

Helper v37 is the matched companion for Status Widget `v2.8.2-ha1200`. Select **Central** in
...
- After `didConnect`, Helper discovers only Android's permanent F04 service, writes `PAIR`, and
  reads encrypted B3 on that same owner. Android attaches its reverse ANCS GATT client only after
  B3 succeeds and Helper writes `ANCS-READY` with real iOS authorization.
- There is no dynamic namespace, UUID quarantine or pending-connect watchdog.
