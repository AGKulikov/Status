...
public final class HelperSwitchRuntimeCoordinator {
    ...
    private let queue = DispatchQueue(label: "ru.natro.kx11ancshelper.v47.switch-owner")
    ...
    public init(defaults: UserDefaults = .standard) throws {
        ...
        } else if let legacyRole = try Self.loadLegacyRole(defaults: defaults) {
            // v46 used the same Core Bluetooth restoration identifiers. Absence of BRS2 is not a
            // fresh-install proof when its role preference exists: durably enter a local-only
            // same-role drain before either restoration namespace is allowed to resume.
            var migrated = BleRoleSwitchPolicy(activeRole: legacyRole)
            let migration = migrated.requestSameRoleRestart(
                nowMs: Self.nowMs(),
                stopTimeoutMs: Self.stopTimeoutMs,
                drainDurationMs: Self.drainDurationMs
            )
            let token = Self.randomToken()
            self.policy = migrated
            self.origin = .legacyMigration
            self.switchToken = token
            self.pendingStartupEffects = migration.effects
            self.bootDetail = "Миграция v46: выбранный namespace сначала полностью дочищается"
            try Self.persist(
                policy: migrated,
    ...
    public func publishTelemetry(_ sample: HelperTelemetrySample) {
        queue.async { [weak self] in
            guard let self, self.runtimeFailure == nil, self.routeReady,
    ...
    private func handle(_ event: HelperBleRuntimeCoordinator.Event) {
        ...
        case .routeBUnprovableRestoration(let generation):
            guard generation == policy.state.sourceGeneration,
                  policy.state.sourceRole == .helperCentralAndroidPeripheral else { return }
            if origin == .legacyMigration { return }
            failRuntime("Восстановленный Route B не прошёл installation-ID/F202 proof")
    ...
    private func handleControl(
        ...
        guard frame.type == .roleClose || frame.type == .roleCloseAck else { return }
    ...
    private func dispatch(_ effect: BleRoleSwitchPolicy.Effect) {
        ...
        case .targetActive:
            routeReady = true
            bootDetail = "Новый маршрут активен после terminal + owner-zero + drain"

        case .failClosed:
            runtimeFailure = "Fail-closed: \(policy.state.failure.rawValue)"
            routeReady = false
            if effect.role == .helperPeripheralAndroidCentral {
    ...
    private func publishSnapshot() {
        ...
        let value = Snapshot(desiredRole: desired, activeRole: active, phase: phase, detail: detail)
