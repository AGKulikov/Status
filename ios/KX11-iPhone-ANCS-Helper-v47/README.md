# KX11 ANCS Helper v47 — transport-v2 candidate

Helper v47 is a clean-room replacement for the previous dual-role BLE runtime. It keeps both
...
Use v47 only with the matching Android transport-v2 implementation from the same source
...
Route A advertises only its service UUID. v47 does not publish `iPhone_ANCS`, another local name,
...
An in-place v46 → v47 update keeps bundle ID `ru.natro.kx11ancshelper`, the stable Core Bluetooth
restoration identifiers and the version-independent v2 installation UUID. The first v47 launch
reads the v46 selected-role preference and performs a local-only same-role migration drain before
starting a fresh v47 generation. The normal update and reconnect path does **not** delete the system
...
stale/timeout/conflict/control-retry/restoration callback replays. It also audits the exact v47
file set, Xcode Sources membership, Info.plist modes and workflow packaging.
...
1. Install over v46 once from each previously selected role; keep the existing system pair.
...
- Bluetooth power loss, missing encrypted ownership, an absent callback, corrupt durable state or
  a mismatched Android build may leave the runtime fail-closed. That is intentional; v47 will not
