...
[ "$(grep -Fc 'CURRENT_PROJECT_VERSION = 47;' "$PROJECT")" -eq 2 ]
[ "$(grep -Fc 'MARKETING_VERSION = 47.0;' "$PROJECT")" -eq 2 ]
...
grep -Fq 'Status: source/replay accepted; hosted Xcode build and physical-device matrix remain release gates' \
    "$ROOT/RELEASE.txt"
grep -Fq 'ios/KX11-iPhone-ANCS-Helper-v47/**' \
    "$REPO_ROOT/.github/workflows/verify-helper-v47.yml"

echo 'PASS: Helper v47 clean-room source, shared vectors, restoration drains, and app contract'
