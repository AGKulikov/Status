...
python3 "$ROOT/verify_live_activity_v61.py"

PROJECT="$ROOT/KX11ANCSHelper.xcodeproj/project.pbxproj"
[ "$(grep -Fc 'CURRENT_PROJECT_VERSION = 61;' "$PROJECT")" -eq 4 ]
[ "$(grep -Fc 'MARKETING_VERSION = 61.0;' "$PROJECT")" -eq 4 ]
...
grep -Fq 'Build: 61' "$ROOT/RELEASE.txt"
grep -Fq 'Paired runtime: Natro 2.2.9' "$ROOT/RELEASE.txt"

echo 'PASS: Helper v61 coalesces C5 diagnostics and state-driven ActivityKit updates'
