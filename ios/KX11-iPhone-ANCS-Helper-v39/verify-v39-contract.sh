...
require_source() {
    grep -Fq "$1" "$SOURCE" || {
        echo "missing v39 contract marker: $1" >&2
        exit 1
    }
}
...
[ "$(grep -Fc 'CURRENT_PROJECT_VERSION = 39;' "$PROJECT")" -eq 2 ]
[ "$(grep -Fc 'MARKETING_VERSION = 39.0;' "$PROJECT")" -eq 2 ]
...
echo "Helper v39 restoration/ANCS contract passed"
