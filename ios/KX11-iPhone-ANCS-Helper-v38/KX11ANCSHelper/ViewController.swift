...
/// Helper v38 keeps one long-lived RequiresANCS Central owner and performs one minimal security
...
/// watchdog is used. A restored owner stuck in `.connecting` is rescued once only after the same
/// stable beacon proves that Android is physically present. The iPhone-owned F05 relay carries
/// telemetry and Android's post-CCCD proof.
final class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        append("v38: стабильный F04 owner + event-driven restore rescue")
    ...
    private func buildInterface() {
        ...
        titleLabel.text = "KX11 ANCS HELPER v38"
    ...
    private func continueCentralConnected(_ peripheral: CBPeripheral) {
        ...
        if centralHandshake == .idle {
            ...
            centralHelperConfirmed = false
            centralB4Subscribed = telemetrySubscribers.contains(peripheral.identifier)
    ...
    private func startCentralRouteIfPossible() {
        ...
        publishServiceIfPossible()
        guard servicePublished, publishedServiceUUID == telemetryRelayServiceUUID else {
    ...
    private func restoredOwnerHasSystemLinkProof(_ restored: CBPeripheral) -> Bool {
        guard centralManager.state == .poweredOn,
              restored.identifier == geelyPeripheral?.identifier else { return false }
        return centralManager.retrieveConnectedPeripherals(withServices: [serviceUUID])
            .contains(where: { $0.identifier == restored.identifier })
    }
    ...
    private func armRestoredOwnerProofObservation(_ restored: CBPeripheral) {
        guard centralRestoredPendingOwner, !centralRestorationRecoveryAttempted,
              ...
              restored.identifier == geelyPeripheral?.identifier else { return }
        ...
        let item = DispatchWorkItem { [weak self, weak restored] in
            ...
            guard self.runRequested, self.role == .central,
                  ...
                  restored.identifier == self.geelyPeripheral?.identifier,
        ...
        let item = DispatchWorkItem { [weak self, weak restored] in
            ...
            case .disconnected:
                ...
                self.append("Restored owner became disconnected after stable F04 proof; "
                    + "open the same owner")
                self.issueCentralConnect(restored,
                    reason: "same owner after evidence-driven restore reconciliation")
            @unknown default:
    ...
    private func resetCentralLink(reason: String) {
        ...
        append("Один controlled reconnect того же F04 owner · \(reason)")
...
extension ViewController: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral,
        ...
        if centralRestoredPendingOwner, let restored = geelyPeripheral,
           restored.identifier == peripheral.identifier {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if self.servicePublished {
                if self.role == .peripheral { self.startAdvertising() }
                    ...
                    if let owner = self.geelyPeripheral {
                        self.centralB4Subscribed = self.telemetrySubscribers
                            .contains(owner.identifier)
                    }
