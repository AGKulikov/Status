...
def encode_control(kind: str, mode: int, payload: bytes, flags: int = 0) -> bytes:
    assert kind in "HCA" and mode in (1, 2)
...
def decode_control(frame: bytes):
    if len(frame) != 20 or frame[0] not in map(ord, "HCA") or frame[1] != 2:
        return None
...
def verify_shared_fixtures() -> None:
    ...
    assert {vector["type"] for vector in fixture["vectors"]} == {"H", "C", "A", "T"}
...
def main() -> None:
    ...
    print("PASS: all 4 shared ANCS v2 wire vectors + strict negative decode fixtures")
