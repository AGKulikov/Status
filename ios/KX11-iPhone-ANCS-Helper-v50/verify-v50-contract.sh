...
[ "$(grep -Fc 'CURRENT_PROJECT_VERSION = 50;' "$PROJECT")" -eq 2 ]
[ "$(grep -Fc 'MARKETING_VERSION = 50.0;' "$PROJECT")" -eq 2 ]
...
grep -Fq 'SWIFT_TREAT_WARNINGS_AS_ERRORS=YES' "$ROOT/../../.github/workflows/verify-helper-v50.yml"
...
grep -Fq 'Status: source/replay accepted; hosted Xcode build and physical-device matrix remain release gates' \
    "$ROOT/RELEASE.txt"
grep -Fq 'ios/KX11-iPhone-ANCS-Helper-v50/**' \
    "$REPO_ROOT/.github/workflows/verify-helper-v50.yml"

echo 'PASS: Helper v50 clean-room source, shared vectors, restoration drains, and app contract'
