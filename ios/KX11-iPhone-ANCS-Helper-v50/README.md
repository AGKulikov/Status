# KX11 ANCS Helper v50 — production Route-A candidate

Helper v50 pairs with HA1217 and keeps Route A as the production topology:
...
managers: Simulator cannot establish the ANCS accessory topology and otherwise emits a misleading
`XPC connection invalid` console message. Compile coverage remains in CI; BLE acceptance requires
a physical iPhone.
...
Helper cannot observe the car's HFP/A2DP/PBAP state. HA1217 is therefore authoritative for the
...
starts in Route B, v50 first reclaims that exact namespace and runs the confirmed-quiescence switch
...
An in-place v49 → v50 update keeps bundle ID `ru.natro.kx11ancshelper`, the version-independent
...
Before calling v50 stable:

1. Install over v49 while preserving the existing iPhone/car pair and verify one visible phone
   identity, Classic HFP/A2DP/PBAP and F201/ANCS operation.
2. Start with Classic already connected and ANCS absent; verify HA1217 recovers it without another
