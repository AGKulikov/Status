...
python3 "$ROOT/verify_live_activity_v56.py"

PROJECT="$ROOT/KX11ANCSHelper.xcodeproj/project.pbxproj"
[ "$(grep -Fc 'CURRENT_PROJECT_VERSION = 56;' "$PROJECT")" -eq 4 ]
[ "$(grep -Fc 'MARKETING_VERSION = 56.0;' "$PROJECT")" -eq 4 ]
...
grep -Fq 'source/replay accepted; hosted Xcode build and physical-device matrix remain release gates' \
    "$ROOT/RELEASE.txt"

echo 'PASS: Helper v56 preserves transport and adds Live Activity, demo and ANCS actions'
