#!/usr/bin/env python3
"""Portable audit for Helper 69 Personal Team local ActivityKit lifecycle and controls."""
...
assert 'forSecurityApplicationGroupIdentifier' not in shared
assert 'UserDefaults(suiteName:' not in shared
assert 'sharedContainerURL' not in shared
assert 'static let supportsSharedDelivery = false' in shared
assert 'private static func defaults() -> UserDefaults { .standard }' in shared
...
assert 'replaceActivities(reason: "обновление Helper 69")' in manager
assert 'private var creatingActivities = false' in manager
assert 'guard !creatingActivities else' in manager
assert 'if becameConnected {' in manager
assert 'becameConnected || runningCount == 0' not in manager
...
# Every color literal authored by Helper is normalized. This turns a future out-of-range UIColor
# regression into a portable test failure; a remaining UIKit warning must be captured with Apple's
# UIColorBreakForOutOfRangeColorComponents breakpoint because it is not produced by these literals.
color_sources = [dashboard, settings, settings_root, widget]
ui_color = re.compile(
    r'UIColor\(red:\s*([0-9.]+),\s*green:\s*([0-9.]+),\s*blue:\s*([0-9.]+),'
    r'\s*alpha:\s*([0-9.]+)\)'
)
swiftui_color = re.compile(
    r'\.sRGB,\s*red:\s*([0-9.]+),\s*green:\s*([0-9.]+),\s*blue:\s*([0-9.]+),'
    r'\s*opacity:\s*([0-9.]+)'
)
components = [match.groups() for source in color_sources for match in ui_color.finditer(source)]
components += [match.groups() for source in color_sources for match in swiftui_color.finditer(source)]
assert components
for literal in components:
    assert all(0.0 <= float(component) <= 1.0 for component in literal), literal
...
print("PASS: Helper v69 Personal Team entitlement-clean Live Activity controls and compact payload")
