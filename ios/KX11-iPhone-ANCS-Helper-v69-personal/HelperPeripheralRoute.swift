    ...
    public final class Coordinator: NSObject {
        ...
        public func beginEnrollment(
            ...
            queue.async { [weak self] in
                ...
                guard self.generation == generation,
                    ...
                    self.diagnostic("запрос привязки отклонён текущим состоянием BLE")
                    onEvent(.failed(reason: "Привязка недоступна в текущем состоянии BLE"))
                    return
                }
                do {
                    ...
                    self.diagnostic("открыто 60-секундное окно защищённой привязки")
                    ...
                    self.diagnostic("не удалось создать криптографическую сессию привязки")
        ...
        public func confirmEnrollmentSAS(generation: BleRoleSwitchPolicy.Sequence) {
            queue.async { [weak self] in
                ...
                self.diagnostic("SAS подтверждён пользователем на iPhone; ждём Natro")
        ...
        public func cancelEnrollment(generation: BleRoleSwitchPolicy.Sequence) {
            queue.async { [weak self] in
                ...
                self.diagnostic("привязка отменена пользователем")
        ...
        private func handleRoutineHello(
            ...
            do {
                ...
                diagnostic("принят routine HELLO; проверяем сохранённую привязку")
                pendingEnrollmentResponse = PendingEnrollmentResponse(
                    generation: generation,
                    deadlineUptimeNanoseconds: deadline,
                    centralID: centralID,
        ...
        private func handleEnrollmentConfirm(
            ...
            enrollmentSession = session
            diagnostic("получено подтверждение SAS от Natro")
            pendingEnrollmentResponse = PendingEnrollmentResponse(
                generation: generation,
                deadlineUptimeNanoseconds: session.deadlineUptimeNanoseconds,
                centralID: centralID,
        ...
        private func completeEnrollmentPreauthenticationIfReady() {
            ...
            if preauthenticatedCentral?.token != session.token {
                ...
                diagnostic("обе стороны подтвердили SAS; открыт ограниченный pre-auth этап")
            }
            pendingEnrollmentResponse = PendingEnrollmentResponse(
                generation: session.generation,
                deadlineUptimeNanoseconds: session.deadlineUptimeNanoseconds,
        ...
        private func handleEnrollmentCommit(
            ...
            do {
                ...
                peripheral.respond(to: request, withResult: .success)
                diagnostic("новая привязка записана в crash-safe pending состояние")
        ...
        private func handleRoutineConfirm(
            ...
            diagnostic("routine proof принят; ждём encrypted H и CONTROL подписку")
            armPreauthenticationExpiry(
                token: session.token,
                centralID: centralID,
                deadlineUptimeNanoseconds: session.deadlineUptimeNanoseconds
        ...
        private func startOnQueue(
            ...
            diagnostic("создан Peripheral-владелец; ожидаем состояние Bluetooth и восстановление")
        ...
        private func publishFreshService(on manager: CBPeripheralManager) {
            ...
            diagnostic("публикуем F201: H, CONTROL, telemetry, enrollment и C5")
            ...
            let carRemote = CBMutableCharacteristic(
                ...
                // C5 is an optional high-volume catalog/state channel.  Android 9 must not wait
                // for a response callback which the ECARX stack can lose; encryption and the
                // authenticated CONTROL owner remain mandatory in didReceiveWrite.
        ...
        private func startUUIDOnlyAdvertisement(on manager: CBPeripheralManager) {
            ...
            diagnostic("запускаем рекламу F201 без имени и идентификаторов устройства")
        ...
        private func bindRestoredService(from dictionary: [String: Any]) -> RestoredTopology {
            ...
            if let carRemote {
                let validProperties = carRemote.properties == [
                    .write, .writeWithoutResponse, .indicate
                ] || carRemote.properties == [
        ...
        /// A new Helper process owns no still-executable local manager. When Route A restores its
        /// own namespace, an incompatible service can be stopped, removed and fenced as one
        /// local-only drain. Explicit role switches and Route B remain fail-closed.
        private var permitsUnprovableLocalOnlyDrain: Bool {
            if case .drainLocalOnlyRestore = restorationDisposition { return true }
            return false
        }
        ...
        private func beginRestorationMigrationDrain(
            ...
            let unprovableLocalOnly = !exactV2
                && topology != .empty
                && permitsUnprovableLocalOnlyDrain
            ...
            if !exactV2 && !emptyLocalOnly && !unprovableLocalOnly {
            ...
            queue.asyncAfter(deadline: .now() + Self.restorationObservationGrace) {
                ...
                // The restored local namespace has been stopped and removed for the full grace
                // interval. Continue Route-A recovery automatically instead of parking the UI on
                // ingressFreezeFailed until the user presses Retry.
                if unprovableLocalOnly, let epoch = self.restorationDrainEpoch {
                    self.observer?.helperPeripheralRoute(
                        self,
                        didFreezeWithoutRemoteOwner: epoch,
                        generation: generation
        ...
        private func emitReadyOnce(generation: BleRoleSwitchPolicy.Sequence) {
            ...
            diagnostic("CONTROL owner прошёл аутентификацию; защищённый маршрут готов")
        ...
        private func emitOperationalOnce(generation: BleRoleSwitchPolicy.Sequence) {
            ...
            managerStartupTimerToken = nil
            publicationTimerToken = nil
            diagnostic("локальный BLE-сервис работает; ждём Natro")
...
extension HelperPeripheralRoute.Coordinator: CBPeripheralManagerDelegate {
    public func peripheralManagerDidUpdateState(_ peripheral: CBPeripheralManager) {
        ...
        case .poweredOn:
            diagnostic("Bluetooth включён; проверяем восстановленную топологию")
            managerStartupTimerToken = nil
        ...
        case .poweredOff, .resetting:
            diagnostic(peripheral.state == .resetting
                ? "Core Bluetooth перезапускается; сохраняем безопасные границы владельца"
                : "Bluetooth выключен; защищённый владелец недоступен")
        ...
        case .unknown:
            diagnostic("состояние Bluetooth пока неизвестно")
            break
        @unknown default:
    ...
    public func peripheralManager(
        ...
        switch topology {
        case .exactV2:
            diagnostic("Core Bluetooth восстановил точную F201 v2 топологию")
        case .exactV52:
            diagnostic("Core Bluetooth восстановил совместимую F201 топологию с C5")
        case .legacyF04:
            diagnostic("обнаружена legacy F04 топология; запускаем безопасный drain")
        case .incompatibleUnprovable:
            diagnostic("восстановленная топология не доказуема; запускаем безопасный drain")
        case .empty:
            diagnostic("восстановленного BLE-владельца нет; создаём свежий F201")
        }
    ...
    public func peripheralManager(
        ...
        publicationTimerToken = nil
        diagnostic("Core Bluetooth подтвердил публикацию F201 сервиса")
        startUUIDOnlyAdvertisement(on: peripheral)
    }
    ...
    public func peripheralManagerDidStartAdvertising(
        ...
        lifecycle = controlSubscriberID == nil
            ...
            : .active(generation: generation)
        emitOperationalOnce(generation: generation)
        diagnostic("реклама F201 подтверждена; ждём аутентифицированную CONTROL подписку")
    ...
    public func peripheralManager(
        ...
        if request.characteristic.uuid == HelperPeripheralRoute.carRemoteUUID {
            guard case .active(let activeGeneration) = lifecycle,
                  ...
                  AuthenticatedC5FrameV1.isValid(value) else {
                peripheral.respond(to: request, withResult: .writeNotPermitted)
                return
            }
            peripheral.respond(to: request, withResult: .success)
    ...
    public func peripheralManager(
        ...
        if characteristic.uuid == HelperPeripheralRoute.controlUUID {
            ...
            lifecycle = .active(generation: generation)
            acceptingIngress = true
            diagnostic("аутентифицированная CONTROL подписка активна")
            ...
            diagnostic("telemetry подписка активна")
            ...
            diagnostic("C5 подписка управления автомобилем активна")
        }
        drainNotifications()
    }
    ...
    public func peripheralManager(
        ...
        if characteristic.uuid == HelperPeripheralRoute.controlUUID {
            ...
            acceptingIngress = false
            diagnostic("CONTROL подписка потеряна; защищённый канал остановлен")
            switch lifecycle {
            case .freezing(let epoch, let frozenGeneration) where frozenGeneration == generation:
            ...
            diagnostic("telemetry подписка снята")
            finishStopIfExactBoundaryReached()
