...
python3 "$ROOT/verify_live_activity_v69_personal.py"

PROJECT="$ROOT/KX11ANCSHelper.xcodeproj/project.pbxproj"
[ "$(grep -Fc 'CURRENT_PROJECT_VERSION = 69;' "$PROJECT")" -eq 4 ]
[ "$(grep -Fc 'MARKETING_VERSION = 69.0;' "$PROJECT")" -eq 4 ]
...
grep -Fq 'Build: 69' "$ROOT/RELEASE.txt"
...
echo 'PASS: Helper v69 Personal Team entitlement-clean Live Activity controls'
