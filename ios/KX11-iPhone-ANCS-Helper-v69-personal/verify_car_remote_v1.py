...
def verify_sources() -> None:
    ...
    assert "properties: [.write, .writeWithoutResponse, .indicate]" in peripheral
    ...
    assert "SUBSCRIBE_CAR_REMOTE" in route_a and "WRITE_TYPE_NO_RESPONSE" in route_a
    assert "CAR_REMOTE_NO_RESPONSE_SETTLE_MS" in route_a
    assert "ancsPreserved=true" in route_a
    ...
    assert "HELLO_COALESCE_MS = 30_000L" in controller
    ...
    assert "withTimeInterval: 5" in swift
