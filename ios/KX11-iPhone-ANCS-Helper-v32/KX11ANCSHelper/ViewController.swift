...
/// Helper v32 keeps one long-lived Central owner opened with RequiresANCS from its first connect.
...
final class ViewController: UIViewController {
    ...
    // These fixed F04 UUIDs remain for the legacy iPhone-peripheral route. In Central mode v32
    ...
    /// exact v32 owner. A system-wide connection discovered through another app is never adopted
    /// as proof of our ANCS contract.
    private var centralOwnerConfiguredForAncs = false
    /// iOS 17 owns reconnect after a callback with isReconnecting=true. While this is set, Helper
    /// observes the retained CBPeripheral indefinitely and must not issue a competing connect().
    ...
    private let centralConnectingObservationInterval: TimeInterval = 30
    ...
    override func viewDidLoad() {
        ...
        append("v32: выбран iPhone \(role.title); Peripheral сохраняет маршрут v11")
        ...
        append("Состояние .connecting не отменяется таймером; owner восстанавливается системой")
    ...
    private func buildInterface() {
        ...
        titleLabel.text = "KX11 ANCS HELPER v32"
    ...
    private func continueCentralConnected(_ peripheral: CBPeripheral) {
        ...
        if centralHandshake == .idle {
            ...
            publishServiceIfPossible()
            append("RequiresANCS didConnect получен; продолжаю PAIR/B3 на этом же owner")
    ...
    private func connectCentral(_ peripheral: CBPeripheral, reason: String) {
        ...
        if peripheral.state == .connecting {
            guard centralOwnerConfiguredForAncs else {
                ...
                append("Не принимаю неизвестный .connecting owner без v32 restoration; "
    ...
    /// Core Bluetooth owns pending connection lifetime. `.connecting` is a valid unbounded state,
    /// especially in background/restoration; this timer observes it but never cancels it.
    private func armCentralConnectTimeout(_ peripheral: CBPeripheral,
        ...
        let item = DispatchWorkItem { [weak self, weak peripheral] in
            ...
            case .connecting:
                self.append("Central остаётся .connecting; pending owner не отменяю")
                self.setStatus("CENTRAL · СИСТЕМА ПОДКЛЮЧАЕТ", color: .systemBlue)
                self.armCentralConnectTimeout(
                    peripheral,
                    delay: self.centralConnectingObservationInterval,
            ...
            case .disconnected:
                if self.centralSystemAutoReconnectActive {
                    self.append("System AutoReconnect остаётся владельцем disconnected state; "
                        + "ручной connect не запускаю")
                    self.setStatus("CENTRAL · SYSTEM RECONNECT", color: .systemBlue)
                    self.armCentralConnectTimeout(
                        peripheral,
                        delay: self.centralConnectingObservationInterval,
                        replaceExisting: true
    ...
    private func handleCentralDisconnect(_ peripheral: CBPeripheral, isReconnecting: Bool,
        ...
        if isReconnecting {
            ...
            armCentralConnectTimeout(
                peripheral,
                delay: centralConnectingObservationInterval,
                replaceExisting: true
            )
            return
        }
