...
v52 does not weaken identity to a BLE name, MAC/address equality, nearest RSSI or manufacturer
...
Without that explicit developer flag, Route B selection is rejected. v52 never runs both roles
...
An in-place v51 → v52 update keeps bundle ID `ru.natro.kx11ancshelper`, both stable Core Bluetooth
...
v52 removes the one-second telemetry timer. Battery, power, path and lock updates are event-driven.
...
- iOS controls background advertising contents and scheduling. v52 does not depend on a name.
