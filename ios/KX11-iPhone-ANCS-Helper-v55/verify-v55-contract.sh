...
python3 "$ROOT/verify_live_activity_v55.py"

PROJECT="$ROOT/KX11ANCSHelper.xcodeproj/project.pbxproj"
[ "$(grep -Fc 'CURRENT_PROJECT_VERSION = 55;' "$PROJECT")" -eq 4 ]
[ "$(grep -Fc 'MARKETING_VERSION = 55.0;' "$PROJECT")" -eq 4 ]
...
echo 'PASS: Helper v55 preserves transport and adds Live Activity, demo and ANCS actions'
