# KX11 ANCS Helper v55 — Bluetooth vehicle control

Minimum supported iOS: 14.0. Helper v55 uses the system CryptoKit HKDF implementation for the
enrollment protocol; devices left on iOS 13.x cannot install this build.

Helper v55 pairs with the Natro C4-enrollment runtime and keeps Route A as the production
topology:
...
v55 does not weaken identity to a BLE name, MAC/address equality, nearest RSSI or manufacturer
...
Without that explicit developer flag, Route B selection is rejected. v55 never runs both roles
...
An in-place v51 → v55 update keeps bundle ID `ru.natro.kx11ancshelper`, both stable Core Bluetooth
...
v55 removes the one-second telemetry timer. Battery, power, path and lock updates are event-driven.
...
Helper 55 embeds `NatroLiveActivityExtension` (minimum iOS 16.2) while the main Bluetooth app keeps
its iOS 14.0 deployment target. The Lock Screen card follows the compact dark Monjaro layout:
Natro/ANCS status, configurable vehicle name, a transparent vehicle asset, cabin/outdoor/set
temperatures, +/- and four user-selectable climate/seat buttons. Dynamic Island has compact,
minimal and expanded presentations. Buttons use `LiveActivityIntent` on iOS 17+ and a small App
Group mailbox; only the main Helper process owns and writes to the protected C5 Bluetooth channel.

Settings contain automatic start, vehicle visibility/name, four swappable controls, explicit
start/stop and a clearly marked demo mode. Demo starts without a car with 19 °C cabin, 7 °C outside
and 22 °C set point; its buttons and temperature steps update locally. Demo never changes the real
ANCS boolean returned to Apple Shortcuts.
...
Helper 55 retains C5, `D2D9E4C5-47F1-4E44-A8BB-A932FD5AF200`, without changing the occupied C2 role
...
They are never catalog or COMMAND IDs, so the 39-control safety contract remains frozen and older
Helpers can ignore them.
...
7. Enable demo away from the car and verify automatic card start, 19°/7°/22° values, all four
   configurable controls, temperature steps, Dynamic Island states and an explicit ДЕМО badge.
...
- iOS controls background advertising contents and scheduling. v55 does not depend on a name.
