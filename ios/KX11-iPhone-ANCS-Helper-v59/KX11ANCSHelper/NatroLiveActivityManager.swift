...
final class NatroLiveActivityManager: NSObject {
    ...
    private static let minimumActivityUpdateInterval: TimeInterval = 0.75
    ...
    func ensureRunning(reason: String, force: Bool = false) -> Bool {
        ...
        if !replacingActivities, existing.contains(where: { activity in
            ...
            replaceActivities(reason: "обновление Helper 59")
            return true
        }
    ...
    private func updateExistingActivities() {
        ...
        for activity in activities { update(activity) }
    }

    @available(iOS 16.2, *)
    private func update(_ activity: Activity<NatroLiveActivityAttributes>) {
    ...
    private func replaceActivities(reason: String) {
        ...
        Task {
            ...
            await MainActor.run {
                self.lastActivityFingerprints.removeAll()
                self.pendingActivityStates.removeAll()
                self.scheduledActivityUpdates.removeAll()
                self.lastActivityUpdateDates.removeAll()
                self.replacingActivities = false
                _ = self.ensureRunning(reason: reason, force: self.isDemoMode)
            }
        }
    }
