...
python3 "$ROOT/verify_live_activity_v59.py"

PROJECT="$ROOT/KX11ANCSHelper.xcodeproj/project.pbxproj"
[ "$(grep -Fc 'CURRENT_PROJECT_VERSION = 59;' "$PROJECT")" -eq 4 ]
[ "$(grep -Fc 'MARKETING_VERSION = 59.0;' "$PROJECT")" -eq 4 ]
...
echo 'PASS: Helper v59 preserves transport and fixes compact Live Activity, demo and controls'
