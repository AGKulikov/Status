...
private struct StatusLine: View {
    ...
    var body: some View {
        HStack(spacing: 4) {
            ...
            Text(state.status)
                .font(.system(size: compact ? 12 : 9, weight: .medium))
                .foregroundStyle(.secondary)
                .lineLimit(1)
                .minimumScaleFactor(0.72)
        }
        ...
        VStack(spacing: functionGrid ? 2 : 1) {
            HStack(spacing: 2) {
                Image(systemName: control.systemImage)
                    .font(.system(size: island ? 15 : (functionGrid ? 17 : 16), weight: .semibold))
                    .foregroundStyle(active ? accent : Color.white.opacity(0.9))
                if control.requiresConfirmation {
                    Image(systemName: "lock.fill")
                        .font(.system(size: island ? 7 : 8, weight: .bold))
                        .foregroundStyle(.secondary)
                }
            }
            Text(control.title)
                .font(.system(size: island ? 9 : (functionGrid ? 10 : 10), weight: .semibold))
                .lineLimit(1)
                .minimumScaleFactor(0.68)
