# KX11 ANCS Helper v53 — Bluetooth vehicle control

Minimum supported iOS: 14.0. Helper v53 uses the system CryptoKit HKDF implementation for the
enrollment protocol; devices left on iOS 13.x cannot install this build.

Helper v53 pairs with the Natro C4-enrollment runtime and keeps Route A as the production
topology:
...
v53 does not weaken identity to a BLE name, MAC/address equality, nearest RSSI or manufacturer
...
Without that explicit developer flag, Route B selection is rejected. v53 never runs both roles
...
An in-place v51 → v53 update keeps bundle ID `ru.natro.kx11ancshelper`, both stable Core Bluetooth
...
v53 removes the one-second telemetry timer. Battery, power, path and lock updates are event-driven.
...
Helper 53 adds C5, `D2D9E4C5-47F1-4E44-A8BB-A932FD5AF200`, without changing the occupied C2 role
protocol. C5 is encryption-required and is available only to the exact authenticated CONTROL owner
...
mechanical commands locally, rejects extra flags, and retries only the session HELLO while the
protected route is ready. Android keeps its existing capability, vehicle-state and read-back gates.
...
- iOS controls background advertising contents and scheduling. v53 does not depend on a name.
