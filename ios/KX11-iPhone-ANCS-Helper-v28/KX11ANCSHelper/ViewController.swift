...
/// Helper v28 uses two explicit Central links. A plain BLE bootstrap discovers a per-publication
/// dynamic namespace announced by a stable beacon and proves the
/// peer; only then does Core Bluetooth reconnect with RequiresANCS while Android skips all custom
/// discovery and immediately attaches its ANCS client. The iPhone-owned F05 relay remains the
/// post-handoff telemetry path. A missing B4 subscriber is telemetry degradation, not evidence
/// that the protected RequiresANCS owner failed. Classic Bluetooth is never modified here.
final class ViewController: UIViewController {
    ...
    // These fixed F04 UUIDs remain for the legacy iPhone-peripheral route. In Central mode v28
    ...
    private let restoreIdentifier =
        "ru.natro.kx11ancshelper.peripheral.v28.dynamic-gatt-g5"
    private let centralRestoreIdentifier =
        "ru.natro.kx11ancshelper.central.v28.dynamic-geely-ancs"
    ...
    override func viewDidLoad() {
        ...
        append("v28: выбран iPhone \(role.title); Peripheral сохраняет маршрут v11")
        append("Фаза 1: dynamic BLE/PAIR/B3; фаза 2: отдельный RequiresANCS reconnect")
        append("На RequiresANCS link custom UUID discovery не запускается")
        append("RequiresANCS didConnect сохраняется даже без B4; relay теперь optional")
    ...
    private func buildInterface() {
        ...
        titleLabel.text = "KX11 ANCS HELPER v28"
    ...
    private func continueCentralConnected(_ peripheral: CBPeripheral) {
        ...
        if centralLinkPhase == .requiresAncs {
            ...
            publishServiceIfPossible()
            append("RequiresANCS phase connected; custom dynamic discovery intentionally skipped")
            setStatus("ANCS OWNER АКТИВЕН · B4 OPTIONAL", color: .systemGreen)
            observeOptionalCentralTelemetryRelay("RequiresANCS relay observation")
    ...
    private func connectCentral(_ peripheral: CBPeripheral, reason: String) {
        ...
        // deterministic: Helper v28 resolves every Android publication from its beacon frame.
    ...
    private func beginCentralDiscovery(_ peripheral: CBPeripheral) {
        ...
        append("Geely_ANCS connected; discover dynamic D2D9 service "
    ...
    private func confirmCentralTelemetryRelay(_ source: String) {
        ...
        centralHandoffProofWorkItem = nil
        if centralHandoffActive {
            if firstProof { append("ANCS handoff подтверждён через B4 relay · \(source)") }
    ...
    private func observeOptionalCentralTelemetryRelay(_ reason: String,
        ...
        let item = DispatchWorkItem { [weak self] in
            ...
            guard self.runRequested, self.role == .central,
                  self.centralHandoffActive, !self.centralRelayProved else { return }
            self.append("B4 relay пока недоступен после \(reason); рабочий RequiresANCS owner "
                + "сохраняю без reset")
            self.setStatus("ANCS АКТИВЕН · B4 НЕДОСТУПЕН", color: .systemGreen)
    ...
    private func useCurrentCentralCharacteristics(_ peripheral: CBPeripheral,
        ...
        guard let control = characteristics.first(where: { $0.uuid == centralControlUUID }),
              let secure = characteristics.first(where: { $0.uuid == centralSecureUUID }) else {
            return false
        }
    ...
    private func updateConnectionStatus() {
        guard runRequested else { return }
        if role == .central {
            if centralHandoffActive && centralRelayProved && telemetrySubscribers.isEmpty {
                setStatus("ANCS OWNER АКТИВЕН · B4 READ OK", color: .systemGreen)
...
extension ViewController: CBCentralManagerDelegate {
    ...
    private func handleCentralDisconnect(_ peripheral: CBPeripheral, isReconnecting: Bool,
        ...
        if plannedAncsReconnect && hardReset == nil {
            ...
            clearCentralRuntime(keepPeripheral: true)
            guard runRequested, role == .central else { return }
            append("Bootstrap link released; открываю phase 2 без custom discovery")
        ...
        if completedSecureHandoff && hardReset == nil {
            centralHandoffActive = true
            publishServiceIfPossible()
            append("RequiresANCS Central link передан Android ANCS owner")
            if centralRelayProved || !telemetrySubscribers.isEmpty {
        ...
        if isReconnecting {
            // Handles a restored connection originally created by an older Helper. New v28
