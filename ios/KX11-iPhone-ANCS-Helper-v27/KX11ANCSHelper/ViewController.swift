...
/// Helper v27 uses two explicit Central links. A plain BLE bootstrap discovers F04 and proves the
...
final class ViewController: UIViewController {
    ...
    // Private GATT generation 4 replaces the F02 namespace that now returns
    // CBError.uuidNotAllowed even for unfiltered discovery on the live ATT link. Both ends use
    // F04 so Core Bluetooth and Android 9 must build a fresh bootstrap service database.
    ...
    // Fresh restoration owners keep pending v24/F02/F03 operations out of this GATT generation.
    private let restoreIdentifier =
        "ru.natro.kx11ancshelper.peripheral.v27.single-link-g5"
    private let centralRestoreIdentifier =
        "ru.natro.kx11ancshelper.central.v27.geely-ancs-g4"
    ...
    override func viewDidLoad() {
        ...
        append("v27: выбран iPhone \(role.title); Peripheral сохраняет маршрут v11")
        append("Фаза 1: plain BLE F04/PAIR/B3; фаза 2: отдельный RequiresANCS reconnect")
        ...
        append("D2D9 service: адресный поиск; B2/B3: отложенный unfiltered без пересечений")
    ...
    private func buildInterface() {
        ...
        titleLabel.text = "KX11 ANCS HELPER v27"
    ...
    private func continueCentralConnected(_ peripheral: CBPeripheral) {
        ...
        if centralLinkPhase == .requiresAncs {
            ...
            publishServiceIfPossible()
            append("RequiresANCS phase connected; custom F04 discovery intentionally skipped")
    ...
    private func connectCentral(_ peripheral: CBPeripheral, reason: String) {
        ...
        // deterministic: Helper v27 always resolves the next generation from a fresh advert.
    ...
    private func useCurrentCentralCharacteristics(_ peripheral: CBPeripheral,
        ...
        guard let control = characteristics.first(where: { $0.uuid == controlUUID }),
              let secure = characteristics.first(where: { $0.uuid == secureUUID }) else {
            return false
        }
...
extension ViewController: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager,
        ...
        append("Central restore owner · \(restored.identifier.uuidString)"
            + " · state=\(restored.state.rawValue)")
        switch restored.state {
        case .connected:
            continueCentralConnected(restored)
        case .connecting:
            armCentralConnectTimeout(restored, replaceExisting: true)
        ...
        case .disconnecting:
            centralRequireFreshAdvertisement = true
            geelyPeripheral = nil
            scheduleCentralReconnect(reason: "fresh advertisement after restored disconnecting")
        @unknown default:
            centralRequireFreshAdvertisement = true
            geelyPeripheral = nil
        }
    ...
    private func handleCentralDisconnect(_ peripheral: CBPeripheral, isReconnecting: Bool,
        ...
        if isReconnecting {
            // Handles a restored connection originally created by an older Helper. New v27
