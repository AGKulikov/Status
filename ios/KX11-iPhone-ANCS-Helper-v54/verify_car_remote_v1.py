...
def verify_sources() -> None:
    ...
    assert "CarRemoteProtocolV1.decode(value) != nil" in peripheral
    ...
    assert "HELLO_COALESCE_MS = 12_000L" in controller
    ...
    assert "withTimeInterval: 10" in swift
    ...
    # Vehicle control is now the primary UI. Legacy Bluetooth/SAS management stays in Settings.
    for tab in ("Климат", "Сиденья", "Медиа", "Комфорт", "Настройки"):
        assert f'"{tab}"' in ui
    for scene in ("Быстро охладить", "Зимнее утро", "Комфортная поездка", "Всё выключить"):
        assert scene in ui
    assert "sendSceneCommand" in swift and "pendingResults" in swift
    assert "Ожидание режима AUTO" in ui and "selectableValues(for: definition)" in ui
