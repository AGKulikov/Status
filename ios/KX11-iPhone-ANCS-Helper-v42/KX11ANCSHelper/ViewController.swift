...
/// Helper v42 keeps one long-lived RequiresANCS Central owner and performs one minimal security
...
/// beacon by itself is never destructive evidence.
final class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        append("v42: issued-connect .connecting recovery по exact current F04 beacon; one claim")
        append("Beacon reclaim разрешён только после фактического app-local connect token")
    ...
    private func buildInterface() {
        ...
        titleLabel.text = "KX11 ANCS HELPER v42"
    ...
    private func armCentralDeferredReclaimObservation(_ peripheral: CBPeripheral) {
        ...
        if !centralDeferredReclaimWaitLogged {
            ...
            append("Deferred .connecting owner: exact current F04 evidence observers armed; "
                + "beacon reclaim requires an app-issued token")
        }
        if !centralManager.isScanning {
            centralManager.scanForPeripherals(
                withServices: [managedIncomingBeaconUUID],
    ...
    private func observeCentralDeferredReclaimBeacon(
        ...
        guard centralDeferredReclaimActive,
              ...
              let issuedGeneration = centralDeferredReclaimIssuedNamespaceGeneration,
              issuedGeneration == 0x2F04,
              advertisedGeneration == issuedGeneration,
              advertisedGeneration == managedIncomingGeneration(from: serviceUUID),
        ...
        if !centralDeferredReclaimBeaconObserved {
            // CONTRACT_V42_BEACON_BINDS_LATER_ISSUED_TOKEN_AND_CURRENT_F04
            ...
            append("Exact saved-owner/current-2F04 beacon observed after app-issued connect "
                + "· token=\(issuedToken) · terminalGeneration="
                + "\(issuedTerminalGeneration) · RSSI=\(rssi); short didConnect grace armed")
        }
        armCentralDeferredReclaimProofGrace(
    ...
    private func armCentralDeferredReclaimProofGrace(_ peripheral: CBPeripheral,
        ...
        let item = DispatchWorkItem { [weak self, weak peripheral] in
            ...
            guard systemEvidence || issuedBeaconEvidence else {
                self.armCentralDeferredReclaimObservation(peripheral)
                return
            }
            ...
            self.append("Eligible F04 proof grace expired without didConnect; one bounded "
                + "ordinary reclaim cancel 1/1 · \(acceptedEvidence) · armed by \(source)")
...
extension ViewController: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral,
        ...
        let advertisedGeneration = advertisedCentralNamespace(advertisementData)
        if observeCentralDeferredReclaimBeacon(
            peripheral, advertisedGeneration: advertisedGeneration, rssi: RSSI) {
            return
        }
