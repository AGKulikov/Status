...
/// Helper v12 exposes two mutually exclusive BLE routes. Peripheral is the v11 production route
...
final class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        append("v12: выбран iPhone \(role.title); Peripheral сохраняет маршрут v11")
    ...
    private func buildInterface() {
        ...
        titleLabel.text = "KX11 ANCS HELPER v12"
    ...
    private func retryCentralSecure(_ peripheral: CBPeripheral, reason: String) {
        guard runRequested, role == .central,
        ...
        guard centralSecureReadAttempt < 45 else {
            ...
            scheduleCentralReconnect(reason: "encrypted SECURE timeout")
            return
        }
        centralSecureRetryWorkItem?.cancel()
...
extension ViewController: CBPeripheralDelegate {
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        if let error = error {
            ...
            scheduleCentralReconnect(reason: "service discovery failed")
            return
        }
        guard let service = peripheral.services?.first(where: { $0.uuid == serviceUUID }) else {
            append("Geely_ANCS service отсутствует после подключения")
            scheduleCentralReconnect(reason: "D2D9 service missing")
            return
        }
    ...
    func peripheral(_ peripheral: CBPeripheral,
        ...
        if let error = error {
            ...
            scheduleCentralReconnect(reason: "characteristic discovery failed")
            return
        }
        ...
        guard centralControlCharacteristic != nil,
            ...
            scheduleCentralReconnect(reason: "Geely_ANCS characteristics incomplete")
            return
        }
    ...
    func peripheral(_ peripheral: CBPeripheral, didWriteValueFor characteristic: CBCharacteristic,
        ...
        if characteristic.uuid == controlUUID {
            if let error = error {
                ...
                scheduleCentralReconnect(reason: "PAIR write failed")
            } else {
        ...
        if characteristic.uuid == telemetryUUID {
            ...
            if let error = error {
                ...
                scheduleCentralReconnect(reason: "encrypted telemetry write failed")
                return
            }
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic,
        ...
        if let error = error {
            retryCentralSecure(peripheral, reason: error.localizedDescription)
            return
        }
    ...
    func peripheral(_ peripheral: CBPeripheral, didModifyServices invalidatedServices: [CBService]) {
        ...
        append("Geely_ANCS service invalidated; rediscover")
        clearCentralRuntime(keepPeripheral: true)
        beginCentralDiscovery(peripheral)
    }
}
