...
python3 "$ROOT/verify_live_activity_v65_personal.py"

PROJECT="$ROOT/KX11ANCSHelper.xcodeproj/project.pbxproj"
[ "$(grep -Fc 'CURRENT_PROJECT_VERSION = 65;' "$PROJECT")" -eq 4 ]
[ "$(grep -Fc 'MARKETING_VERSION = 65.0;' "$PROJECT")" -eq 4 ]
...
grep -Fq 'Build: 65' "$ROOT/RELEASE.txt"
...
echo 'PASS: Helper v65 Personal Team direct Live Activity controls'
