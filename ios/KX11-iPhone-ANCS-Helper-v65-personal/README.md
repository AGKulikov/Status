# KX11 ANCS Helper v65 Personal Team — Bluetooth vehicle control and local Live Activity
...
APNs push-to-start from Natro is unavailable. Lock Screen controls use the existing URL fallback,
open Helper and require confirmation before a vehicle command is sent.
...
Minimum supported iOS: 14.0. Helper v65 uses the system CryptoKit HKDF implementation for the
enrollment protocol; devices left on iOS 13.x cannot install this build.

Helper v65 pairs with the Natro C4-enrollment runtime and keeps Route A as the production
topology:
...
Helper 64 embeds `NatroLiveActivityExtension` (minimum iOS 16.2) while the main Bluetooth app keeps
...
each tile uses a `natrohelper://command` link: Helper is brought forward, asks for explicit
confirmation (custom URL schemes are not an identity boundary), and then executes the same
validated C5 command path. Controls are enabled by a fully synchronised C5 vehicle route,
independently of ANCS availability.
...
```sh
./verify-v65-personal-contract.sh
```
...
Before calling v65 Personal Team stable:
