...
/// Helper v22 uses Central only to bootstrap the protected ANCS link. At the same time it exposes
...
final class ViewController: UIViewController {
    ...
    private let restoreIdentifier =
        "ru.natro.kx11ancshelper.peripheral.v22.single-link-g3"
    private let centralRestoreIdentifier =
        "ru.natro.kx11ancshelper.central.v22.geely-ancs-g2"
    ...
    private var centralUnfilteredCharacteristicDiscoveryAttempted = false
    ...
    override func viewDidLoad() {
        ...
        append("v22: выбран iPhone \(role.title); Peripheral сохраняет маршрут v11")
    ...
    private func buildInterface() {
        ...
        titleLabel.text = "KX11 ANCS HELPER v22"
    ...
    private func connectCentral(_ peripheral: CBPeripheral, reason: String) {
        ...
        // deterministic: Helper v22 always resolves the next generation from a fresh advert.
    ...
    private func beginCentralDiscovery(_ peripheral: CBPeripheral) {
        ...
        centralUnfilteredCharacteristicDiscoveryAttempted = false
    ...
    private func updateConnectionStatus() {
        guard runRequested else { return }
        if role == .central {
            if centralHandoffActive && centralRelayProved {
                setStatus("ANCS + ТЕЛЕМЕТРИЯ АКТИВНЫ", color: .systemGreen)
...
extension ViewController: CBCentralManagerDelegate {
    ...
    private func handleCentralDisconnect(_ peripheral: CBPeripheral, isReconnecting: Bool,
        ...
        if isReconnecting {
            // Handles a restored connection originally created by an older Helper. New v22
...
extension ViewController: CBPeripheralDelegate {
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        let missing = [controlUUID, secureUUID].filter { !cached.contains($0) }
    ...
    func peripheral(_ peripheral: CBPeripheral,
        ...
        if let error = error {
            ...
            if isCentralUuidNotAllowedError(error) {
                ...
                if !centralUnfilteredCharacteristicDiscoveryAttempted {
                    // A new restoration owner can receive uuidNotAllowed for an explicit B2/B3
                    // filter even though this current service is valid. Core Bluetooth supports
                    // nil to enumerate all characteristics. Retry once on this same ATT link;
                    // tearing it down here only repeats the identical failure after every scan.
                    centralUnfilteredCharacteristicDiscoveryAttempted = true
                    append("Filtered B2/B3 discovery запрещён; повторяю все характеристики "
                        + "на текущем BLE link")
        ...
        guard useCurrentCentralCharacteristics(peripheral, service: service,
            ...
            append("Geely_ANCS incomplete: CONTROL/SECURE required")
...
extension ViewController: CBPeripheralManagerDelegate {
    ...
    func peripheralManager(_ peripheral: CBPeripheralManager, central: CBCentral,
        ...
        if role == .central && centralHandoffActive && telemetrySubscribers.isEmpty {
            centralRelayProved = false
            waitForCentralTelemetryRelay("B4 relay unsubscribed", timeout: 5)
        }
        updateTelemetryLabel()
        updateConnectionStatus()
    }
