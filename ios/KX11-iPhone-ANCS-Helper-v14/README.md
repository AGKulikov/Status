# KX11 ANCS Helper v14 for iPhone

Helper v14 accompanies Status Widget HA1166 and keeps the explicit iPhone BLE-role selector. The
...
Helper v14 also treats a removed D2D9 service, invalid ATT handle, or encryption loss after a
...
It discovers the complete four-characteristic D2D9 table without a UUID filter, rejects callbacks
from an older `CBService` instance, records the Core Bluetooth error domain/code, and backs repeated
failures off through 1/2/5/10/20/30 seconds instead of hammering the same session every second.

Status Widget HA1166 no longer pre-claims a saved iOS resolvable private address when it creates a
new `Geely_ANCS` GATT-server generation. The current incoming callback becomes verified through
`PAIR`, and its address is persisted only after the encrypted `SECURE` operation succeeds. Helper
v14 Central mode therefore must be tested together with HA1166 or newer.
