...
/// Helper v14 exposes two mutually exclusive BLE routes. Peripheral is the v11 production route
...
final class ViewController: UIViewController {
    ...
    // Keep the v10 restoration identity stable so an in-place v11 update can reclaim the same
    // Core Bluetooth peripheral state instead of publishing a competing restored service.
    private let restoreIdentifier = "ru.natro.kx11ancshelper.peripheral.v10.single-link"
    private let centralRestoreIdentifier =
        "ru.natro.kx11ancshelper.central.v12.geely-ancs"
    ...
    /// Survives a runtime clear so a later encryption error is recognized as a broken reconnect,
    /// not mistaken for the first pairing prompt and retried against stale ATT handles for 90 s.
    private var centralHadSecureSession = false
    ...
    override func viewDidLoad() {
        ...
        // The Geely identifier is persisted only by markCentralReady after encrypted SECURE.
        centralHadSecureSession = defaults.string(
            forKey: savedGeelyPeripheralPreference) != nil
        buildInterface()
        roleControl.selectedSegmentIndex = role.rawValue
        updateButtons()

        append("v14: выбран iPhone \(role.title); Peripheral сохраняет маршрут v11")
    ...
    private func buildInterface() {
        ...
        titleLabel.text = "KX11 ANCS HELPER v14"
    ...
    private func startCentralRouteIfPossible() {
        ...
        if let peripheral = geelyPeripheral {
            ...
            case .connecting:
                ...
                return
            case .disconnecting:
                scheduleCentralReconnect(reason: "peer disconnecting")
                return
            case .disconnected:
    ...
    private func connectCentral(_ peripheral: CBPeripheral, reason: String) {
        ...
        // Use the literal only on iOS 17+. This avoids an eager dynamic-symbol lookup when the
        // same source is installed on iOS 13–16 while still enabling the public
        // CBConnectPeripheralOptionEnableAutoReconnect behavior. The public NSString constant's
        // dictionary value is kCBConnectOptionEnableAutoReconnect.
        if #available(iOS 17.0, *) {
            options["kCBConnectOptionEnableAutoReconnect"] = true
        }
        centralManager.connect(peripheral, options: options)
        setStatus("CENTRAL · ПОДКЛЮЧЕНИЕ", color: .systemBlue)
        append("connect Geely_ANCS · RequiresANCS=true · AutoReconnect="
            + "\(ProcessInfo.processInfo.operatingSystemVersion.majorVersion >= 17) · \(reason)")
    }
    ...
    private func writeCentralPair(_ peripheral: CBPeripheral) {
        guard let control = centralControlCharacteristic else {
            scheduleCentralReconnect(reason: "CONTROL B2 missing")
            return
        }
        centralHandshake = .writingPair
        peripheral.writeValue(Data("PAIR".utf8), for: control, type: .withResponse)
        setStatus("CENTRAL · PAIR / ШИФРОВАНИЕ", color: .systemOrange)
    ...
    private func readCentralSecure(_ peripheral: CBPeripheral) {
        guard let secure = centralSecureCharacteristic else {
            scheduleCentralReconnect(reason: "SECURE B3 missing")
            return
        }
        centralSecureRetryWorkItem?.cancel()
        centralSecureRetryWorkItem = nil
        ...
        append("READ encrypted SECURE · attempt \(centralSecureReadAttempt)")
    }
    ...
    private func retryCentralSecure(_ peripheral: CBPeripheral, error: Error) {
        ...
        // Once this app has already completed SECURE, five protected reads are enough to let
        // Core Bluetooth restore the stored LTK. Continuing after that only hammers an
        // unencrypted/stale link; force a genuine reconnect instead.
        if centralHadSecureSession && isCentralEncryptionError(error)
                && centralSecureReadAttempt >= 5 {
            resetCentralLink(reason: "LE encryption was not restored · \(reason)")
            return
        }
        guard centralSecureReadAttempt < 45 else {
            append("SECURE не подтверждён за 90 с · \(reason)")
            resetCentralLink(reason: "encrypted SECURE timeout")
            return
        }
        centralSecureRetryWorkItem?.cancel()
        ...
        setStatus("ПОДТВЕРДИТЕ LE-СОПРЯЖЕНИЕ", color: .systemOrange)
        append("SECURE ждёт LE encryption; повтор через 2 с · \(reason)")
    ...
    private func updateTelemetryLabel() {
        if role == .central {
            ...
            telemetryLabel.text = "\(batteryDescription()) · \(currentNetworkType())\n"
                + "KX11 WRITE: \(write) · \(centralHandshake == .ready ? "защищён" : "ожидание")"
            return
        }
...
extension ViewController: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didFailToConnect peripheral: CBPeripheral,
        ...
        scheduleCentralReconnect(reason: "didFailToConnect")
    }
    ...
    private func handleCentralDisconnect(_ peripheral: CBPeripheral, isReconnecting: Bool,
        ...
        if let reason = centralHardResetReason {
            centralHardResetReason = nil
            geelyPeripheral = nil
            scheduleCentralReconnect(reason: "fresh advertisement after \(reason)")
            return
        ...
        if isReconnecting {
            setStatus("CENTRAL · СИСТЕМА ПЕРЕПОДКЛЮЧАЕТ", color: .systemOrange)
        } else {
            scheduleCentralReconnect(reason: "link disconnected")
        }
    }
...
extension ViewController: CBPeripheralDelegate {
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        // The private D2D9 service has only four characteristics.  Discovering all of them avoids
        // Core Bluetooth's uuidNotAllowed failure against a stale filtered characteristic cache;
        // the callback below still accepts only B2/B3/B4 from this exact CBService instance.
        peripheral.discoverCharacteristics(nil, for: service)
        append("D2D9 service current; discover all characteristics (unfiltered)")
    }
    ...
    func peripheral(_ peripheral: CBPeripheral,
        ...
        for characteristic in service.characteristics ?? [] {
            switch characteristic.uuid {
            case controlUUID: centralControlCharacteristic = characteristic
            case secureUUID: centralSecureCharacteristic = characteristic
            case telemetryUUID: centralTelemetryCharacteristic = characteristic
            default: break
            }
        }
        guard centralControlCharacteristic != nil,
              centralSecureCharacteristic != nil,
              centralTelemetryCharacteristic != nil else {
        ...
        guard centralControlCharacteristic?.properties.contains(.write) == true,
              centralSecureCharacteristic?.properties.contains(.read) == true,
              centralTelemetryCharacteristic?.properties.contains(.write) == true else {
    ...
    func peripheral(_ peripheral: CBPeripheral, didWriteValueFor characteristic: CBCharacteristic,
        ...
        if characteristic.uuid == controlUUID {
            if let error = error {
                ...
                append("WRITE PAIR accepted; читаю encrypted SECURE")
        ...
        if characteristic.uuid == telemetryUUID {
            ...
            if let error = error {
                ...
                resetCentralLink(reason: "encrypted telemetry write failed · "
                    + detail)
                return
            }
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic,
        ...
        guard peripheral.identifier == geelyPeripheral?.identifier,
              characteristic.uuid == secureUUID else { return }
        if let error = error {
