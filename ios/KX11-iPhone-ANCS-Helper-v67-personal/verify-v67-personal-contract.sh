...
python3 "$ROOT/verify_live_activity_v67_personal.py"

PROJECT="$ROOT/KX11ANCSHelper.xcodeproj/project.pbxproj"
[ "$(grep -Fc 'CURRENT_PROJECT_VERSION = 67;' "$PROJECT")" -eq 4 ]
[ "$(grep -Fc 'MARKETING_VERSION = 67.0;' "$PROJECT")" -eq 4 ]
...
grep -Fq 'Build: 67' "$ROOT/RELEASE.txt"
grep -Fq 'Paired runtime: Natro 2.2.13' "$ROOT/RELEASE.txt"
...
echo 'PASS: Helper v67 Personal Team entitlement-clean Live Activity controls'
