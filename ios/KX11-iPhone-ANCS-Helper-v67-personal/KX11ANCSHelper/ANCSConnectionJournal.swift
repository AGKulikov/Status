...
final class ANCSConnectionJournal {
    ...
    static var helperVersion: String {
        ...
        switch (marketing?.isEmpty == false ? marketing : nil,
                build?.isEmpty == false ? build : nil) {
        case let (.some(marketing), .some(build)):
            return "Helper \(marketing) (build \(build))"
        case let (.some(marketing), .none):
            return "Helper \(marketing)"
        case let (.none, .some(build)):
            return "Helper build \(build)"
        case (.none, .none):
            return "Helper version unavailable"
        }
    }
    ...
    func exportURL() throws -> URL {
        let header = "Natro ANCS \(Self.helperVersion)\n"
