...
public final class HelperBleRuntimeCoordinator {
    ...
    /// Privacy-safe, human-readable BLE stages for the in-app connection journal.
    ...
    public init(
        ...
        routeA.onDiagnostic = { [weak self] message in
            self?.onDiagnosticEvent?("Route A: \(message)")
        }
        routeB.onDiagnostic = { [weak self] message in
            self?.onDiagnosticEvent?("Route B: \(message)")
        }
    }
    ...
    public func sendRouteACarRemoteFrame(
        _ frame: Data,
        generation: BleRoleSwitchPolicy.Sequence
    ) {
        routeA.sendCarRemoteFrame(frame, generation: generation)
    }

    public func beginRouteAEnrollment(
        generation: BleRoleSwitchPolicy.Sequence,
        onEvent: @escaping (HelperPeripheralRoute.EnrollmentEvent) -> Void
    ) {
        routeA.beginEnrollment(generation: generation, onEvent: onEvent)
    }

    public func confirmRouteAEnrollmentSAS(
        generation: BleRoleSwitchPolicy.Sequence
    ) {
        routeA.confirmEnrollmentSAS(generation: generation)
    }

    public func cancelRouteAEnrollment(
        generation: BleRoleSwitchPolicy.Sequence
    ) {
        routeA.cancelEnrollment(generation: generation)
    }

    public func routeAEnrollmentBindingState(
        _ completion: @escaping (HelperPeripheralRoute.EnrollmentBindingState) -> Void
    ) {
        routeA.enrollmentBindingState(completion)
    }

    public func resetRouteAEnrollmentBinding(
        generation: BleRoleSwitchPolicy.Sequence,
        completion: @escaping (Result<Void, Error>) -> Void
    ) {
        routeA.resetEnrollmentBinding(generation: generation, completion: completion)
    }
    ...
    public func sendRouteBCarRemoteFrame(
        _ frame: Data,
        generation: BleRoleSwitchPolicy.Sequence
    ) {
        routeB.sendCarRemoteFrame(frame, generation: generation)
    }
...
extension HelperBleRuntimeCoordinator: HelperPeripheralRoute.Observer {
    ...
    public func helperPeripheralRoute(
        ...
        onEvent?(.routeACarRemoteFrame(frame, generation: generation))
    }
...
extension HelperBleRuntimeCoordinator: HelperCentralRoute.Observer {
    ...
    public func helperCentralRoute(
        ...
        onEvent?(.routeBCarRemoteFrame(frame, generation: generation))
    }
