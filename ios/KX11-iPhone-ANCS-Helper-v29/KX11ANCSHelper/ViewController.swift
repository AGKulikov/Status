...
/// Helper v29 uses two explicit Central links. A plain BLE bootstrap discovers a per-publication
...
final class ViewController: UIViewController {
    ...
    // These fixed F04 UUIDs remain for the legacy iPhone-peripheral route. In Central mode v29
    ...
    private let restoreIdentifier =
        "ru.natro.kx11ancshelper.peripheral.v29.dynamic-gatt-g5"
    private let centralRestoreIdentifier =
        "ru.natro.kx11ancshelper.central.v29.dynamic-geely-ancs"
    ...
    override func viewDidLoad() {
        ...
        append("v29: выбран iPhone \(role.title); Peripheral сохраняет маршрут v11")
        ...
        append("didConnect не считается ANCS: зелёный статус даёт только Android CCCD ACK")
    ...
    private func buildInterface() {
        ...
        titleLabel.text = "KX11 ANCS HELPER v29"
    ...
    private func connectCentral(_ peripheral: CBPeripheral, reason: String) {
        ...
        // deterministic: Helper v29 resolves every Android publication from its beacon frame.
    ...
    private func observeOptionalCentralTelemetryRelay(_ reason: String,
        ...
        let item = DispatchWorkItem { [weak self] in
            ...
            guard self.runRequested, self.role == .central,
                  self.centralHandoffActive, !self.centralAncsProved else { return }
            self.append("ANCS-SUBSCRIBED пока не получен после \(reason); phase-2 link "
                + "сохраняю без reset")
            self.setStatus(self.centralRelayProved
                ? "B4 АКТИВЕН · ЖДУ ANCS ACK"
                : "REQUIRES_ANCS OK · ЖДУ ANDROID", color: .systemOrange)
    ...
    private func updateConnectionStatus() {
        guard runRequested else { return }
        if role == .central {
            if centralHandoffActive && centralAncsProved {
                setStatus("ANCS + ТЕЛЕМЕТРИЯ АКТИВНЫ", color: .systemGreen)
...
extension ViewController: CBCentralManagerDelegate {
    ...
    private func handleCentralDisconnect(_ peripheral: CBPeripheral, isReconnecting: Bool,
        ...
        if isReconnecting {
            // Handles a restored connection originally created by an older Helper. New v29
    ...
    func peripheral(_ peripheral: CBPeripheral,
        ...
        if let error = error {
            ...
            if isCentralUuidNotAllowedError(error) {
                ...
                append("Characteristic discovery временно запрещён; отложенный retry")
                recoverCentralCharacteristicDiscovery(
                    peripheral, service: service, reason: detail)
                return
            }
