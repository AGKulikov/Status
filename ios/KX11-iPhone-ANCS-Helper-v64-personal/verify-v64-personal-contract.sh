...
python3 "$ROOT/verify_live_activity_v64_personal.py"

PROJECT="$ROOT/KX11ANCSHelper.xcodeproj/project.pbxproj"
[ "$(grep -Fc 'CURRENT_PROJECT_VERSION = 64;' "$PROJECT")" -eq 4 ]
[ "$(grep -Fc 'MARKETING_VERSION = 64.0;' "$PROJECT")" -eq 4 ]
...
grep -Fq 'Build: 64' "$ROOT/RELEASE.txt"
grep -Fq 'Paired runtime: Natro 2.2.10' "$ROOT/RELEASE.txt"
...
echo 'PASS: Helper v64 Personal Team local ActivityKit lifecycle and Shortcut controls'
