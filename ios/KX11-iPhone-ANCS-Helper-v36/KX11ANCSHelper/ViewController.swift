...
/// Helper v36 follows the connection model used by production ANCS accessories: iPhone owns one
/// long-lived GAP Central connection opened with RequiresANCS, while Android attaches its ANCS
/// GATT client to that same physical link. Helper never discovers or touches Android's GATT
/// database, so a cached/custom service cannot tear down a healthy ANCS connection. The
/// iPhone-owned F05 relay carries telemetry and Android's post-CCCD ANCS-SUBSCRIBED proof.
final class ViewController: UIViewController {
    ...
    // F04 is the permanent link-anchor UUID. Central mode scans the stable FFFF beacon and never
    // discovers Android characteristics; Peripheral mode keeps the legacy diagnostic service.
    ...
    override func viewDidLoad() {
        ...
        append("v36: стандартная ANCS link-owner схема; Peripheral сохраняет legacy-маршрут")
        append("Один Central owner: RequiresANCS=true с первого connect")
        append("Helper не выполняет GATT discovery Android и не зависит от его service cache")
        append("Pending connect не имеет watchdog: Core Bluetooth сам ждёт доступный peer")
    ...
    private func buildInterface() {
        ...
        titleLabel.text = "KX11 ANCS HELPER v36"
    ...
    private func continueCentralConnected(_ peripheral: CBPeripheral) {
        ...
        if centralHandshake == .idle {
            ...
            append("RequiresANCS owner connected · ancsAuthorized=\(centralAncsAuthorized); "
                + "Android сам запускает ANCS discovery на этом физическом link")
            refreshCentralReadiness("RequiresANCS owner connected")
            observeCentralReadiness("RequiresANCS owner connected", timeout: 60)
        }
    }
    ...
    private func connectCentral(_ peripheral: CBPeripheral, reason: String) {
        ...
        if peripheral.state == .connecting {
            guard centralOwnerConfiguredForAncs else {
                issueCentralConnect(peripheral,
                    reason: "app-local RequiresANCS on retrieved pending owner · \(reason)")
                return
            }
    ...
    private func beginCentralDiscovery(_ peripheral: CBPeripheral) {
        // Intentionally empty in v36. Production ANCS accessories do not make the iPhone-side
        // Central inspect the accessory GATT database. Android owns all ANCS discovery and CCCD
        // work on the reverse GATT role of this same physical connection.
        continueCentralConnected(peripheral)
    }
    ...
    private func armCentralConnectTimeout(_ peripheral: CBPeripheral,
        ...
        let item = DispatchWorkItem { [weak self, weak peripheral] in
            ...
            guard self.centralManager != nil,
                  self.centralManager.state == .poweredOn else {
                self.append("Central watchdog ждёт poweredOn; BLE-команду не отправляю")
                return
            }
            switch peripheral.state {
            case .connected:
    ...
    private func resetCentralLink(reason: String) {
        ...
        if centralNamespaceResolved {
            centralRejectedNamespaceGeneration = centralNamespaceGeneration
        }
        centralNamespaceResolved = false
...
extension ViewController: CBCentralManagerDelegate {
    ...
    private func handleCentralDisconnect(_ peripheral: CBPeripheral, isReconnecting: Bool,
        ...
        // The same CBPeripheral was originally opened with RequiresANCS. Keep that fact through
        // the callback boundary: Core Bluetooth may deliver a queued didConnect before our
        // manual-backoff work item runs. Clearing it here made v35 reject and cancel that valid
        // reconnect seven milliseconds later.
        centralOwnerConfiguredForAncs = true
        centralSystemAutoReconnectActive = false
        scheduleCentralReconnect(reason: "ordinary disconnect; retained owner")
    }
...
extension ViewController: CBPeripheralDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didModifyServices invalidatedServices: [CBService]) {
        guard runRequested, role == .central,
              peripheral.identifier == geelyPeripheral?.identifier else { return }
        append("Android service invalidation ignored by link-owner Helper; "
            + "ANCS discovery remains Android-owned")
    }
}
