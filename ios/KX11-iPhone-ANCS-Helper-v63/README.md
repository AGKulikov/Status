# KX11 ANCS Helper v63 — Bluetooth vehicle control and APNs Live Activity

Minimum supported iOS: 14.0. Helper v63 uses the system CryptoKit HKDF implementation for the
enrollment protocol; devices left on iOS 13.x cannot install this build.

Helper v63 pairs with the Natro C4-enrollment runtime and keeps Route A as the production
topology:
...
Helper 63 embeds `NatroLiveActivityExtension` (minimum iOS 16.2) while the main Bluetooth app keeps
...
and expanded presentations for both panels. Buttons use `LiveActivityIntent` on iOS 17+ and a
bounded App Group mailbox; only the main Helper process owns and writes to protected C5. Controls
are enabled by a fully synchronised C5 vehicle route, independently of ANCS availability. If an
Xcode development profile cannot provision the App Group, each tile falls back to a
`natrohelper://command` link: Natro is brought forward, asks for explicit confirmation (custom URL
schemes are not an identity boundary), and then executes the same validated command path instead
...
Natro 2.2.10 is the direct APNs provider. Helper exports the push-to-start token, two compact static
panel configurations and every per-activity push token through authenticated C5; no APNs signing
key travels to iPhone. Natro sends `start` only on the exact selected phone's Classic Bluetooth
rising edge and `end` on the falling edge. The user imports a dedicated Topic Specific `.p8` key
after installing Natro; Android Keystore encrypts it under no-backup storage, and it is absent from
the APK, Git and logs. The required topic is
`ru.natro.kx11ancshelper.push-type.liveactivity`. Xcode development installs use APNs Sandbox;
distribution profiles use Production.

If the provider was ready before C5 sync, Helper waits for remote start and uses a bounded two-
second local foreground fallback. This prevents duplicate cards but keeps first provisioning and
temporary APNs failure usable. A remote start is observed through `Activity.activityUpdates`, and
locally-created activities request `.token`, so Natro learns the exact token needed to end each
card. Helper also ends existing activities locally on Bluetooth loss as an independent safety path.
...
Natro also sends four read-only STATE frames outside the command registry: `0xFB` direct APNs
provider readiness, `0xFC` cabin temperature (tenths °C), `0xFD` outdoor temperature (tenths °C),
and `0xFE` exact ANCS readiness.
