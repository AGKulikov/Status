    ...
    var body: some View {
        Group {
            if #available(iOSApplicationExtension 17.0, *) {
                if NatroLiveActivityCommandMailbox.supportsSharedDelivery {
                    Button(intent: NatroLiveControlIntent(action: control.commandAction)) {
                        label
                    }
                    .buttonStyle(.plain)
                } else {
                    Link(destination: NatroLiveActivityCommandLink.url(
                        for: control.commandAction
                    )) { label }
                    .buttonStyle(.plain)
                }
            } else {
        ...
        Group {
            if #available(iOSApplicationExtension 17.0, *) {
                if NatroLiveActivityCommandMailbox.supportsSharedDelivery {
                    ...
                    Link(destination: NatroLiveActivityCommandLink.url(for: action)) { circle }
                        .buttonStyle(.plain)
                }
            } else {
                circle
