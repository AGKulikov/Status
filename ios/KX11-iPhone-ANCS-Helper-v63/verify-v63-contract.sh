...
python3 "$ROOT/verify_live_activity_v63.py"

PROJECT="$ROOT/KX11ANCSHelper.xcodeproj/project.pbxproj"
[ "$(grep -Fc 'CURRENT_PROJECT_VERSION = 63;' "$PROJECT")" -eq 4 ]
[ "$(grep -Fc 'MARKETING_VERSION = 63.0;' "$PROJECT")" -eq 4 ]
...
grep -Fq 'Build: 63' "$ROOT/RELEASE.txt"
...
echo 'PASS: Helper v63 direct APNs lifecycle and low-latency ActivityKit controls'
