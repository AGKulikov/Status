#!/usr/bin/env python3
"""Portable source/asset audit for Helper 63 APNs lifecycle and responsive companion UI."""
...
# Dev profiles without the App Group no longer instantiate an invalid suite; signed profiles keep
# the same mailbox identity for app-extension commands.
for entitlement in (
...
assert 'NatroLiveActivityCommandLink.url(' in widget
...
assert 'replaceActivities(reason: "обновление Helper 63")' in manager
...
# Direct push-to-start is provisioned only through authenticated C5. Helper has no APNs private
# key, observes both remote starts and per-activity tokens, and retains a bounded local fallback.
assert 'pushToStartTokenUpdates' in manager
assert 'Activity<NatroLiveActivityAttributes>.activityUpdates' in manager
assert 'activity.pushTokenUpdates' in manager
assert 'pushType: .token' in manager
assert 'scheduleLocalStartFallback' in manager
assert '.now() + 2' in manager
...
assert '<key>aps-environment</key>' in text(
    ROOT / "KX11ANCSHelper/KX11ANCSHelper.entitlements")
...
assert 'терминальный снимок v56 восстановлен в основной Route A' in runtime
...
print("PASS: Helper v63 APNs lifecycle, responsive controls, compact payload and startup repair")
