...
final class NatroLiveActivityManager: NSObject {
    ...
    private var activityPushTokens: [String: Data] = [:]
    private var apnsStartFallback: DispatchWorkItem?
    ...
    private static let pushToStartDefaultsKey = "liveActivity.pushToStartToken.v63"
    ...
    func remoteDidChange(_ remote: CarRemoteClient) {
        ...
        if isDemoMode {
            ...
            if NatroLiveActivityPreferences.automaticStart,
               becameConnected || runningCount == 0 {
                provisionPushState(to: remote)
                if remote.liveActivityProviderReady && runningCount == 0 {
                    scheduleLocalStartFallback(remote)
                } else {
                    cancelLocalStartFallback()
    ...
    func ensureRunning(reason: String, force: Bool = false) -> Bool {
        ...
        if !replacingActivities, existing.contains(where: { activity in
            ...
            replaceActivities(reason: "обновление Helper 63")
            return true
        }
    ...
    private func installPushObserversIfNeeded() {
        guard #available(iOS 16.2, *) else { return }
    ...
    private func observePushToken(
        for activity: Activity<NatroLiveActivityAttributes>
    ) {
        guard activityTokenTasks[activity.id] == nil else { return }
        activityTokenTasks[activity.id] = Task { [weak self, weak activity] in
    ...
    private func provisionPushState(to remote: CarRemoteClient) {
        guard remote.isSynced else { return }
        sendConfiguration(to: remote)
        if let token = UserDefaults.standard.data(forKey: Self.pushToStartDefaultsKey) {
            remote.sendLiveActivityProvisioning(type: .pushToStartToken, payload: token)
        }
        if #available(iOS 16.2, *) {
            for activity in Activity<NatroLiveActivityAttributes>.activities {
                observePushToken(for: activity)
                if let token = activityPushTokens[activity.id] {
                    sendActivityToken(activity: activity, token: token, to: remote)
                }
            }
        }
    }
    ...
    @available(iOS 16.2, *)
    private func sendActivityToken(
        activity: Activity<NatroLiveActivityAttributes>,
        token: Data,
        to remote: CarRemoteClient
    ) {
        let identifier = Array(activity.id.utf8.prefix(96))
        guard !identifier.isEmpty, token.count >= 16, token.count <= 64 else { return }
        var payload: [UInt8] = [
            1, activity.attributes.resolvedPanel == .climate ? 0 : 1,
            UInt8(identifier.count)
        ]
        payload.append(contentsOf: identifier)
        payload.append(UInt8(token.count))
        payload.append(contentsOf: token)
        remote.sendLiveActivityProvisioning(type: .activityPushToken, payload: Data(payload))
    }

    @available(iOS 16.2, *)
    private func sendActivityEnded(
        activity: Activity<NatroLiveActivityAttributes>,
        to remote: CarRemoteClient
    ) {
        let identifier = Array(activity.id.utf8.prefix(96))
        guard !identifier.isEmpty else { return }
        let payload = Data([1, activity.attributes.resolvedPanel == .climate ? 0 : 1,
                            UInt8(identifier.count)] + identifier)
        remote.sendLiveActivityProvisioning(type: .activityEnded, payload: payload)
    }

    private func scheduleLocalStartFallback(_ exactRemote: CarRemoteClient) {
        cancelLocalStartFallback()
        publishStatus("Push-to-start отправлен магнитолой…")
        let fallback = DispatchWorkItem { [weak self, weak exactRemote] in
            guard let self, let exactRemote, self.remote === exactRemote,
                  exactRemote.isSynced, self.runningCount == 0 else { return }
            _ = self.ensureRunning(reason: "резерв после APNs push-to-start")
        }
        apnsStartFallback = fallback
        DispatchQueue.main.asyncAfter(deadline: .now() + 2, execute: fallback)
    }

    private func cancelLocalStartFallback() {
        apnsStartFallback?.cancel()
        apnsStartFallback = nil
    }
    ...
    @objc private func preferencesChanged() {
        ...
        updateExistingActivities()
        if let remote, remote.isSynced { provisionPushState(to: remote) }
        publishChangeOnly()
    }
