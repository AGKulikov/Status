...
/// Helper v25 uses Central only to bootstrap the protected ANCS link. At the same time it exposes
/// one non-advertised B4 relay on the iPhone GATT server. After the expected role handoff, KX11
/// reads battery/power/network from that relay on its already-working ANCS GATT owner. Classic
/// Bluetooth is never modified here.
final class ViewController: UIViewController {
    ...
    private let restoreIdentifier =
        "ru.natro.kx11ancshelper.peripheral.v25.single-link-g5"
    private let centralRestoreIdentifier =
        "ru.natro.kx11ancshelper.central.v25.geely-ancs-g4"
    ...
    override func viewDidLoad() {
        ...
        append("v25: выбран iPhone \(role.title); Peripheral сохраняет маршрут v11")
        append("Central bootstrap и B4 relay используют один post-handoff GATT owner")
    ...
    private func buildInterface() {
        ...
        titleLabel.text = "KX11 ANCS HELPER v25"
    ...
    private func connectCentral(_ peripheral: CBPeripheral, reason: String) {
        ...
        // deterministic: Helper v25 always resolves the next generation from a fresh advert.
    ...
    private func waitForCentralTelemetryRelay(_ reason: String,
        ...
        let item = DispatchWorkItem { [weak self] in
            ...
            self.centralHandoffActive = false
            self.centralRequireFreshAdvertisement = true
            self.append("B4 relay не подтвердил рабочий ANCS link; возвращаю Central bootstrap")
            self.scheduleCentralReconnect(reason: "fresh bootstrap after \(reason)")
        }
    ...
    private func armCentralConnectTimeout(_ peripheral: CBPeripheral,
        ...
        let item = DispatchWorkItem { [weak self, weak peripheral] in
            ...
            case .connected:
                ...
                peripheral.delegate = self
                if self.centralHandshake == .idle {
                    self.append("Central уже connected при проверке timeout; продолжаю discovery")
                    self.beginCentralDiscovery(peripheral)
                }
    ...
    private func useCurrentCentralCharacteristics(_ peripheral: CBPeripheral,
        ...
        guard control.properties.contains(.write),
              secure.properties.contains(.read) else {
            append("Geely_ANCS properties invalid: B2 WRITE and B3 READ required")
...
extension ViewController: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager,
        ...
        case .connected:
            cancelCentralConnectTimeout()
            if centralHandshake == .idle { beginCentralDiscovery(restored) }
        case .connecting:
    ...
    private func handleCentralDisconnect(_ peripheral: CBPeripheral, isReconnecting: Bool,
        ...
        if completedSecureHandoff && hardReset == nil {
            ...
            publishServiceIfPossible()
            append("Защищённый Central bootstrap завершён; второй Central connect не нужен")
        ...
        if isReconnecting {
            // Handles a restored connection originally created by an older Helper. New v25
