...
def main() -> None:
    ...
    assert "permitsUnprovableLocalOnlyDrain" in route
    assert "let unprovableLocalOnly" in route
    assert "!emptyLocalOnly && !unprovableLocalOnly" in route
    assert "if unprovableLocalOnly, let epoch" in route
