#!/usr/bin/env python3
"""Event-driven v70 telemetry, active data-SIM selection and generation fencing gate."""
...
def active_technology(data_service: str | None, services: dict[str, str]) -> str | None:
    """Portable model of the v70 dual-SIM selector."""
    if data_service is not None:
        return services.get(data_service)
    if len(services) == 1:
...
def main() -> None:
    ...
    for marker in (
        ...
        "import Network",
        "NSObject, CTTelephonyNetworkInfoDelegate",
        "telephony.delegate = self",
        "dataServiceIdentifierDidChange(_ identifier: String)",
        "telephony.dataServiceIdentifier",
        "byService[activeIdentifier]",
        "scheduleDataServiceRefreshes()",
        "dataServiceRefreshGeneration",
    ...
    assert "Array(byService.values)" not in source
    assert "technologies.contains" not in source
    ...
    # An inactive 2G SIM must not override the active LTE data line. Reversing the data-service
    # identifier must reverse the result without rebuilding the telemetry source.
    dual_sim = {"line-a": "2G", "line-b": "LTE"}
    assert active_technology("line-b", dual_sim) == "LTE"
    assert active_technology("line-a", dual_sim) == "2G"
    assert active_technology("missing", dual_sim) is None
    assert active_technology(None, dual_sim) is None
    assert active_technology(None, {"only": "LTE"}) == "LTE"
    ...
    print(
        "PASS: v70 active data-SIM telemetry, bounded CoreTelephony settling, authenticated "
        "one-shot refresh, control-first queues, single ATT write and generation fencing"
    )
