...
public final class CarRemoteClient {
    ...
    public func start() {
        ...
        // One full catalog plus initial states is more than 100 ATT20 frames. Natro coalesces
        // retries while that protected snapshot drains; five seconds keeps recovery prompt but
        // avoids waking the iOS main queue five times during a normal ten-second bootstrap.
        retryTimer = Timer.scheduledTimer(withTimeInterval: 5, repeats: true) {
            [weak self] _ in
