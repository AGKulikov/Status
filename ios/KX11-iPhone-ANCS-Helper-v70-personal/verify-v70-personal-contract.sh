...
python3 "$ROOT/verify_live_activity_v70_personal.py"

PROJECT="$ROOT/KX11ANCSHelper.xcodeproj/project.pbxproj"
[ "$(grep -Fc 'CURRENT_PROJECT_VERSION = 70;' "$PROJECT")" -eq 4 ]
[ "$(grep -Fc 'MARKETING_VERSION = 70.0;' "$PROJECT")" -eq 4 ]
...
grep -Fq 'source candidate; Apple-SDK CI and physical-device checks are release gates' \
    "$ROOT/RELEASE.txt"
grep -Fq 'message.contains("получен валидный C5 кадр автомобиля")' \
    "$ROOT/KX11ANCSHelper/ViewController.swift"
grep -Fq 'if shouldNotify { onChange?() }' "$ROOT/CarRemoteProtocolV1.swift"
grep -Fq 'withTimeInterval: 5' "$ROOT/CarRemoteProtocolV1.swift"
grep -Fq 'if becameConnected {' "$ROOT/KX11ANCSHelper/NatroLiveActivityManager.swift"
! grep -Fq 'becameConnected || runningCount == 0' \
    "$ROOT/KX11ANCSHelper/NatroLiveActivityManager.swift"
grep -Fq 'Build: 70' "$ROOT/RELEASE.txt"
grep -Fq 'Paired runtime: Natro 2.2.19' "$ROOT/RELEASE.txt"
grep -Fq 'UIApplication.shared.beginBackgroundTask' \
    "$ROOT/HelperSwitchRuntimeCoordinator.swift"
grep -Fq 'same_role_drain fired plannedMs=' \
    "$ROOT/HelperSwitchRuntimeCoordinator.swift"
grep -Fq 'properties: [.write, .writeWithoutResponse, .indicate]' \
    "$ROOT/HelperPeripheralRoute.swift"
grep -Fq 'permitsUnprovableLocalOnlyDrain' "$ROOT/HelperPeripheralRoute.swift"
grep -Fq 'if unprovableLocalOnly, let epoch' "$ROOT/HelperPeripheralRoute.swift"
grep -Fq 'static var helperVersion: String' \
    "$ROOT/KX11ANCSHelper/ANCSConnectionJournal.swift"
! grep -Fq '<key>aps-environment</key>' "$ROOT/KX11ANCSHelper/KX11ANCSHelper.entitlements"
! grep -Fq 'com.apple.security.application-groups' \
    "$ROOT/KX11ANCSHelper/KX11ANCSHelper.entitlements"
! grep -Fq 'com.apple.security.application-groups' \
    "$ROOT/NatroLiveActivityExtension/NatroLiveActivityExtension.entitlements"
grep -Fq 'struct NatroStopLiveActivityIntent: AppIntent' \
    "$ROOT/KX11ANCSHelper/NatroShortcuts.swift"
grep -Fq 'pushType: nil' "$ROOT/KX11ANCSHelper/NatroLiveActivityManager.swift"
grep -Fq 'LiveActivityPushProtocolV1' "$ROOT/CarRemoteProtocolV1.swift"

echo 'PASS: Helper v70 Personal Team, active data-SIM telemetry and entitlement-clean Live Activity controls'
