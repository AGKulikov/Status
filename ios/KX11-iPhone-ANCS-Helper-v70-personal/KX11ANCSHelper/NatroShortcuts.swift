...
@available(iOS 16.2, *)
struct NatroStopLiveActivityIntent: AppIntent {
    static var title: LocalizedStringResource = "Остановить Live Activity Natro"
    static var description = IntentDescription(
        "Немедленно завершает обе карточки Natro на экране блокировки."
    )
    static var openAppWhenRun = false

    @MainActor
    func perform() async throws -> some IntentResult & ReturnsValue<Bool> {
        let hadActivities = NatroLiveActivityManager.shared.runningCount > 0
        NatroLiveActivityManager.shared.stop()
        return .result(value: hadActivities)
    }
}
...
struct NatroAppShortcuts: AppShortcutsProvider {
    static var appShortcuts: [AppShortcut] {
        ...
        AppShortcut(
            intent: NatroStopLiveActivityIntent(),
            phrases: ["Остановить виджет \(.applicationName)"],
            shortTitle: "Остановить Live Activity",
