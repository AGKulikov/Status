...
private final class VehicleDashboardViewController: UIViewController {
    ...
    private func rebuild() {
        ...
        contentStack.addArrangedSubview(sectionTitle("УПРАВЛЕНИЕ"))
        contentStack.addArrangedSubview(makeCategoryGrid())
        view.layoutIfNeeded()
    ...
    private func makeCategoryGrid() -> UIView {
        let outer = UIStackView()
        outer.axis = .vertical
        outer.spacing = 8
        let sections = CarRemoteControlDefinition.Section.allCases
        for offset in stride(from: 0, to: sections.count, by: 2) {
            let row = UIStackView()
            row.axis = .horizontal
            row.spacing = 8
            row.distribution = .fillEqually
            for index in offset..<min(offset + 2, sections.count) {
                ...
                button.setTitle(categorySummary(section), for: .normal)
                button.setTitleColor(.white, for: .normal)
                button.setImage(UIImage(systemName: categoryIcon(section)), for: .normal)
                button.tintColor = categoryAccent(section)
                button.backgroundColor = card
                button.layer.cornerRadius = 18
                button.layer.cornerCurve = .continuous
                button.imageEdgeInsets = UIEdgeInsets(top: 0, left: 12, bottom: 0, right: 0)
                button.titleEdgeInsets = UIEdgeInsets(top: 0, left: 20, bottom: 0, right: 8)
    ...
    private func categorySummary(_ section: CarRemoteControlDefinition.Section) -> String {
        let controls = CarRemoteCatalogV1.controls.filter { $0.section == section }
        let available = controls.filter { manager.isAvailable($0.id) }.count
        let active = controls.filter {
            guard let state = manager.state(for: $0.id), state.known else { return false }
            return state.active || state.value != 0
        }.count
        let detail = manager.isDemoMode ? "\(controls.count) функций · ДЕМО"
            : available == 0 ? "Синхронизация…"
            : "\(available) доступно · \(active) активно"
        return "  \(section.rawValue)\n  \(detail)"
    }

    @objc private func categoryTapped(_ button: CategoryButton) {
        guard let section = button.section else { return }
        let controller = VehicleControlSectionViewController(
            remote: remote,
            section: section
        )
...
/// Tesla-style contextual screen: the overview stays short, while one category exposes its
/// controls as a two-column grid. Multi-stage functions always open an explicit value picker;
/// they are never reduced to an ambiguous on/off switch.
private final class VehicleControlSectionViewController: UIViewController {
    private let remote: CarRemoteClient
    ...
    init(remote: CarRemoteClient, section: CarRemoteControlDefinition.Section) {
        self.remote = remote
    ...
    private func rebuild() {
        ...
        var pending: [UIView] = []
        for definition in CarRemoteCatalogV1.controls where definition.section == section {
            let card = makeCard(definition)
            if definition.kind == .range {
                flush(&pending)
                stack.addArrangedSubview(card)
            } else {
                pending.append(card)
                if pending.count == 2 { flush(&pending) }
            }
        }
        flush(&pending)
        view.layoutIfNeeded()
    ...
    private func makeHeader() -> UIView {
        ...
        box.spacing = 12
        let icon = UIImageView(image: UIImage(systemName: categoryIcon(section)))
        icon.tintColor = categoryAccent(section)
        ...
        subtitle.text = manager.isDemoMode
            ? "ДЕМО · все значения изменяются локально"
            : manager.isANCSConnected ? "ANCS подключён · команды идут через Natro"
            : remote.statusText
        subtitle.font = .systemFont(ofSize: 12, weight: .medium)
        subtitle.textColor = manager.isDemoMode ? .systemOrange : .secondaryLabel
        subtitle.numberOfLines = 2
        labels.addArrangedSubview(title)
        labels.addArrangedSubview(subtitle)
        box.addArrangedSubview(icon)
        box.addArrangedSubview(labels)
        box.addArrangedSubview(UIView())
        box.layoutMargins = UIEdgeInsets(top: 5, left: 2, bottom: 9, right: 2)
        box.isLayoutMarginsRelativeArrangement = true
        return box
    }

    private func flush(_ pending: inout [UIView]) {
        guard !pending.isEmpty else { return }
        let row = UIStackView(arrangedSubviews: pending)
        row.axis = .horizontal
        row.spacing = 9
        row.distribution = .fillEqually
        if pending.count == 1 {
    ...
    private func makeCard(_ definition: CarRemoteControlDefinition) -> DashboardControlCard {
        let state = manager.state(for: definition.id)
        let available = manager.isAvailable(definition.id) && state?.available != false
        let control = DashboardControlCard(
            definition: definition,
    ...
    private func controlTapped(_ definition: CarRemoteControlDefinition) {
        ...
        case .levels, .options:
            let sheet = UIAlertController(title: definition.title, message: nil,
                                          preferredStyle: .actionSheet)
            for (value, title) in selectableValues(definition) {
                sheet.addAction(UIAlertAction(title: title, style: .default) { [weak self] _ in
                    self?.manager.send(controlID: definition.id, operation: .set, value: value)
                })
            }
            ...
            present(sheet, animated: true)
        case .toggle:
            perform(definition, operation: .toggle)
        case .action:
            perform(definition, operation: .activate)
        case .range:
            break
        }
    }
    ...
    private func perform(
        ...
        guard definition.requiresConfirmation else { execute(); return }
        let alert = UIAlertController(
            title: definition.title,
            message: "Проверьте автомобиль и убедитесь, что рядом с механизмом нет людей и препятствий.",
            preferredStyle: .alert
...
private final class DashboardControlCard: UIControl {
    ...
    init(
        ...
        let active = state?.known == true && state?.active == true
        self.backgroundColor = active ? accent(definition).withAlphaComponent(0.17) : background
    ...
    @objc private func sliderCommitted(_ slider: UISlider) {
        let step = max(1, definition.step)
        let rounded = Int32((Double(slider.value) / Double(step)).rounded()) * step
        commit?(min(definition.maximum, max(definition.minimum, rounded)))
    }
}
...
private func detail(
    ...
    if definition.kind == .action {
        return state.active ? "Выполнено" : (definition.requiresConfirmation
            ? "Команда с подтверждением" : "Готово")
    }
...
private func categoryIcon(_ section: CarRemoteControlDefinition.Section) -> String {
    switch section {
    case .climate: return "fanblades.fill"
    case .seats: return "seat.max.fill"
...
private func categoryAccent(_ section: CarRemoteControlDefinition.Section) -> UIColor {
    switch section {
    case .climate: return natroCyan
    case .seats: return .systemOrange
...
private final class CategoryButton: UIButton {
    var section: CarRemoteControlDefinition.Section?
}
