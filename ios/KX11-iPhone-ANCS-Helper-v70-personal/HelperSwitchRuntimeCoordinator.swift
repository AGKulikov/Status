...
public final class HelperSwitchRuntimeCoordinator {
    ...
    /// Keeps the short owner-zero -> same-role recreation interval runnable while iOS backgrounds
    /// the app after the remote central disconnects. Core Bluetooth background mode alone cannot
    /// guarantee that an ordinary DispatchQueue.asyncAfter deadline will execute on time once the
    /// last local manager has been released.
    private var recoveryBackgroundTask: UIBackgroundTaskIdentifier = .invalid
    private var recoveryBackgroundTaskStartedAtMs: UInt64?
    private var recoveryStartedAtMs: UInt64?
    private var localOwnersZeroAtMs: UInt64?
    private var drainArmedAtMs: UInt64?
    ...
    public init(
        ...
        if quarantinedCorruption {
            ...
            self.bootDetail = "Терминальный снимок v56 восстановлен в основной Route A"
            ...
            self.switchToken = nil
            self.pendingStartupEffects = []
            self.bootDetail = "Legacy-настройка перенесена в основной Route A"
            defaults.removeObject(forKey: Self.legacyRoleKey)
            try Self.persist(
                policy: policy,
                origin: nil,
    ...
    private func dispatch(_ effect: BleRoleSwitchPolicy.Effect) {
        ...
        case .freezeSourceIngress:
            beginRecoveryBackgroundTask(
                reason: "freeze_\(effect.role.rawValue)",
                epoch: effect.epoch,
        ...
        case .targetActive:
            ...
            endRecoveryBackgroundTask(outcome: "target_active")
            resetRecoveryTiming()
        ...
        case .failClosed:
            ...
            endRecoveryBackgroundTask(outcome: "fail_closed")
            resetRecoveryTiming()
    ...
    private func handleClosedCleanup(_ event: HelperBleRuntimeCoordinator.Event) {
        ...
        if cleanup.routeATerminal, cleanup.routeAOwnersZero,
           cleanup.routeBTerminal, cleanup.routeBOwnersZero {
            ...
            endRecoveryBackgroundTask(outcome: "closed_cleanup_complete")
            resetRecoveryTiming()
            publishSnapshot()
        }
    }
    ...
    private func armDrainDeadline(_ effect: BleRoleSwitchPolicy.Effect) {
        ...
        let delay = deadline > now ? deadline - now : 0
        onDiagnosticEvent?(
            "same_role_drain armed delayMs=\(delay), epoch=\(effect.epoch), " +
            "backgroundTask=\(recoveryBackgroundTask != .invalid)"
        )
        queue.asyncAfter(deadline: .now() + .milliseconds(Int(delay))) { [weak self] in
            guard let self, self.drainTimerToken == token else { return }
            self.drainTimerToken = nil
            let firedAt = Self.nowMs()
            let actualDelay = Self.elapsed(from: now, to: firedAt)
            let lateness = firedAt > deadline ? firedAt - deadline : 0
            self.onDiagnosticEvent?(
                "same_role_drain fired plannedMs=\(delay), actualMs=\(actualDelay), " +
                "latenessMs=\(lateness), backgroundTask=" +
                "\(self.recoveryBackgroundTask != .invalid)"
            )
            self.reduce { policy in
    ...
    private func beginRecoveryBackgroundTask(
        reason: String,
        epoch: BleRoleSwitchPolicy.Sequence,
        generation: BleRoleSwitchPolicy.Sequence
    ) {
        let now = Self.nowMs()
        if recoveryStartedAtMs == nil { recoveryStartedAtMs = now }
        guard recoveryBackgroundTask == .invalid else { return }
        let begin = { [weak self] () -> UIBackgroundTaskIdentifier in
            UIApplication.shared.beginBackgroundTask(
                withName: "Natro BLE same-role recovery"
            ) { [weak self] in
                self?.queue.async { [weak self] in
                    self?.expireRecoveryBackgroundTask()
                }
            }
        }
        ...
        onDiagnosticEvent?(
            "ble_recovery_background begin valid=\(identifier != .invalid), " +
            "reason=\(reason), epoch=\(epoch), generation=\(generation)"
        )
    }
    ...
    private func endRecoveryBackgroundTask(outcome: String) {
        let identifier = recoveryBackgroundTask
        ...
        recoveryBackgroundTaskStartedAtMs = nil
        onDiagnosticEvent?(
            "ble_recovery_background end outcome=\(outcome), durationMs=\(duration)"
        )
    ...
    private func recordLocalOwnersZero(
        epoch: BleRoleSwitchPolicy.Sequence,
        generation: BleRoleSwitchPolicy.Sequence,
        role: BleRoleSwitchPolicy.Role
    ) {
        let now = Self.nowMs()
        localOwnersZeroAtMs = now
        onDiagnosticEvent?(
            "ble_recovery owner_zero totalMs=" +
            "\(Self.elapsed(from: recoveryStartedAtMs, to: now)), " +
            "role=\(role.rawValue), epoch=\(epoch), generation=\(generation)"
        )
    }

    private func journalTargetStart(_ effect: BleRoleSwitchPolicy.Effect) {
        let now = Self.nowMs()
        onDiagnosticEvent?(
            "ble_recovery target_start totalMs=" +
            "\(Self.elapsed(from: recoveryStartedAtMs, to: now)), " +
            "ownerZeroToStartMs=\(Self.elapsed(from: localOwnersZeroAtMs, to: now)), " +
            "drainArmedToStartMs=\(Self.elapsed(from: drainArmedAtMs, to: now)), " +
            "epoch=\(effect.epoch), generation=\(effect.generation)"
        )
    }

    private func resetRecoveryTiming() {
        recoveryStartedAtMs = nil
        localOwnersZeroAtMs = nil
        drainArmedAtMs = nil
    }
    ...
    private func failRuntime(_ reason: String) {
        ...
        endRecoveryBackgroundTask(outcome: "runtime_failure")
        resetRecoveryTiming()
        publishSnapshot()
    }
