# KX11 ANCS Helper v70 Personal Team — Bluetooth vehicle control and local Live Activity

This build is the installable fallback for a free Apple Personal Team. It intentionally contains
no Push Notifications or App Groups entitlement, because Apple does not provision those
capabilities for a Personal Team. Live Activities start locally while Helper is in the foreground;
APNs push-to-start from Natro is unavailable. On iOS 17+, Lock Screen buttons use
`LiveActivityIntent` and deliver commands to the already-running Helper through a finite Darwin
notification channel without opening the app. The card-level URL remains a non-button fallback.

The Shortcuts app exposes both `Запустить Live Activity Natro` and
`Остановить Live Activity Natro`. Start opens Helper because iOS only permits this local
`Activity.request` path in the foreground. Stop ends both cards immediately without APNs.

Minimum supported iOS: 14.0. Helper v70 uses the system CryptoKit HKDF implementation for the
enrollment protocol; devices left on iOS 13.x cannot install this build.

Helper v70 pairs with the Natro C4-enrollment runtime and keeps Route A as the production
topology:
...
v63 does not weaken identity to a BLE name, MAC/address equality, nearest RSSI or manufacturer
...
Helper 70 keeps a bounded iOS background task alive across that same-role recovery boundary. The
fixed 750 ms drain is measured from monotonic uptime even after Helper moves to the background,
instead of being deferred until iOS next resumes the process. The journal records the requested
and actual drain time, lateness, owner-zero-to-target-start gap and background-task lifetime.

Helper 70 also treats C5 synchronization as one connection edge. Catalog and state callbacks no
longer retry foreground-only `Activity.request` or resend Live Activity provisioning while the app
is backgrounded. If no local card exists, the one permitted retry occurs when Helper actually
becomes active. This removes the ActivityKit/C5 main-queue storm found in the 2026-08-24 road log.
...
Without that explicit developer flag, Route B selection is rejected. v63 never runs both roles
...
An in-place v51 → v63 update keeps bundle ID `ru.natro.kx11ancshelper`, both stable Core Bluetooth
...
Helper 70 selects the radio class only from the service keyed by CoreTelephony's `dataServiceIdentifier`. It implements `CTTelephonyNetworkInfoDelegate`, so changing the line used for mobile data triggers an immediate bounded refresh even when NWPath remains cellular. An inactive 2G SIM can no longer override the active LTE/5G line. Diagnostics record only whether an active identifier exists, the service count and the resulting class; the identifiers themselves are never logged.
...
v63 retains the event-driven telemetry model. Battery, power, path and lock updates are event-driven.
...
Helper 70 embeds `NatroLiveActivityExtension` (minimum iOS 16.2) while the main Bluetooth app keeps
its iOS 14.0 deployment target. It maintains two simultaneous Lock Screen activities. The upper
climate panel contains the compact Monjaro asset, cabin/outdoor/set temperatures, +/- and four
user-selectable climate buttons. The lower panel exposes four large configurable favourites; the
complete catalog is kept in contextual in-app sections instead of a tiny ten-button grid.
Three-stage seat heat/ventilation, steering-wheel heat and fragrance show their real levels (plus
Auto where the vehicle supports it) instead of a binary lamp. Dynamic Island has compact, minimal
and expanded presentations for both panels. A Personal Team cannot provision the App Group, so
on iOS 17+ each tile uses `Button(intent:)` and a finite Darwin notification name to reach the
already-running Helper without opening it. Commands that move glass or other protected hardware
still use the system AppIntent confirmation. A card-level URL fallback is validated and confirmed
inside Helper, but is not attached to the control buttons. Controls are enabled by a fully
synchronised C5 vehicle route, independently of ANCS availability.

The dynamic `ContentState` carries only status, three temperatures, four numeric values and four
flag bytes. Vehicle name, chosen IDs, vehicle visibility and the 300×157 image are static activity
attributes/resources. The encoded-state guard is 3.5 KB, below ActivityKit's 4 KB maximum, so one
bad catalog snapshot cannot make all subsequent updates disappear.

Settings contain automatic start, vehicle visibility/name, four upper and four lower swappable
controls, explicit start/stop for both cards and a clearly marked demo mode. In normal mode both
cards are created only after the authenticated C5 catalog reaches `isSynced`, and both are ended
with immediate dismissal as soon as that route is lost. ANCS remains a separate notification
status and never disables vehicle controls. Demo is the explicit exception: it starts without a
car with 19 °C cabin, 7 °C outside and 22 °C set point; both activities and the in-app dashboard
update from that same local state. Every toggle, finite option, range and one-shot command changes
visible demo state. Demo never changes the real ANCS boolean returned to Apple Shortcuts.

Automatic mode reacts to the exact `isSynced` false-to-true edge, not to every subsequent C5 state
update. A background connection records one bounded deferred-start status; opening Helper is the
foreground retry boundary required by ActivityKit.

This Personal Team build never requests or exports ActivityKit push tokens. It creates both cards
locally with `pushType: nil`, only while Helper is in the foreground and the authenticated C5 car
route is connected (or demo mode is explicitly enabled). It still ends existing cards immediately
when that route is lost. Direct APNs push-to-start from Natro is unavailable in this build.

Apple Shortcuts receives four App Intents: `Получить состояние ANCS`, `Ожидать подключения ANCS`
(bounded to 25 seconds), `Запустить Live Activity Natro`, and `Остановить Live Activity Natro`.
The start intent opens Helper for the foreground-only local ActivityKit request; stop ends both
cards without APNs. iOS 26 does not expose an API for a
...
Helper keeps an always-on, bounded 1,600-line diagnostic journal covering Core Bluetooth power and
restoration, F201 publication/advertising, enrollment stages, CONTROL/C5 subscriptions, peer-ready
transitions, reducer phases/effects, Live Activity payload sizes and terminal/retry failures. Every
line has a process-session marker and sequence. Notification content, raw protocol payloads, keys,
tokens and device identifiers are filtered out. Settings can hide/show the journal, clear it and
export a plain `.log` file for troubleshooting; hiding affects presentation only, not collection.

## C5 vehicle control

Helper 70 retains C5, `D2D9E4C5-47F1-4E44-A8BB-A932FD5AF200`, without changing the occupied C2 role
...
not strand the session. C5 is encryption-required and is available only to the exact authenticated
CONTROL owner in the ACTIVE route generation. Route A uses indications toward Android and supports
both legacy writes-with-response and locally paced writes-without-response toward Helper;
diagnostic Route B reverses those directions. Both use the existing serialized ATT owner and clear
pending frames on freeze, disconnect or generation replacement.
...
gates. The 58-entry finite registry adds rear-seat heat/ventilation, ambient mode/effect/color/theme,
close-only commands for four windows plus the sunroof, four individual reading lamps, all cabin
lamps and fragrance level 0–3. The latter six use the exact Passenger 1.14.0 PA methods and remain
capability-gated. Massage is intentionally absent because this vehicle does not have it.
Window/sunroof opening and continuous seat movement were not exposed as unsafe one-tap actions.
The shared protocol reserves four read-only STATE frames outside the command registry: `0xFB` direct APNs
provider readiness, `0xFC` cabin temperature (tenths °C), `0xFD` outdoor temperature (tenths °C),
and `0xFE` exact ANCS readiness. This Personal Team Helper never advertises `0xFB` readiness.
...
```sh
./verify-v70-personal-contract.sh
```
...
For a Personal Team, leave Push Notifications and App Groups disabled for both targets; Xcode may
then create ordinary managed development profiles for the app and extension.

## Physical acceptance still required

Before calling v70 Personal Team stable:
...
7. Enable demo away from the car and verify both cards start, show 19°/7°/22°, four upper and four
...
- iOS controls background advertising contents and scheduling. v63 does not depend on a name.
