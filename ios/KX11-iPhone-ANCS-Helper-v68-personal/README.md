# KX11 ANCS Helper v68 Personal Team — Bluetooth vehicle control and local Live Activity
...
Minimum supported iOS: 14.0. Helper v68 uses the system CryptoKit HKDF implementation for the
enrollment protocol; devices left on iOS 13.x cannot install this build.

Helper v68 pairs with the Natro C4-enrollment runtime and keeps Route A as the production
topology:
...
Helper 68 keeps a bounded iOS background task alive across that same-role recovery boundary. The
...
Helper 68 embeds `NatroLiveActivityExtension` (minimum iOS 16.2) while the main Bluetooth app keeps
...
Helper 68 retains C5, `D2D9E4C5-47F1-4E44-A8BB-A932FD5AF200`, without changing the occupied C2 role
...
```sh
./verify-v68-personal-contract.sh
```
...
Before calling v68 Personal Team stable:
