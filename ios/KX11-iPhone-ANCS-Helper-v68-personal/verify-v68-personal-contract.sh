...
python3 "$ROOT/verify_live_activity_v68_personal.py"

PROJECT="$ROOT/KX11ANCSHelper.xcodeproj/project.pbxproj"
[ "$(grep -Fc 'CURRENT_PROJECT_VERSION = 68;' "$PROJECT")" -eq 4 ]
[ "$(grep -Fc 'MARKETING_VERSION = 68.0;' "$PROJECT")" -eq 4 ]
...
grep -Fq 'Build: 68' "$ROOT/RELEASE.txt"
grep -Fq 'Paired runtime: Natro 2.2.15' "$ROOT/RELEASE.txt"
...
echo 'PASS: Helper v68 Personal Team entitlement-clean Live Activity controls'
