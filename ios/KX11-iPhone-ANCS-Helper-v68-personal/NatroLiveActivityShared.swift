...
enum NatroLiveActivityPreferences {
    ...
    private enum Key {
        ...
        static let functionControls = "liveActivity.functionControls.v58"
    }
...
struct NatroLiveActivityAttributes: ActivityAttributes {
    ...
    struct ContentState: Codable, Hashable {
        /// 0 no car, 1 car connected/ANCS unknown, 2 car connected/ANCS off,
        /// 3 car connected/ANCS on, 4 demo.
        ...
        /// Decode-only bridge for a card left running by Helper 56. Never encoded by Helper 68.
        ...
        var vehicleConnected: Bool { (1...4).contains(statusCode) }
        var status: String {
            switch statusCode {
            case 4: return "ДЕМО · ANCS подключён"
            case 3: return "Магнитола · ANCS подключён"
            case 2: return "Магнитола · ANCS отключён"
            case 1: return "Магнитола · ANCS уточняется"
            default: return "Магнитола не подключена"
            }
        }
...
enum NatroLiveActivityCommandMailbox {
    ...
    private static let directNotificationPrefix =
        "ru.natro.kx11ancshelper.live-command.action."
    private static let actionKey = "liveCommand.action"
    private static let actionQueueKey = "liveCommand.queue.v62"
    private static let nonceKey = "liveCommand.nonce"

    // This directory is the Personal Team product. Its signed targets deliberately have no App
    // Group entitlement, so even probing an App Group container URL makes sandboxd emit
    // "client is not entitled". Keep this decision compile-time constant and
    // never touch an App Group API or suite-backed UserDefaults from either process.
    static let supportsSharedDelivery = false

    /// Personal Team profiles cannot carry the App Group used by the paid build.  A Live
    /// Activity intent still runs without opening the app, so use one Darwin notification name
    /// per finite command. Helper is already alive while its Bluetooth peripheral is connected
    /// and receives the command directly; no shared container or URL launch is required.
    static var directNotificationNames: [CFString] {
        supportedDirectActions.compactMap { directNotificationName(for: $0) }
    }

    private static var supportedDirectActions: [String] {
        var actions = ["temperature:-1", "temperature:1"]
        for control in NatroLiveControl.allCases {
            let action = control.commandAction
            actions.append(action)
            if requiresConfirmation(action) { actions.append("confirmed|\(action)") }
        }
        return Array(Set(actions)).sorted()
    }

    private static func directNotificationName(for action: String) -> CFString? {
        guard supportedDirectActions.contains(action) else { return nil }
        let encoded = Data(action.utf8).base64EncodedString()
            .replacingOccurrences(of: "+", with: "-")
            .replacingOccurrences(of: "/", with: "_")
            .replacingOccurrences(of: "=", with: "")
        return "\(directNotificationPrefix)\(encoded)" as CFString
    }

    static func action(fromDirectNotification name: CFNotificationName?) -> String? {
        guard let rawValue = name?.rawValue else { return nil }
        let raw = rawValue as String
        guard raw.hasPrefix(directNotificationPrefix) else { return nil }
        var encoded = String(raw.dropFirst(directNotificationPrefix.count))
            .replacingOccurrences(of: "-", with: "+")
        ...
        guard let data = Data(base64Encoded: encoded),
              ...
              supportedDirectActions.contains(action) else { return nil }
        return action
    }

    private static func defaults() -> UserDefaults { .standard }

    static func enqueue(_ action: String) {
        guard !action.isEmpty else { return }
        if !supportsSharedDelivery {
            guard let directName = directNotificationName(for: action) else { return }
            CFNotificationCenterPostNotification(
                CFNotificationCenterGetDarwinNotifyCenter(),
                CFNotificationName(directName),
                nil,
                nil,
                true
            )
        ...
        var queue = defaults.stringArray(forKey: actionQueueKey) ?? []
        queue.append(action)
        defaults.set(Array(queue.suffix(16)), forKey: actionQueueKey)
    ...
    static func consume() -> String? {
        guard supportsSharedDelivery else { return nil }
        let defaults = defaults()
        var queue = defaults.stringArray(forKey: actionQueueKey) ?? []
        if !queue.isEmpty {
            ...
            if queue.isEmpty {
                ...
                defaults.set(queue, forKey: actionQueueKey)
            }
            return action
        }
        // Consume a command left by Helper 61 before the queue migration.
...
enum NatroLiveActivityCommandLink {
    static func url(for action: String) -> URL {
        var components = URLComponents()
        components.scheme = "natrohelper"
        components.host = "command"
        components.queryItems = [URLQueryItem(name: "action", value: action)]
        return components.url ?? URL(string: "natrohelper://live-activity")!
    }

    static func action(from url: URL) -> String? {
        guard url.scheme == "natrohelper", url.host == "command",
