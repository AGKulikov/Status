...
/// Helper v30 uses two explicit Central links. A plain BLE bootstrap discovers a per-publication
/// dynamic namespace announced by a stable beacon and proves the
/// peer; only then does Core Bluetooth reconnect with RequiresANCS. Phase two rediscovers the
/// current B3 and sends ANCS-READY, while the iPhone-owned F05 relay carries telemetry plus
/// Android's post-CCCD ANCS-SUBSCRIBED proof. A bare didConnect or B4 read never turns the UI
/// green. Classic Bluetooth is never modified here.
final class ViewController: UIViewController {
    ...
    private enum CentralHandshake {
        ...
        case writingPair
        case readingSecure
        case writingAncsHandoff
        case writingAncsReady
        case ready
    }

    private enum CentralLinkPhase {
        case bootstrap
        case requiresAncs
    }
    ...
    // These fixed F04 UUIDs remain for the legacy iPhone-peripheral route. In Central mode v30
    ...
    // Generation 5 is iPhone-owned and exists only on the post-handoff Android->iPhone link.
    ...
    // Fresh restoration owners keep pending fixed-F04 operations out of the dynamic namespace.
    private let restoreIdentifier =
        "ru.natro.kx11ancshelper.peripheral.v30.dynamic-gatt-g5"
    private let centralRestoreIdentifier =
        "ru.natro.kx11ancshelper.central.v30.dynamic-geely-ancs"
    ...
    private var centralLinkPhase: CentralLinkPhase = .bootstrap
    private var centralPlannedAncsReconnect = false
    ...
    private var centralHandoffActive = false
    private var centralRelayProved = false
    private var centralAncsProved = false
    private var centralHandoffProofWorkItem: DispatchWorkItem?
    private let centralHandoffProofTimeout: TimeInterval = 30
    private let centralConnectTimeout: TimeInterval = 15
    private let centralConnectCallbackGrace: TimeInterval = 3
    ...
    override func viewDidLoad() {
        ...
        append("v30: выбран iPhone \(role.title); Peripheral сохраняет маршрут v11")
        append("Фаза 1: dynamic BLE/PAIR/B3; фаза 2: отдельный RequiresANCS reconnect")
        append("На RequiresANCS link заново проверяется только текущий dynamic B3")
        append("didConnect не считается ANCS: зелёный статус требует CCCD ACK + B4 данные")
        append("Connect timeout защищён от поздних didConnect после background/resume")
        append("Stable beacon объявляет новую GATT generation после каждого Android restart")
    ...
    deinit {
        ...
        centralHandoffProofWorkItem?.cancel()
    ...
    private func buildInterface() {
        ...
        titleLabel.text = "KX11 ANCS HELPER v30"
    ...
    private func continueCentralConnected(_ peripheral: CBPeripheral) {
        ...
        if centralLinkPhase == .requiresAncs {
            centralPlannedAncsReconnect = false
            centralReconnectFailureCount = 0
            centralHandoffActive = false
            centralRelayProved = false
            centralAncsProved = false
            publishServiceIfPossible()
            append("RequiresANCS didConnect получен; подтверждаю phase 2 записью ANCS-READY")
            setStatus("REQUIRES_ANCS · ПРОВЕРЯЮ B3", color: .systemOrange)
            beginCentralDiscovery(peripheral)
        } else if centralHandshake == .idle {
            beginCentralDiscovery(peripheral)
        }
    }
    ...
    private func startCentralRouteIfPossible() {
        ...
        publishServiceIfPossible()
        if centralHandoffActive {
            updateConnectionStatus()
            return
        }
        if centralLinkPhase == .bootstrap, !centralNamespaceResolved {
            if let previous = geelyPeripheral, previous.state != .disconnected {
        ...
        if let connected = centralManager.retrieveConnectedPeripherals(
                withServices: [centralServiceUUID]).first {
    ...
    private func applyCentralNamespace(_ generation: UInt16) {
    ...
    private func connectCentral(_ peripheral: CBPeripheral, reason: String) {
        guard runRequested, role == .central, centralManager.state == .poweredOn else { return }
        guard centralLinkPhase == .requiresAncs || centralNamespaceResolved else {
            startCentralScan()
            return
        }
        ...
        let requiresAncs = centralLinkPhase == .requiresAncs
        let options: [String: Any] = [
            CBConnectPeripheralOptionRequiresANCS: requiresAncs,
        ...
        // deterministic: Helper v30 resolves every Android publication from its beacon frame.
        centralManager.connect(peripheral, options: options)
        armCentralConnectTimeout(peripheral, replaceExisting: true)
        setStatus("CENTRAL · ПОДКЛЮЧЕНИЕ", color: .systemBlue)
        append("connect Geely_ANCS · phase="
            + (requiresAncs ? "requiresANCS" : "bootstrap")
            + " · RequiresANCS=\(requiresAncs) · AutoReconnect=false"
    ...
    private func beginCentralDiscovery(_ peripheral: CBPeripheral) {
        ...
        centralAncsProved = false
        if !requiresAncs {
            centralLinkPhase = .bootstrap
            centralPlannedAncsReconnect = false
            lastAndroidReadAt = nil
        }
        centralHandoffProofWorkItem?.cancel()
        centralHandoffProofWorkItem = nil
        // Ask for D2D9 explicitly. The v23 unfiltered service enumeration could complete without
        // this custom service even though the immediately preceding filtered scan advertised it.
        // uuidNotAllowed was observed at the B2/B3 characteristic stage, not here.
        peripheral.discoverServices([centralServiceUUID])
        setStatus(requiresAncs ? "REQUIRES_ANCS · ИЩУ B3" : "ПОДКЛЮЧЕНО · ИЩУ GATT",
                  color: .systemOrange)
        append((requiresAncs ? "Phase 2" : "Bootstrap")
            + " connected; discover dynamic D2D9 service " + centralServiceUUID.uuidString)
    }
    ...
    private func confirmCentralTelemetryRelay(_ source: String) {
        guard runRequested, role == .central else { return }
        let firstProof = !centralRelayProved
        centralRelayProved = true
        if centralHandoffActive {
            if firstProof { append("B4 relay подтверждён · \(source)") }
            if centralAncsProved {
                centralHandoffProofWorkItem?.cancel()
                centralHandoffProofWorkItem = nil
                setStatus("ANCS + ТЕЛЕМЕТРИЯ АКТИВНЫ", color: .systemGreen)
            } else {
                setStatus("B4 АКТИВЕН · ЖДУ ANCS ACK", color: .systemOrange)
                observeOptionalCentralTelemetryRelay("B4 relay · \(source)")
            }
        }
        updateTelemetryLabel()
    }
    ...
    private func confirmCentralAncsReady(_ source: String) {
        guard runRequested, role == .central, centralHandoffActive else { return }
        let firstProof = !centralAncsProved
        centralAncsProved = true
        if firstProof { append("Android подтвердил обе ANCS CCCD · \(source)") }
        if centralRelayProved {
            centralHandoffProofWorkItem?.cancel()
            centralHandoffProofWorkItem = nil
            setStatus("ANCS + ТЕЛЕМЕТРИЯ АКТИВНЫ", color: .systemGreen)
        } else {
            setStatus("ANCS АКТИВЕН · ЖДУ B4 ДАННЫЕ", color: .systemOrange)
            observeOptionalCentralTelemetryRelay("ANCS ACK без B4 payload")
        }
        updateTelemetryLabel()
    }

    private func observeOptionalCentralTelemetryRelay(_ reason: String,
                                                      timeout: TimeInterval? = nil) {
        guard runRequested, role == .central, centralHandoffActive else { return }
        centralHandoffProofWorkItem?.cancel()
        let item = DispatchWorkItem { [weak self] in
            guard let self = self else { return }
            self.centralHandoffProofWorkItem = nil
            guard self.runRequested, self.role == .central,
                  self.centralHandoffActive,
                  !(self.centralAncsProved && self.centralRelayProved) else { return }
            self.append("Полная готовность пока не доказана после \(reason); phase-2 link "
                + "сохраняю без reset")
            let status = self.centralAncsProved
                ? "ANCS АКТИВЕН · ЖДУ B4 ДАННЫЕ"
                : self.centralRelayProved
                ? "B4 АКТИВЕН · ЖДУ ANCS ACK"
                : "REQUIRES_ANCS OK · ЖДУ ANDROID"
            self.setStatus(status, color: .systemOrange)
            self.updateTelemetryLabel()
        }
        centralHandoffProofWorkItem = item
        DispatchQueue.main.asyncAfter(
            deadline: .now() + (timeout ?? centralHandoffProofTimeout), execute: item)
    }
    ...
    /// republishes its GATT database. A timeout delivered while iOS was suspended can run before
    /// the already-queued `didConnect` callback. Give that callback one short main-queue turn and
    /// use a token so an obsolete timeout can never tear down a newer connection.
    private func armCentralConnectTimeout(_ peripheral: CBPeripheral,
                                          delay: TimeInterval? = nil,
                                          allowCallbackGrace: Bool = true,
        ...
        let expectedFireAt = Date().addingTimeInterval(timeout)
        let item = DispatchWorkItem { [weak self, weak peripheral] in
            ...
            case .connecting where allowCallbackGrace:
                let lateBy = max(0, Date().timeIntervalSince(expectedFireAt))
                if lateBy >= 1 {
                    self.append("Central connect timeout доставлен после background с опозданием "
                        + "\(Int(lateBy)) с; жду queued didConnect ещё "
                        + "\(Int(self.centralConnectCallbackGrace)) с")
                } else {
                    self.append("Central всё ещё connecting; жду didConnect ещё "
                        + "\(Int(self.centralConnectCallbackGrace)) с")
                }
                self.armCentralConnectTimeout(
                    peripheral,
                    delay: self.centralConnectCallbackGrace,
                    allowCallbackGrace: false,
                    replaceExisting: true
                )
            case .connecting, .disconnecting, .disconnected:
                self.resetCentralLink(
                    reason: "connect timeout after callback grace; fresh D2D9 advertisement required"
                )
            @unknown default:
                self.resetCentralLink(reason: "unknown connect state after timeout")
            }
        }
    ...
    private func useCurrentCentralCharacteristics(_ peripheral: CBPeripheral,
        ...
        if centralLinkPhase == .requiresAncs {
            guard secure.properties.contains(.write) else {
                append("RequiresANCS B3 properties invalid: WRITE required")
                resetCentralLink(reason: "RequiresANCS B3 is not writable")
                return true
            }
            cancelCentralDiscoveryWork()
            centralCharacteristicDiscoveryAttempt = 0
            centralServiceRediscoveryAttempt = 0
            append("D2D9 phase-2 SECURE characteristic ready · \(source)")
            writeCentralAncsReady(peripheral)
            return true
        }
        guard let bootstrapControl = control,
              bootstrapControl.properties.contains(.write),
              secure.properties.contains(.read),
              secure.properties.contains(.write) else {
            append("Geely_ANCS properties invalid: B2 WRITE and B3 READ/WRITE required")
            resetCentralLink(reason: "Geely_ANCS characteristic properties invalid")
            return true
        }
        centralControlCharacteristic = bootstrapControl
        cancelCentralDiscoveryWork()
        centralCharacteristicDiscoveryAttempt = 0
        centralServiceRediscoveryAttempt = 0
        append("D2D9 bootstrap characteristics ready · \(source)")
    ...
    private func markCentralReady(_ peripheral: CBPeripheral, value: Data?) {
        ...
        requestCentralAncsHandoff(peripheral)
    }

    private func requestCentralAncsHandoff(_ peripheral: CBPeripheral) {
        guard centralLinkPhase == .bootstrap,
              let secure = centralSecureCharacteristic else {
            resetCentralLink(reason: "SECURE B3 missing before ANCS handoff")
            return
        }
        centralHandshake = .writingAncsHandoff
        peripheral.writeValue(Data("ANCS-HANDOFF".utf8), for: secure, type: .withResponse)
        setStatus("BOOTSTRAP OK · ГОТОВЛЮ ANCS LINK", color: .systemOrange)
        append("WRITE ANCS-HANDOFF → encrypted B3")
    }

    private func writeCentralAncsReady(_ peripheral: CBPeripheral) {
        guard centralLinkPhase == .requiresAncs,
              let secure = centralSecureCharacteristic else {
            resetCentralLink(reason: "SECURE B3 missing before phase-2 proof")
            return
        }
        centralHandshake = .writingAncsReady
        peripheral.writeValue(Data("ANCS-READY".utf8), for: secure, type: .withResponse)
        setStatus("REQUIRES_ANCS · ПОДТВЕРЖДАЮ LINK", color: .systemOrange)
        append("WRITE ANCS-READY → phase-2 dynamic B3")
    }

    private func responseData(for characteristic: CBCharacteristic) -> Data? {
        switch characteristic.uuid {
        case infoUUID:
            return Data("\(logicalName)/20/realtime-single-link".utf8)
        case secureUUID:
    ...
    private func updateTelemetryLabel() {
        if role == .central {
            ...
            telemetryLabel.text = "\(batteryDescription()) · \(currentNetworkType()) · "
                ...
                + (centralHandoffActive ? "ANCS owner" : "bootstrap")
            return
        }
    ...
    private func updateConnectionStatus() {
        guard runRequested else { return }
        if role == .central {
            if centralHandoffActive && centralAncsProved && centralRelayProved {
                setStatus("ANCS + ТЕЛЕМЕТРИЯ АКТИВНЫ", color: .systemGreen)
            } else if centralHandoffActive && centralAncsProved {
                setStatus("ANCS АКТИВЕН · ЖДУ B4 ДАННЫЕ", color: .systemOrange)
            } else if centralHandoffActive && centralRelayProved {
                setStatus("B4 АКТИВЕН · ЖДУ ANCS ACK", color: .systemOrange)
            } else if centralHandoffActive {
                setStatus("REQUIRES_ANCS OK · ЖДУ ANDROID", color: .systemOrange)
            } else if centralHandshake == .ready {
                setStatus("BOOTSTRAP OK · ЖДУ ANCS", color: .systemOrange)
...
extension ViewController: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager,
        ...
        centralNamespaceResolved = false
        geelyPeripheral = nil
        append("Central restore owner отброшен до свежего namespace beacon · "
    ...
    private func handleCentralDisconnect(_ peripheral: CBPeripheral, isReconnecting: Bool,
        ...
        let plannedAncsReconnect = centralPlannedAncsReconnect
            && centralLinkPhase == .requiresAncs
        let completedSecureHandoff = centralHandshake == .ready
        let hardReset = centralHardResetReason
        centralHardResetReason = nil
        if plannedAncsReconnect && hardReset == nil {
            centralPlannedAncsReconnect = false
            centralRequireFreshAdvertisement = false
            centralReconnectFailureCount = 0
            clearCentralRuntime(keepPeripheral: true)
            guard runRequested, role == .central else { return }
            append("Bootstrap link released; открываю phase 2 и перепроверяю dynamic B3")
            scheduleCentralReconnect(reason: "planned RequiresANCS phase 2")
            return
        }
        centralRequireFreshAdvertisement = true
        centralNamespaceResolved = false
        clearCentralRuntime(keepPeripheral: false)
        guard runRequested, role == .central else { return }
        if completedSecureHandoff && hardReset == nil {
            centralHandoffActive = true
            publishServiceIfPossible()
            append("RequiresANCS phase подтверждена B3 и передана Android ANCS client")
            if centralAncsProved {
                confirmCentralAncsReady("сохранённый Android ACK")
            } else if centralRelayProved || !telemetrySubscribers.isEmpty {
                confirmCentralTelemetryRelay("relay уже прочитан/подписан")
            } else {
                setStatus("REQUIRES_ANCS OK · ЖДУ ANDROID", color: .systemOrange)
                observeOptionalCentralTelemetryRelay("handoff relay observation")
            }
            return
        }
        if isReconnecting {
            // Handles a restored connection originally created by an older Helper. New v30
    ...
    func peripheral(_ peripheral: CBPeripheral,
        ...
        if let error = error {
            ...
            if isCentralUuidNotAllowedError(error) {
                ...
                // indefinitely in the in-car trace. End the unverified bootstrap once; Android
                // rotates its dynamic namespace after the disconnect, so the next scan cannot
                // reuse the prohibited service/characteristic handles.
        ...
        guard useCurrentCentralCharacteristics(peripheral, service: service,
            ...
            append("Geely_ANCS incomplete: CONTROL/SECURE required; повторяю без overlap")
    ...
    func peripheral(_ peripheral: CBPeripheral, didWriteValueFor characteristic: CBCharacteristic,
        ...
        if characteristic.uuid == centralSecureUUID,
           centralHandshake == .writingAncsHandoff {
            if let error = error {
                let detail = centralErrorDescription(error)
                append("WRITE ANCS-HANDOFF error: \(detail)")
                resetCentralLink(reason: "ANCS handoff write failed · \(detail)")
            } else {
                centralPlannedAncsReconnect = true
                centralLinkPhase = .requiresAncs
                setStatus("ПЕРЕХОЖУ НА REQUIRES_ANCS", color: .systemOrange)
                append("ANCS-HANDOFF accepted; закрываю только bootstrap Central link")
                centralManager.cancelPeripheralConnection(peripheral)
            }
            return
        }
        if characteristic.uuid == centralSecureUUID,
           centralHandshake == .writingAncsReady {
            if let error = error {
                ...
                resetCentralLink(reason: "phase-2 ANCS-READY failed · \(detail)")
            } else {
                ...
                publishServiceIfPossible()
                setStatus("REQUIRES_ANCS OK · ЖДУ ANDROID", color: .systemOrange)
                append("ANCS-READY accepted; Android может безопасно открыть same-link client")
                observeOptionalCentralTelemetryRelay("phase-2 ANCS-READY")
            }
            return
        }
    }
...
extension ViewController: CBPeripheralManagerDelegate {
    ...
    func peripheralManager(_ peripheral: CBPeripheralManager, central: CBCentral,
        ...
        confirmCentralTelemetryRelay("CCCD subscription")
        updateConnectionStatus()
    ...
    func peripheralManager(_ peripheral: CBPeripheralManager, central: CBCentral,
        ...
        if role == .central && centralHandoffActive && telemetrySubscribers.isEmpty {
            // A CCCD unsubscribe is not evidence that the protected ANCS owner failed. Android
            // can briefly rebuild its local client while the iPhone GATT owner remains valid.
            // Restarting Central here tears down the very handoff that already passed B3/B4.
            append("B4 subscription завершена; подтверждённый ANCS owner не сбрасываю")
        }
        updateTelemetryLabel()
        updateConnectionStatus()
    }
    ...
    func peripheralManager(_ peripheral: CBPeripheralManager,
        ...
        if isLocalTelemetryUUID(request.characteristic.uuid) {
            ...
            confirmCentralTelemetryRelay("B4 read")
            updateTelemetryLabel()
