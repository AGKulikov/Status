...
/// Helper v43 keeps one long-lived RequiresANCS Central owner and performs one minimal security
...
/// beacon by itself is never destructive evidence. HA1208 adds a UInt24 publication nonce to
...
final class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        append("v43: HA1208 publication nonce + bounded missing-didConnect recovery")
    ...
    private func buildInterface() {
        ...
        titleLabel.text = "KX11 ANCS HELPER v43"
    ...
    private func lastAutomaticPublicationAdoption(
        for ownerID: UUID
    ) -> UInt32? {
        guard let record = UserDefaults.standard.string(
                forKey: centralLastAutomaticPublicationAdoptionPreference) else { return nil }
        let fields = record.split(separator: "|", omittingEmptySubsequences: false)
        guard fields.count == 2,
              String(fields[0]) == ownerID.uuidString,
              let nonce = UInt32(fields[1], radix: 16),
    ...
    private func centralPublicationNonceEligibleForCurrentRoot(
        ...
        case .automaticPublication:
            let notAlreadyAdopted =
                lastAutomaticPublicationAdoption(for: ownerID) != nonce
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral,
        ...
        if let decodedIdentity = decodedIdentity,
           decodedIdentity.hasPublicationAuthority {
            advertisedIdentity = rememberExactPublicationIdentity(
                peripheral, identity: decodedIdentity) ? decodedIdentity : nil
