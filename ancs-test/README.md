...
11. Если прямой ANCS отсутствует — пассивную подписку на все notify/indicate
    характеристики фирменного ECARX-сервиса
    `61555e49-79c5-4d1f-b591-d97975f5e3e5` с выводом HEX и безопасного ASCII.

## Диагностические характеристики v4
...
8. После состояния `ANCS READY` отправьте на iPhone новое уведомление.
   Если вместо ANCS найден внутренний ECARX proxy, дождитесь
   `PROXY SNIFFER READY · ОТПРАВЬТЕ УВЕДОМЛЕНИЕ` и отправьте уведомление тогда.
...
| `PROXY SNIFFER READY` | Все доступные notify/indicate каналы ECARX proxy подписаны |
| `PROXY DATA RECEIVED` | Фирменный модуль передал пакет; HEX/ASCII добавлены в журнал и список |
| `PROXY SERVICE · НЕТ NOTIFY-КАНАЛОВ` | Сервис найден, но пассивная подписка невозможна |
...
## Пассивный ECARX proxy sniffer

В реальном KX11 на коррелированном адресе обнаружены стандартные сервисы
`1800`, `1801`, `180A` и неизвестный сервис `61555e49…`, но не Apple ANCS
`7905…`. Версия v4 после включения Service Changed последовательно подписывается
на каждую характеристику `61555e49…` с флагом Notify/Indicate. Она ничего не
записывает в фирменные характеристики и не угадывает команды протокола.

После `PROXY SNIFFER READY` новое уведомление на iPhone используется как
контрольное событие. Любой полученный пакет появляется как `PROXY DATA RECEIVED`
с UUID канала, длиной, HEX и печатным ASCII. Отсутствие пакетов означает, что
уведомления либо не проходят через этот сервис, либо передача требует неизвестной
команды/handshake.
